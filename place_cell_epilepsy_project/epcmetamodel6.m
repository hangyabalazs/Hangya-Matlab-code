function [PC LC T] = epcmetamodel6(constB,constA,constM,k_LP0)
%EPCMETAMODEL6   Rate constant dynamics for kinetic models (Model4B).
%   EPCMETAMODEL6 models temporal changes of low rate cell-to-place cell
%   rate constant (place cell-to-low rate cell rate constant is kept fixed.)
%   It calls EPCKINETICMODEL6 for each day and hour (two nested cycles).
%   EPCKINETICMODEL6 simulates one hour with given rate constants.
%
%   EPCMETAMODEL6 simulates rate constant changes as follows. Each day
...
%   change (subtraction of constant 'M').
%
%   [PC LC T] = EPCMETAMODEL6(B,A,M) runs the model with the constants
...
%
%   [PC LC T] = EPCMETAMODEL6(B,A,M,K_LP0) takes the initial LC to PC rate
...
%
%   See also EPCKINETICMODEL2, EPCFIT6 and EPCERROR6.
