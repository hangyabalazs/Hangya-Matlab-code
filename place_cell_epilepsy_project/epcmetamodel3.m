function [PC LC T] = epcmetamodel3(constB,constA,k_PL0)
%EPCMETAMODEL3   Rate constant dynamics for kinetic models (Model2B).
%   EPCMETAMODEL3 models temporal changes of place cell-to-low rate cell
...
%
%   EPCMETAMODEL3 simulates rate constant changes as follows. Each day
%   begins with an abrupt increase, followed by a power law decay. The
%   dynamics is of the form a*(t^-b).
%
%   [PC LC T] = EPCMETAMODEL3(B,A) runs the model with the constants given 
%   as  input arguments. It returns the dynamics of place cells (PC) and
%   low rate cells (LC) with their corresponding time vector (T).
%
%   [PC LC T] = EPCMETAMODEL3(B,A,K_PL0) takes the initial PC to LC rate
...
%
%   See also EPCKINETICMODEL, EPCFIT3 and EPCERROR3.
