function [PC LC T] = epcmetamodel2(constM,k_PL0)
%EPCMETAMODEL2   Rate constant dynamics for kinetic models (Model1A).
%   EPCMETAMODEL2 models temporal changes of place cell percentage. Both
...
%
%   EPCMETAMODEL2 simulates PC percentage changes as follows. Each day
...
%   constant ('M'), that is multiplyed with the current PC percentage to
%   provide the new value.
%
%   [PC LC T] = EPCMETAMODEL2(M) runs the model with the constant M given 
%   as  input argument. It returns the dynamics of place cells (PC) and
%   low rate cells (LC) with their corresponding time vector (T).
%
%   [PC LC T] = EPCMETAMODEL2(M,K_PL0) takes the initial PC to LC rate
...
% cps = 60 + [0 30 4*60 3*24*60 3*24*60+30 3*24*60+4*60 7*24*60 7*24*60+30 7*24*60+4*60];
