function [optinitparams optparams optdynamics] = epcfitcall5
%EPCFITCALL5   Fits kinetic model with different initial parameters.
%   [OPTINITPARAMS OPTPARAMS OPTDYNAMICS] = EPCFITCALL5 calls EPCFIT5 with
...
%
%   See also EPCFIT5 and EPCERROR5.
    ...
    for k2 = 1:length(constA)
        for k3 = 1:length(constM)
            for k4 = 1:length(k_LP0)
                disp([k1 k2 k3 k4])
                [cB cA cM kLP0 Rc] = epcfit5(constB(k1),constA(k2),constM(k3),k_LP0(k3));
                if Rc < Ri
                    Ri = Rc;
