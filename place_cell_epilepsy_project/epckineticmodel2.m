function [PC LC T] = epckineticmodel2(pc,k_PL,k_LP,isplot)
%EPCKINETICMODEL2   Simulation of stochastic 'kinetic reaction'.
%   EPCKINETICMODEL2 simulates a simple reversible reaction with two rate
%   constants.
%   Reaction: PC <-> LC
%   It is a simulation of a continuous time discrete state Markov process
%   (equivalent to M/M/1 problem in queuing theory).
%
%   EPCKINETICMODEL2(PC,K_PL,K_LP) receives the initial percentage of PC as
%   well as the PC to LC and LC to PC rate constants as input arguments. In
%   this implementation K_PL and K_PL can change in time; thus, they should
%   be given as time-resolved vectors.
%
%   [PC LC T] = EPCKINETICMODEL2(PC,K_PL,K_LP) returns the concentrations
%   (PC, LC) along with their time vector (T).
%
%   [PC LC T] = EPCKINETICMODEL2(PC,K_PL,K_LP,ISPLOT) takes a fourth input
%   argument. If ISPLOT = 1, the dynamics are displayed (default).
%
%   See also EPCKINETICMODEL and EPCMETAMODEL.
...
%     disp(t)
    ...
    [pc,lc,t] = onestep(pc,lc,t,k_PL(floor(t)+1),k_LP(floor(t)+1));    % calculate one simulation step
