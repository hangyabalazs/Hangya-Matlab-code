function [R p] = regression_surrogate(cellid,varargin)
%REGRSSION_SURROGATE   Control for linear regression.
%   REGRESSION_SURROGATE(CELLID) calculates linear regression coefficients
%   for firing rate variables (aligned to stimulus and response onset) and
%   various task-related regressors, with the firing rate vector flipped
%   (trials start from last and go to first).
%
%   See also REGRESION_ANALYSIS and NBREGRESSION.
...
y = fr_iti(hitinx);
X = [ones(length(hitinx),1) flipud(gort(hitinx))];
