...
pChATactinx = [384 428 461 464 543 559];
numChAT = length(ChATactinx);
numpChAT = length(pChATactinx);
...
u1 = [v01(ChATactinx) v01(PVactinx) v01(setdiff(NTactinx,pChATactinx)) v01(pChATactinx)]';
u2 = [v02(ChATactinx) v02(PVactinx) v02(setdiff(NTactinx,pChATactinx)) v02(pChATactinx)]';
u3 = [v03(ChATactinx) v03(PVactinx) v03(setdiff(NTactinx,pChATactinx)) v03(pChATactinx)]';
u4 = [v04(ChATactinx) v04(PVactinx) v04(setdiff(NTactinx,pChATactinx)) v04(pChATactinx)]';
...
celltypes = {1:numChAT; numChAT+1:numChAT+numPV; numChAT+numPV+1:numChAT+numPV+numNT};
vars = [v1 v2];
vars2 = [u1 u2];
pc11 = llda(vars,celltyps,vars2,celltypes);

% LDA #2
vars = [v3 v4];
vars2 = [u3 u4];
pc21 = llda(vars,celltyps,vars2,celltypes);


% Plot
figure   % plot
plot(pc11(celltypes{3}),pc21(celltypes{3}),'o','MarkerSize',5,'Color',[0.7 0.7 0.7],'MarkerFaceColor',[0.7 0.7 0.7]);
hold on
plot(pc11(celltypes{1}),pc21(celltypes{1}),'o','MarkerSize',12,'MarkerFaceColor',[0 0.8 0],'MarkerEdgeColor',[0 0.8 0]);
plot(pc11(celltypes{2}),pc21(celltypes{2}),'o','MarkerSize',12,'MarkerFaceColor',[0.8 0 0],'MarkerEdgeColor',[0.8 0 0]);
plot(pc11(end-numpChAT+1:end),pc21(end-numpChAT+1:end),'o','MarkerSize',12,'MarkerFaceColor',[0 0 0.7],'MarkerEdgeColor',[0 0 0.7]);
keyboard

% Find putative ChATs
pinx = find(pc11>10&pc21>1.5);
inxset = [ChATactinx PVactinx setdiff(NTactinx,pChATactinx) pChATactinx];
rpinx = inxset(pinx);
pids = tags(rpinx);
for k = 1:length(pids)
    cid = pids{k};
...
% -------------------------------------------------------------------------
function PC1 = llda(vars,celltyps,vars2,celltypes)
