function error_list = nbresponsesorter_baseline(I,issave)
%NBRESPONSESORTER_BASELINE   Peri-event time histogram.
%   NBRESPONSESORTER_BASELINE(I,ISSAVE) calculates 'doubly adaptive' PSTHs for a
...
%
%   ERROR_LIST = NBRESPONSESORTER_BASELINE(I,ISSAVE) returns a structure with all
...
%   automatically.
%
%   NBRESPONSESORTER_BASELINE is a version of NBRESPONSESORTER for
%   calculating baseline firing rate.
%
%   See also NBRESPONSESORTER.
...
global DATAPATH
fs = filesep;
resdir = [DATAPATH 'NB' fs 'responsesorter_baseline' fs];   % results directory
...
if isempty(I)
    ...
    I = find(ptinx);
    cellids = [CELLIDLIST(I) 'n045_121217x_4.6'];
else
...
for k = 1:NumCells
    ...
    try
        ...
        [psth, spsth, ~, ~, spt, stats] = ultimate_psth(cellid,'trial',...
            ...
            'baselinewin',[-1.1 -0.6],'testwin',[0 0.5],'relative_threshold',0.1);
