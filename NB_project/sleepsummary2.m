...
[hits,fas,gos,nogos,hitrate,farate,discrimination,dprime,engagement,...
    ...
    fr_iti fr_iti05 fr_iti10 fr_trial fr_trialnoresp] = deal(nan(NumTrials-1,1));
for iT = 1:NumTrials-1     % last trial may be incomplete
    ...
    % Firing rate in the full trial except 0-0.2 s after response
    lim1 = TE.TrialStart(iT);
    lim2 = TE.TrialStart(iT+1);
    lspikes = SP;   % absolute spikes
    lspikes2 = lspikes(lspikes>lim1&lspikes<lim2);   % spikes in the time window
    
    if ~isnan(TE.Hit(iT)) || ~isnan(TE.FalseAlarm(iT))
        inlim1 = 0;  % start of time window
        inlim2 = 0.2;   % end of time window
        lspikes = spikes_response{iT};   % spikes relative to the event
        lspikes2s = lspikes(lspikes>inlim1&lspikes<inlim2);   % spikes in the time window
        fr_trialnoresp(iT) = (length(lspikes2) - length(lspikes2s)) / ((lim2 - lim1) - (inlim2 - inlim1));   % firing rate in the time window
    else
        fr_trialnoresp(iT) = fr_trial(iT);
    end
end
...
inx1 = 1:21;
inx2 = NumTrials-21:NumTrials-2;
fr_first20hits = fr_trialnoresp(inx1);   % first 20 hits
fr_last20hits = fr_trialnoresp(inx2);   % last 20 hits

% RT for first and last 20 hits
inx1 = hitinx(1:20);
inx2 = hitinx(end-19:end);
rt_first20hits = gort(inx1) - nanmedian(gort);   % first 20 hits
rt_last20hits = gort(inx2) - nanmedian(gort);   % last 20 hits