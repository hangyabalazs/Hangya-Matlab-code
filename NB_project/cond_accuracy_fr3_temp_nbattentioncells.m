...
% note: FR split included median FR for both groops
...
prestimfreq = nan(1,NUMtrials-1);
lim1 = g.window(1);   % window boundaries
lim2 = g.window(2);
for k = 1:NUMtrials-1   % loop through trials
    if g.limit2iti
...
shortRT = filterTrials(cellid,'event_type','trial','event','StimulusOn',...
    'event_filter','selectRT','filterinput',[0.1 0.5]);
longRT = filterTrials(cellid,'event_type','trial','event','StimulusOn',...
    'event_filter','selectRT','filterinput',[0.5 0.9]);
SI = [20 30 40 50];
...
[C.condPerfS C.condGoPerfS C.condNoGoPerfS C.condActionPerfS ...
    C.condRespS C.condGoRespS C.condNoGoRespS C.condCorrectRespS nobS nobhS nobfS nobmS nobcS ...
    C.condPerfL C.condGoPerfL C.condNoGoPerfL C.condActionPerfL ...
    C.condRespL C.condGoRespL C.condNoGoRespL C.condCorrectRespL nobL nobhL nobfL nobmL nobcL] = ...
    deal(zeros(1,4));
for iB = 1:4   % loop through stim. intensities
    bins = TE.StimulusDuration == SI(iB);
