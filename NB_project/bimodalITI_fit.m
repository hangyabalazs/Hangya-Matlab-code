...
for iS = 1:numSessions
    ...
    % RT vs ITI
    TE2 = filterTE(TE,ismember(TE.StimulusDuration,ints(I)));
%     TE2 = TE;
...
errorshade(cnts,smnsmedian,smnsmedian_se,...
    'LineColor',mean(clr(I,:),1),'ShadeColor',mean(clr(I,:),1))
...
param.tau = 0;
% param.psi = 0.3;
% [param err] = fitHazardRate(param,alliti2,allrt2,@bimodal_hazard_model2);
[param err] = fitHazardRate(param,cnts,smnsmedian,@bimodal_hazard_model2);

% Plot fit
t = 0.1:0.001:3;
fitted_rt = bimodal_hazard_model2(param,t);
hold on
plot(t,fitted_rt,'LineWidth',3,'Color',[0.8 0 0])
