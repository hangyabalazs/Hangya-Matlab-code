function y = bimodal_hazard_model2b(param,x)
%BIMODAL_HAZARD_MODEL2B   Bimodal subjective hazard rate model.
%   RT = BIMODAL_HAZARD_MODEL2B(PARAM,ITI) calculates model results (RT,
...
%   variability linearly increasing in time (Phi and Psi, slope and offset
%   parameter of this increase, respectively).
%
%   Reference:
...
y = param.we + param.wr * exp(x-param.tau) +...
    param.wb * subjective_hazard_rate(param.phi,param.psi,x-param.tau);

% -------------------------------------------------------------------------
function v = subjective_hazard_rate(Phi,Psi,iti)
...
for tm = t2
    fg = ft .* exp(-(tau-tm).^2/(2*(Phi*tm+Psi).^2));
    intfg(cntr) = sum(fg*dt);
    cntr = cntr + 1;
end
fwt = 1 ./ (Phi * (t2 + Psi) * sqrt(2*pi)) .* intfg;
fwt = fwt / sum(fwt);
