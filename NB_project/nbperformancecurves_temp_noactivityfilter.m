...
global DATAPATH
fs = filesep;
resdir = [DATAPATH 'NB' fs 'performancecurves_' align '_' group fs];   % results directory
...
for k = 1:NumCell
    ...
    try
        ...
        [psth, spsth, spsth_se, tags] = ultimate_psth(cellid,'trial',hitevent,wn,...
            ...
            'isadaptive',0,'event_filter','custom','filterinput','Hit==1|Miss==1',...
...
%         xlim([-0.2 0.2])
        ...
        [psth, spsth, ~, tags] = ultimate_psth(cellid,'trial',faevent,wn,...
            'dt',dt,'display',true,'sigma',0.02,'parts','#ResponseType',...
            'isadaptive',0,'event_filter','custom','filterinput','FalseAlarm==1|CorrectRejection==1',...
            'maxtrialno',Inf,'baselinewin',[-0.5 0],'testwin',[0 0.5],...
            'relative_threshold',0.1);
        psth = psth(2:3,:);
        spsth = spsth(2:3,:);
