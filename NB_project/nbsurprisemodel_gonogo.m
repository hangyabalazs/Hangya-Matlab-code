...
[tone_onset difficulty trialtype eta ...
    RT isdetected G_strength NG_strength] = deal(nan(1,NumTrials));
...
for iT = 1:NumTrials
    ...
    while isequal(O{iT},'restart')
        [O{iT} RT(iT) isdetected(iT) G_strength(iT) NG_strength(iT)] = ...
            runtrial(T,ITIDistribution,eta(iT),tone_onset(iT),trialtype(iT),meta,display);
    end
    disp(iT)
end
...
% -------------------------------------------------------------------------
function [O RT isdetected G_strength NG_strength] = runtrial(T,ITIDistribution,eta,tone_onset,trialtype,meta,display)
...
P_lick = 0*base_lick_rate + 0.05 * Pg - 0.03 * Png;   % lick probability (1; 0.035; 0.032)
% P_lick = 1.5*base_lick_rate + 0.02 * Pg - 0.01 * Png;
P_lick(P_lick<0) = 0;
r = rand(size(T));
R = r < P_lick;   % lick
alllickinx = find(R);
