...
Baseline = getvalue('Baseline',allChAT);   % baseline FR
hitlat = getvalue('Hit_Latency',allChAT);   % Hit resp. latency
falat = getvalue('FA_Latency',allChAT);   % FA resp. latency
hitjit = getvalue('Hit_Jitter',allChAT);   % Hit resp. jitter
fajit = getvalue('FA_Jitter',allChAT);   % FA resp. jitter
hitspno = getvalue('Hit_SpikeNumberDistribution',allChAT);   % Number of evoked spikes (Hit)
faspno = getvalue('FA_SpikeNumberDistribution',allChAT);   % Number of evoked spikes (FA)
burstindex = getvalue('ThetaIndex',allChAT);   % baseline FR
% y = cellfun(@mean,faspno)';
y = burstindex';
...
%     pause(2)
    ...
    plot(x(k),y(k),cmrk,'MarkerSize',12,'MarkerFaceColor',cclr,'MarkerEdgeColor','k')  % scatter plot
end
