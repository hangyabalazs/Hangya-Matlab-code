function nbiticurves(I,issave)
%NBITICURVES   PSTHs for different reaction times.
%   NBITICURVES(I,ISSAVE) calculates non-adaptive PSTHs and raster plots
%   for a set of cells aligned to 'Go' and 'No-go' tone/response onset. The
%   trials are partitioned based on ITI (median split). The PSTHs, rasters
%   and population average PSTHs are plotted and saved. Input parameters:
...
%
%   See also NBRTCURVES, NBTUNINGCURVES, ULTIMATE_PSTH and
%   NBRESPONSESORTER.
...
global DATAPATH
fs = filesep;
resdir = [DATAPATH 'NB' fs 'iticurves_new_' align '_' group fs];   % results directory
...
for k = 1:NumCell
    ...
    hitfilter1 = 'Hit==1&ITIDistribution<nanmedian(TE.ITIDistribution)'; %#ok<NASGU>
    hitfilter2 = 'Hit==1&ITIDistribution>nanmedian(TE.ITIDistribution)'; %#ok<NASGU>
    fafilter1 = 'FalseAlarm==1&ITIDistribution<nanmedian(TE.ITIDistribution)'; %#ok<NASGU>
    fafilter2 = 'FalseAlarm==1&ITIDistribution>nanmedian(TE.ITIDistribution)'; %#ok<NASGU>
    cfilter1 = eval([trialtype 'filter1']);   % hitfilter or fafilter
    cfilter2 = eval([trialtype 'filter2']);   % hitfilter or fafilter
    ...
    try
        ...
        [psth1, spsth1, spsth_se1, ~, spt1] = ultimate_psth(cellid,'trial',cevent,wn,...
            ...
            'isadaptive',2,...
            'event_filter','custom','filterinput',cfilter1,...
            'maxtrialno',Inf,'baselinewin',[-0.5 0],'testwin',[0 0.5],...
            'relative_threshold',0.1);
        [psth2, spsth2, spsth_se2, ~, spt2] = ultimate_psth(cellid,'trial',cevent,wn,...
            'dt',dt,'display',true,'sigma',0.02,'parts','all',...
            'isadaptive',2,...
            'event_filter','custom','filterinput',cfilter2,...
            'maxtrialno',Inf,'baselinewin',[-0.5 0],'testwin',[0 0.5],...
            'relative_threshold',0.1);
        psth = cat(1,psth1,psth2);
        spsth = cat(1,spsth1,spsth2);
        spsth_se = cat(1,spsth_se1,spsth_se2);
        tags = {'short ITI','long ITI'};
        ...
        disp(size([spt1; spt2],1))  % display number of trials 
