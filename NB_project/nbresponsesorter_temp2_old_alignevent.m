...
global DATAPATH
fs = filesep;
resdir = [DATAPATH 'NB' fs 'responsesorter_new3_' fs];   % results directory
...
for k = 1:NumCells
    ...
    try
        ...
        alignevent = findAlignEvent(cellid,alignfilter);
...
% -------------------------------------------------------------------------
function alignevent = findAlignEvent(cellid,alignfilter)

% Checking whether 'DeliverFeedback' event is available
if isequal(alignfilter,'FalseAlarm==1')   % 'LeftPortIn' is not the right alternative in case of Hits (use 'LeftWaterValveOn')
    ES = loadcb(cellid,'EVENTSPIKES');   % load events
    evs = ES.events(:,1);
    if ismember('DeliverFeedback',evs)   % in 'feedbackdelay' version, feedback delivery was delayed after 'LeftPortIn'
        event_pos = findcellstr(ES.events(:,1),'DeliverFeedback');
        event_pos2 = findcellstr(ES.events(:,1),'LeftPortIn');
        if ~isempty(ES.event_stimes{event_pos})   % for a few re-clustered cells, an empty 'DeliverFeedback' event was automatically generated
            numev = sum(cellfun(@(s)~isempty(s),(ES.event_stimes{event_pos})));
            numev2 = sum(cellfun(@(s)~isempty(s),(ES.event_stimes{event_pos2})));
            if isequal(numev,numev2)   % in new sessions with no feedback-delay, 'DeliverFeedback' is for Hits only
        ...
        alignevent = 'LeftPortIn';  % use 'LeftPortIn' when ther is no 'DeliverFeedback'
    end
else
