...
selstr = ['"pChAT+"==1&"ID_PC">20&"Lr_PC"<0.15&"validity"==1&' ...
    'ismember("session_type",{''gonogo'',''feedbackdelay''})&' ...
    'ismember("Area1",{''GP'',''GP/SI'',''SI'',''IC'',''RT/IC'',''EP'',''EA'',''EAC''})'];
pChAT = selectcell(selstr);  % putative chloinergic
allChAT = [ChAT pChAT];
...
SpikeWidth_NT = [SpikeShape_NT.SpikeWidth];
...
SpikeWidth_allChAT = [SpikeShape_allChAT.SpikeWidth];
...
SpikeWidth_ChAT = [SpikeShape_ChAT.SpikeWidth];
...
SpikeWidth_pChAT = [SpikeShape_pChAT.SpikeWidth];
...
SpikeWidth_PV = [SpikeShape_PV.SpikeWidth];
...
errorshade(1:32,mean(Spike_ChAT),nanse(Spike_ChAT),'LineColor',green,...
    'ShadeColor',green)
errorshade(1:32,mean(Spike_pChAT),nanse(Spike_pChAT),'LineColor',blue,...
    'ShadeColor',blue)
