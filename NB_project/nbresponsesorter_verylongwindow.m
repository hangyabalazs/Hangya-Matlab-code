function error_list = nbresponsesorter_verylongwindow(I,issave)
...
global DATAPATH
fs = filesep;
resdir = [DATAPATH 'NB' fs 'responsesorter_verylongwindow_newdata' fs];   % results directory
...
fnm = fullfile(resdir,['temp_' dsr '.mat']);   % file name
save(fnm,'allpsth_orig','allpsth','allspsth_orig','allspsth','allstats','time',...
        'tags','error_list')
...
for k = 1:NumCells
    ...
    try
        ...
        [psth, spsth, ~, ~, spt, stats] = ultimate_psth(cellid,'trial',...
            ...
            'baselinewin',[-3 0],'testwin',[0 3],'relative_threshold',0.1);
