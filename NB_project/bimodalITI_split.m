...
SESSIONS.n040.bimodal0 = {'121010a' '121011a' '121012a' '121015a' ...
    '121016a' '121019a' '121021a'};  % bimodal with narrow peaks
SESSIONS.n040.bimodal = {};     % bimodal
...
% Binning
edges = 0.1:0.25:3;  % for bimodal (more sessions) (increment=0.25: stronger smoothing)
% edges = 0.1:0.2:5;  % for exponential (quasi-matched min and max number of samples per bin)
% edges = 0.1:0.3:7;  % for human bimodal, longer times
...
%
limL1 = icdf('norm',0.1,0.3,0.15);
limU1 = icdf('norm',0.9,0.3,0.15);
limL2 = icdf('norm',0.1,2,0.15);
limU2 = icdf('norm',0.9,2,0.15);
expectedinx = (alliti2 > limL1 & alliti2 < limU1) | (alliti2 > limL2 & alliti2 < limU2) & ~isnan(allrt2);
unexpectedinx = ~expectedinx & ~isnan(allrt2);
boxstat(allrt2(expectedinx),allrt2(unexpectedinx),'expected','unexpected')
...
errorshade(cnts,smnsmedian,smnsmedian_se,...
    'LineColor',clr(I,:),'ShadeColor',clr(I,:))
[nm xout] = hist(TE.ITIDistribution,50);
stairs(xout,nm/700+0.15,'Color','r')
