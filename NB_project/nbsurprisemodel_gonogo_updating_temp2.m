...
% Parameters
detection_probability = [0.05 0.33 0.66 0.99];   % difficult to easy
...
lgdefault = 0.25;   % stady-state go lick coefficient 0.5
lngdefault = 0.1;   % stady-state no-go lick coefficient 0.3
    ...
    if iT == 1   % initial condition
        ...
        switch O{iT-1}
            case 'hit'   % increase positive corr. between go prob. and lick
                lickgocoeff(iT) = lickgocoeff(iT-1) * 1.4;
                lickgocoeff(iT) = min(lickgocoeff(iT),1);
                licknogocoeff(iT) = licknogocoeff(iT-1) * 0.7;
                basecoeff(iT) = basecoeff(iT-1) * 1.2;
...
%                 licknogocoeff(iT) = licknogocoeff(iT-1);
    ...
    while isequal(O{iT},'restart') && cntr < 5000
        ...
        [O{iT} RT(iT) isdetected(iT) G_strength(iT) NG_strength(iT)] = ...
            ...
            basecoeff(iT),lickgocoeff(iT),licknogocoeff(iT),display);
    end
    restarts(iT) = cntr - 1;
    disp(iT)
end
...
% Updating
next_hit_inx = strcmp([O(4:end) NaN NaN NaN],'hit');
prev_hit_inx = strcmp([{NaN} O(1:end-1)],'hit');
next_fa_inx = strcmp([O(4:end) NaN NaN NaN],'fa');
...
function [O RT isdetected G_strength NG_strength] = runtrial(T,ITIDistribution,...
    eta,tone_onset,trialtype,meta,basecoeff,lickgocoeff,licknogocoeff,display)
