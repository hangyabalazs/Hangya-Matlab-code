...
% Directories
global DATAPATH
resdir = ([DATAPATH 'NB\clustering_newdata2\']);   % result directory
...
% Wpar = round(Wpa.*1000) / 1000;   % round p value to 1/1000
% Wpir = round(Wpi.*1000) / 1000;   % round p value to 1/1000
% inx_act = Wpar <= 0.025;  % significant activation
% inx_inh = Wpir <= 0.025;   % significant inhibition
...
v06 = NTcorr;   % correlation with mean NT PSTH
v07 = ACTcorr;
v08 = INHcorr;
...
C1vinx = sort(find(c==1));
C2vinx = sort(find(c==2));
C3vinx = sort(find(c==3));
C4vinx = sort(find(c==4));
...
% Colors
grey = [0.7 0.7 0.7];
green = [205 85 160]/255;
blue = [95 215 255]/255;
clusclr1 = [0 0.498 0];
clusclr2 = [0.9294 0.6941 0.1255];

% 2D scatter plot of features
xa = 7;  % select feature for x axis
...
figure
plot(va(NTactinx),vb(NTactinx),'o','MarkerSize',6,'MarkerFaceColor',...
    grey,'MarkerEdgeColor',grey)   % NT cells
hold on
plot(va(C3inx),vb(C3inx),'o','MarkerSize',6,'MarkerFaceColor',...
    clusclr2,'MarkerEdgeColor',clusclr2);
plot(va(C4inx),vb(C4inx),'o','MarkerSize',6,'MarkerFaceColor',...
    clusclr1,'MarkerEdgeColor',clusclr1);
...
xlabel(la)
ylabel(lb)
saveas(gcf,fullfile(resdir,[la ' VS ' lb '.fig']))

% 3D scatter plots
xa = 7;  % select feature for x axis
xb = 8;  % select feature for y axis
xc = 5;  % select feature for z axis
va = eval(['v0' num2str(xa)]);   % define x axis feature
vb = eval(['v0' num2str(xb)]);   % define y axis feature
vc = eval(['v0' num2str(xc)]);   % define z axis feature
la = eval(['l' num2str(xa)]);   % x axis label
lb = eval(['l' num2str(xb)]);   % y axis label
lc = eval(['l' num2str(xc)]);   % z axis label
figure
plot3(va(C2inx),vb(C2inx),vc(C2inx),'o','MarkerSize',5,...
    'MarkerFaceColor', grey,'MarkerEdgeColor',grey);   % NT cells
hold on
plot3(va(pChATactinx),vb(pChATactinx),vc(pChATactinx),'o','MarkerSize',12,...
    'MarkerFaceColor',blue,'MarkerEdgeColor',blue);   % pChAT cells
plot3(va(ChATactinx),vb(ChATactinx),vc(ChATactinx),'o','MarkerSize',12,...
    'MarkerFaceColor',green,'MarkerEdgeColor',green);   % ChAT cells
plot3(va(C3inx),vb(C3inx),vc(C3inx),'o','MarkerSize',5,...
    'MarkerFaceColor',clusclr2,'MarkerEdgeColor',clusclr2);
plot3(va(C4inx),vb(C4inx),vc(C4inx),'o','MarkerSize',5,...
    'MarkerFaceColor',clusclr1,'MarkerEdgeColor',clusclr1);
grid on
xlabel(la)
ylabel(lb)
zlabel(lc)
saveas(gcf,fullfile(resdir,[la ' VS ' lb ' VS ' lc '.fig']))

% PSTH
baseline_C4 = mean(allpsth(C4inx,1:200));   % baseline-subtracted average z-scores
baseline_C3 = mean(allpsth(C3inx,1:200));
baseline_pChAT = mean(allpsth(pChATactinx,1:200));
baseline_ChAT = nanmean(allpsth(ChATactinx,1:200));
mn_C4 = mean(baseline_C4);
mn_C3 = mean(baseline_C3);
mn_pChAT = mean(baseline_pChAT);   % baseline for activated pChAT cells
mn_ChAT = mean(baseline_ChAT);   % baseline for activated cholinergic cells
figure
errorshade(time,mean(allpsth(C4inx,:))-mn_C4,nanse(allpsth(C4inx,:)),'LineColor',...
    clusclr1,'ShadeColor',clusclr1)
hold on
errorshade(time,mean(allpsth(C3inx,:))-mn_C3,nanse(allpsth(C3inx,:)),'LineColor',...
    clusclr2,'ShadeColor',clusclr2)
errorshade(time,mean(allpsth(pChATactinx,:))-mn_pChAT,nanse(allpsth(pChATactinx,:)),'LineColor',...
    blue,'ShadeColor',blue)
errorshade(time,mean(allpsth(ChATactinx,:))-mn_ChAT,nanse(allpsth(ChATactinx,:)),'LineColor',...
    green,'ShadeColor',green)
