...
% Call the model
y = param.we + param.wb * subjective_hazard_rate(param.phi,x-param.tau);
...
tau = t;
cntr = 1;   % counter
t2 = [fliplr(t(1)-dt:-dt:dt) t t(end)+dt:dt:10*t(end)];   % we need a longer time range, to get a better estimate of Fwt
for tm = t2
