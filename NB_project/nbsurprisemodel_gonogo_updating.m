function [GoHitUpdate NoGoHitUpdate GoFAUpdate NoGoFAUpdate] = nbsurprisemodel_gonogo_updating
%NBSURPRISEMODEL_GONOGO_UPDATING   HMM model go/no-go task.
%   NBSURPRISEMODEL_GONOGO_UPDATING implements a hidden Markov model for
...
%   generated every time step (0.1s). The animal has access only to the
...
%
%   This version implements updating of lick probability after hit trials.
%
%   Reference:
...
% Parameters
salience_coeff = 0.4;  % asymmetric detection probabilities for the go and no-go tones
detection_probability_go = [0.05 0.33 0.66 0.99];   % difficult to easy
detection_probability_nogo = detection_probability_go * salience_coeff;   % difficult to easy
...
[tone_onset difficulty trialtype eta_go eta_nogo ...
    RT isdetected G_strength NG_strength ...
    lickgocoeff licknogocoeff basecoeff restarts] = deal(nan(1,NumTrials));
meta_go = mean(1-(1-detection_probability_go(1:4)).^(dt/0.5));   % mean go detection prob. in time bin
meta_nogo = mean(1-(1-detection_probability_nogo(1:4)).^(dt/0.5));   % mean no-go detection prob. in time bin
lgdefault = 0.7;   % stady-state go lick coefficient 0.5
lngdefault = 0;   % stady-state no-go lick coefficient 0.3
basedefault = 0;   % stady-state baseline lick coefficient
for iT = 1:NumTrials
    ...
    eta_go(iT) = 1 - (1 - detection_probability_go(difficulty(iT))) ^ (dt / 0.5);   % go detection probability in time bin
    eta_nogo(iT) = 1 - (1 - detection_probability_nogo(difficulty(iT))) ^ (dt / 0.5);   % no-go detection probability in time bin
    
    % Update lick probability (eagerness to lick)
    if iT == 1   % initial condition
        lickgocoeff(iT) = lgdefault;
        licknogocoeff(iT) = lngdefault;
        basecoeff(iT) = basedefault;
    else
        switch O{iT-1}
            case 'hit'   % increase positive corr. between go prob. and lick
                lickgocoeff(iT) = lickgocoeff(iT-1) * 1.25;
                lickgocoeff(iT) = min(lickgocoeff(iT),1.5);
                licknogocoeff(iT) = licknogocoeff(iT-1) * 1;
                basecoeff(iT) = basecoeff(iT-1) * 1;
                basecoeff(iT) = min(basecoeff(iT),1);
            case 'fa'    % increase negative corr. between go prob. and lick
                lickgocoeff(iT) = lickgocoeff(iT-1) * 1;
                licknogocoeff(iT) = licknogocoeff(iT-1) * 1;
%                 lickgocoeff(iT) = lgdefault;
%                 licknogocoeff(iT) = lngdefault;
                basecoeff(iT) = basecoeff(iT-1) * 1;
            case 'cr'    % no updating
%                 lickgocoeff(iT) = lickgocoeff(iT-1);
%                 licknogocoeff(iT) = licknogocoeff(iT-1);
%                 lickgocoeff(iT) = mean([lickgocoeff(iT-1) lgdefault]);
%                 licknogocoeff(iT) = mean([licknogocoeff(iT-1) lngdefault]);
                lickgocoeff(iT) = lgdefault;
                licknogocoeff(iT) = lngdefault;
                basecoeff(iT) = mean([basecoeff(iT-1) basedefault]);
...
%                 licknogocoeff(iT) = mean([licknogocoeff(iT-1) lngdefault]);
    ...
    O{iT} = 'restart';
    cntr = 0;
    while isequal(O{iT},'restart') && cntr < 5000
        cntr = cntr + 1;
        [O{iT} RT(iT) isdetected(iT) G_strength(iT) NG_strength(iT)] = ...
            runtrial(T,ITIDistribution,eta_go(iT),eta_nogo(iT),tone_onset(iT),trialtype(iT),...
            meta_go,meta_nogo,basecoeff(iT),lickgocoeff(iT),licknogocoeff(iT),display);
    end
    restarts(iT) = cntr - 1;
    disp(iT)
end
...
% Updating
next_hit_inx3 = strcmp([O(4:end) NaN NaN NaN],'hit');
next_hit_inx4 = strcmp([O(5:end) NaN NaN NaN NaN],'hit');
next_hit_inx5 = strcmp([O(6:end) NaN NaN NaN NaN NaN],'hit');
next_hit_inx = next_hit_inx3 | next_hit_inx4 | next_hit_inx5;
prev_hit_inx = strcmp([{NaN} O(1:end-1)],'hit');
next_fa_inx3 = strcmp([O(4:end) NaN NaN NaN],'fa');
next_fa_inx4 = strcmp([O(5:end) NaN NaN NaN NaN],'fa');
next_fa_inx5 = strcmp([O(6:end) NaN NaN NaN NaN NaN],'fa');
next_fa_inx = next_fa_inx3 | next_fa_inx4 | next_fa_inx5;
...
% -------------------------------------------------------------------------
function [O RT isdetected G_strength NG_strength] = runtrial(T,ITIDistribution,...
    eta_go,eta_nogo,tone_onset,trialtype,meta_go,meta_nogo,...
    basecoeff,lickgocoeff,licknogocoeff,display)
...
p_Xt_St = [1 0 0; 1-meta_go meta_go 0; 1-meta_nogo 0 meta_nogo];   % p(x_t|s_t) output probability matrix
% row 1: s_t = 0
% row 2: s_t = 1
% row 3: s_t = 2
% column 1: x_t = 0
...
P_lick = basecoeff * base_lick_rate + lickgocoeff * Pg - licknogocoeff * Png;   % lick probability (1; 0.035; 0.032)
