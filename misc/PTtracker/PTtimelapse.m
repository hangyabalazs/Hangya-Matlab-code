...
% Make frames
next = 0;
NumFrames = length(dr) / 2;
[yrange xrange] = deal(nan(NumFrames,2,2));
...
for k = 1:2:length(dr)
    ...
    cdt = get(I,'CData');
    yrange(next,1,:) = [-fixpoint(1) size(cdt,1)-fixpoint(1)];
    xrange(next,1,:) = [-fixpoint(2) size(cdt,2)-fixpoint(2)];
...
%     postxt = get(T,'Position');
    ...
    cdt = get(I,'CData');
    yrange(next,2,:) = [-fixpoint(1) size(cdt,1)-fixpoint(1)];
    xrange(next,2,:) = [-fixpoint(2) size(cdt,2)-fixpoint(2)];
    cdtt = graypad(cdt,300);
    cdtn = cdtt(fixpoint(1):fixpoint(1)+600,fixpoint(2):fixpoint(2)+600,:);
...
%     set(A4n,'XLimMode','manual','YLimMode','manual')
    ...
    M(next) = getframe(H);   % make frame
    close all
end
M(next+1:next+10) = M(end);   % freeze last frame

% Make movie
movie2avi(M,'timelapse13.avi','Compression','none','fps',4)
