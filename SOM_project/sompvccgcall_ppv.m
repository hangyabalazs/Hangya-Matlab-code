function sompvccgcall_ppv(UniqueCellPairs)
%SOMPVCCGCALL_PPV   Calls SOMCCG_CONF_FILTER.
%   Caller for SOMCCG_CONF_FILTER for putative PV cells. Copyright to Duda.
%
...
for iPair = 1:r
    ...
    fr = length(ncc1) / sum(tseg(2,:)-tseg(1,:))
    [ncc2,seltsind,selisi] = extractSegSpikes(Cell2,tseg);
    fr = length(ncc2) / sum(tseg(2,:)-tseg(1,:))
...
% save ppv_ppv2 CCR UCCR LCCR PairOfCells UniqueCellPairs MeanH0 SDH0