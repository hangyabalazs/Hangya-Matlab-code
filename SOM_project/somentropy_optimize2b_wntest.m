...
pv0_MatrixPsth1 = pv;
som0_MatrixPsth1 = som;
non_tagged0_MatrixPsth1 = non_tagged_all;

load([DATADIR 'SOM_Sachin\PSTH\HomeZin.mat'])
pv0_MatrixPsth2 = pv;
som0_MatrixPsth2 = som;
non_tagged0_MatrixPsth2 = non_tagged_all;
...
% Remove NaNs
[x1 y1] = find(isnan(non_tagged0_MatrixPsth1));
[x2 y2] = find(isnan(non_tagged0_MatrixPsth2));
x = union(unique(x1),unique(x2));
non_tagged0_MatrixPsth1(x,:) = [];
non_tagged0_MatrixPsth2(x,:) = [];
...
SomSpikeWidth0 = SomSpikeWidth;
for wnw = 30:10:100
    wn1 = -wnw;
    wn2 = wnw;
    pv_MatrixPsth1 = pv0_MatrixPsth1(:,mdl+wn1:mdl+wn2-1);
    som_MatrixPsth1 = som0_MatrixPsth1(:,mdl+wn1:mdl+wn2-1);
    non_tagged_MatrixPsth1 = non_tagged0_MatrixPsth1(:,mdl+wn1:mdl+wn2-1);
    pv_MatrixPsth2 = pv0_MatrixPsth2(:,mdl+wn1:mdl+wn2-1);
    som_MatrixPsth2 = som0_MatrixPsth2(:,mdl+wn1:mdl+wn2-1);
    non_tagged_MatrixPsth2 = non_tagged0_MatrixPsth2(:,mdl+wn1:mdl+wn2-1);
    ...
    % Focus on NS SOM
    sinx = (SomSpikeWidth0>270);
    ssom1 = ssom1(sinx,:);
    ssom2 = ssom2(sinx,:);
    SomSpikeWidth = SomSpikeWidth0(sinx);
    sizesom = size(ssom1);
    ...
    kl_pv(wnw/10) = KLdist_discrete(allpsth5_pv(tags_pv),allpsth5_pv);  % PV, HomeZoneOut
    kl_som(wnw/10) = KLdist_discrete(allpsth5_som(tags_som),allpsth5_som);  % SOM, HomeZoneOut
    kl_pv2(wnw/10) = KLdist_discrete(allpsth5_pv2(tags_pv),allpsth5_pv2);  % PV, HomeZoneIn
    kl_som2(wnw/10) = KLdist_discrete(allpsth5_som2(tags_som),allpsth5_som2);  % SOM, HomeZoneIn
    ...
    % Kullback-Leibler divergence
    kl_ns(wnw/10) = KLdist_discrete(allpsth5_ns(tags_ns),allpsth5_ns);
    kl_ns2(wnw/10) = KLdist_discrete(allpsth5_ns2(tags_ns),allpsth5_ns2);
    ...
    % Kullback-Leibler divergence
    kl_ws(wnw/10) = KLdist_discrete(allpsth5_ws(tags_ws),allpsth5_ws);    % HomeZoneOut
    kl_ws2(wnw/10) = KLdist_discrete(allpsth5_ws2(tags_ws),allpsth5_ws2);   % HomeZoneIn

end

figure;plot(kl_pv,'r')
hold on;plot(kl_som)
hold on;plot(kl_ns,'k')
hold on;plot(kl_ws,'Color',[0.7 0.7 0.7])
figure;plot(kl_pv2,'r')
hold on;plot(kl_som2)
hold on;plot(kl_ns2,'k')
hold on;plot(kl_ws2,'Color',[0.7 0.7 0.7])
