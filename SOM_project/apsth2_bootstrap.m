function psth = apsth2_bootstrap(cellid)
%APSTH2_BOOTSTRAP   Spike dendity function with adaptive Gaussian kernel.
%	PSTH_ACONV = APSTH2_BOOTSTRAP(CELLID) computes the spike density
...
%	linearly mapped to probabilities. Bootstrap resampling is used to
%	estimate stadard error of the PSTH.
%
...
[agvd psth A] = convvarkernel(binraster,dt);

% Calculate bootstrap confidance interval
B = 200;
psth_bst = nan(B,size(psth,2));
for k = 1:B
    disp(k)
    rr = round(rand(1,tno)*(tno-1)+0.5);
    [agvd_bst psth_bst(k,:)] = convvarkernel(binraster(rr,:),dt);
    close(gcf)
end
psth_err = std(psth_bst);
...
% -------------------------------------------------------------------------
function [agvd psth A] = convvarkernel(binraster,dt)
