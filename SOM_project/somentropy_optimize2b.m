function somentropy_optimize2b
%SOMENTROPY_OPTIMIZE2B   Kullbach-Leibler divergence for PSTH in different groups.
%   SOMENTROPY_OPTIMIZE2B linearizes PSTH distributions by taking the
...
%   are calculated. Spike width threshold is 210 us for NS and 360 us for 
%   WS (optimized by SOMENTROPY_OPTIMIZE_WN to get maximal KL value).
%
...
%
%   SOMENTROPY_OPTIMIZE2B includes SOM as a marker and calculates
...
% Load & define PSTHs
global DATADIR
load([DATADIR 'SOM_Sachin\PSTH_old5_1stsubmission2Nature\HomeZout.mat'])
pv_MatrixPsth1 = pv;
som_MatrixPsth1 = som;
non_tagged_MatrixPsth1 = non_tagged_all;

load([DATADIR 'SOM_Sachin\PSTH_old5_1stsubmission2Nature\HomeZin.mat'])
pv_MatrixPsth2 = pv;
som_MatrixPsth2 = som;
non_tagged_MatrixPsth2 = non_tagged_all;

% Load spike width data
load([DATADIR 'SOM_Sachin\PSTH_old5_1stsubmission2Nature\SpikeWidth.mat'])
...
% Limits for spike witdh (see SOMENTROPY_OPTIMIZE_WN)
lmt_ns = 210;
lmt_ns2 = 210;
lmt_ws = 360;
lmt_ws2 = 360;
...
% Kullback-Leibler divergence
kl_nso = KLdist_discrete(allpsth5_ns(tags_ns),allpsth5_ns);
kl_nso2 = KLdist_discrete(allpsth5_ns2(tags_ns),allpsth5_ns2);
...
% Kullback-Leibler divergence
kl_wso = KLdist_discrete(allpsth5_ws(tags_ws),allpsth5_ws);    % HomeZoneOut
kl_wso2 = KLdist_discrete(allpsth5_ws2(tags_ws),allpsth5_ws2);   % HomeZoneIn
...
for k = 1:sss2
    ...
    % Kullback-Leibler divergence for SOM
    srg_som(k) = KLdist_discrete(allpsth5_r(tt),allpsth5_r);
...
for k = 1:sss2
    ...
    % Kullback-Leibler divergence for PV
    srg_pv2(k) = KLdist_discrete(allpsth5_r2(tt),allpsth5_r2);
    ...
    % Kullback-Leibler divergence for SOM
    srg_som2(k) = KLdist_discrete(allpsth5_r2(tt),allpsth5_r2);
    ...
    % Kullback-Leibler divergence for NS
    srg_ns2(k) = KLdist_discrete(allpsth5_r2(tt),allpsth5_r2);
    ...
    % Kullback-Leibler divergence for NS
    srg_ws2(k) = KLdist_discrete(allpsth5_r2(tt),allpsth5_r2);
...
for k = 1:sss2
    ...
    % Kullback-Leibler divergence for NS SOM
    srg_nssom2(k) = KLdist_discrete(allpsth5_r2(tt),allpsth5_r2);
    ...
    % Kullback-Leibler divergence for WS SOM
    srg_wssom(k) = KLdist_discrete(allpsth5_r(tt),allpsth5_r);
