...
config_results(['wpt_' sct '.txt']); % configure result file
...
% cgmakesprite(2,scrn_width,scrn_hight, 0.87, 0.92, 0.98);    % for empty background
...
while hitno < 2
    ...
    for k = 1:4
        ...
        cgsetsprite(2);
        cgtext(wp1{rp(k)+4},-100,150);
        %     cgtext(wp2{rp(k)+4},100,150);
        cgsetsprite(0);
        cgdrawsprite(2,0,0);
        t = cgflip(0,0,0);
        recalled_word = [];
