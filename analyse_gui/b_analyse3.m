function varargout = b_analyse3(varargin)
% B_ANALYSE3 Application M-file for b_analyse3.fig
%    FIG = B_ANALYSE3 launch b_analyse3 GUI.
%    B_ANALYSE3('callback_name', ...) invoke the named callback.

% Last Modified by GUIDE v2.5 24-Sep-2007 11:25:55
...
if nargin == 0  % LAUNCH GUI
    ...
    handles.burstnum = -1;
    global DATAPATH
    burstdir = [DATAPATH 'Andi\Hajni_intra\Cluster\mat2\'];
    burstdir0 = [DATAPATH 'Andi\Hajni_intra\Cluster\mat\'];
    ...
    Cstart = imread('e:\matlab6_1\work\balazs\doubleleft_arrow.bmp');
    Cprev = imread('e:\matlab6_1\work\balazs\left_arrow.bmp');
    Cnext = imread('e:\matlab6_1\work\balazs\right_arrow.bmp');
    Clast = imread('e:\matlab6_1\work\balazs\doubleright_arrow.bmp');
...
% Clear message
set(handles.figure1,'Name','Thalamus Data Analysis');
...
isi = diff(vdisc);
artefact = [];
while min(isi./handles.sr) < 0.001     % automatically declare art. if ISI < 1 ms
    ainx = find(isi==min(isi));
    vdisc(ainx) = round((vdisc(ainx)+vdisc(ainx+1))/2);
    artefact = [artefact vdisc(ainx+1)];
    vdisc(ainx+1) = [];
    
    isi = diff(vdisc);
end

short = find(isi./handles.sr<0.0015);   % ask if 1 ms < ISI < 1.5 ms
artinx = [];
if ~isempty(short)
    for k = 1:length(short)
        ...
        artq = questdlg('Declare as an artefact?','Artefacts','Yes');
        if isequal(artq,'Yes')
            artefact = [artefact vdk];
            artinx = [artinx inx];
        elseif isequal(artq,'Cancel')
            delete(L)
            return
        end
        delete(L)
...
artefact = unique(artefact);
artinx = unique(artinx);
vdisc(artinx) = [];
...
mm = pwd;
cd(handles.pathname)
str = ['save ' nm ' vdisc thr artefact'];
eval(str)
cd(mm)
...
set(gcf,'KeypressFcn','b_analyse3(''b_callfftswitch'',''eeg'')');
...
% Set keypress function
set(gcf,'KeypressFcn','b_analyse3(''b_callfftswitch'',''unit'')');
...
% Set buttondown functions
bdf1 = 'b_analyse3(''axes_eeg_ButtonDownFcn'',gcbo,[],guidata(gcbo))';
bdf2 = 'b_analyse3(''axes_unit_ButtonDownFcn'',gcbo,[],guidata(gcbo))';
...
axes(handles.axes_eeg);
axis([max(focus(1)-wn,time(1)) min(focus(2)+wn,time(end)) y_eeg(1) y_eeg(2)]);
b_liner(handles.plot1_handle,handles.eegs,handles.gui_pos,handles.la,handles.len,handles.sr);
