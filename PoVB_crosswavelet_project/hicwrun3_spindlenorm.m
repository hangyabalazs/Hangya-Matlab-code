function hicwrun3_spindlenorm
%HICWRUN3_SPINDLENORM   Runs crosswavelet on a sequence of files.
%   HICWRUN3_SPINDLENORM calculates and saves wavelet and crosswavelet
%   images. It normalizes EEG and unit wavelet with mean maximal spindle
%   power before crosswavelet calculation.
%
...
resdir = [DATAPATH 'Hajni\EEGMPO_deltanorm\crosswavelet_spindle\'];
mm = pwd;
...
% Progress indicator
wb = waitbar(0,'Running HICWRUN3 SPINDLENORM...','Position',[360 250 275 50]);
global WB
WB(end+1) = wb;
...
for o = 1:sf
    ...
    im_eeg = im_eeg ./ mean(MaxSpindlePowerEEG);  % normalize with spindle power
    im_unit = im_unit ./ mean(MaxSpindlePowerUnit);
    wave_cross = sqrt(im_eeg) .* sqrt(im_unit);       % crosswavelet power
    im = wave_cross .^ 2;
    clear wave_cross
    
    MeanDeltaPower = mean(im(pwind1_delta:pwind2_delta,:));  % recalculate power values
