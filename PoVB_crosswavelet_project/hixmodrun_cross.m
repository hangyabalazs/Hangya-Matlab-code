function hixmodrun_cross
%HIXMODRUN_CROSS   Runs cross-modulation on a sequence of files.
%   HIXMODRUN_CROSS calculates and saves crossmodulation plots (see Tort et al.,
%   2008, PNAS).
%
...
resdir = [DATAPATH 'Hajni\EEGMPO_new\crossmod_cross\'];
mm = pwd;
...
% Progress indicator
wb = waitbar(0,'Running HIXMODRUN CROSS...','Position',[360 250 275 50]);
global WB
WB(end+1) = wb;
...
for o = 1:sf
    ...
    [pow_eeg0,phase_eeg,f] = eegwavelet(eeg,dsr);        % MPO wavelet
    [pow_eeg,phase_eeg0,f] = eegwavelet(unit,dsr);        % unit wavelet
    ...
    tt = fname(1:end-4);
    tt(tt=='_') = ' ';
    tt = [tt ' EEG-Unit crossmod'];
    title(tt)
    fn = [resdir tt '_EUXMOD.fig'];       % save EEG-unit cross-modulation
    saveas(H,fn)
    fn = [resdir tt '_EUXMOD.tiff'];
    saveas(H,fn)
        
    fn = [resdir fname(1:end-4) '_EUXMOD.mat'];
    save(fn,'MI')
