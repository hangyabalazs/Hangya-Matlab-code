function hicwrun3_deltanorm
%HICWRUN3_DELTANORM   Runs crosswavelet on a sequence of files.
%   HICWRUN3_DELTANORM calculates and saves wavelet and crosswavelet images.
%   It normalizes EEG and unit wavelet with mean maximal delta power before
%   crosswavelet calculation.
%
...
resdir = [DATAPATH 'Hajni\EEGMPO_deltanorm\crosswavelet_delta_newcells\'];
mm = pwd;
...
% Progress indicator
wb = waitbar(0,'Running HICWRUN3 DELTANORM...','Position',[360 250 275 50]);
global WB
WB(end+1) = wb;
...
for o = 1:sf
    ...
    im_eeg = im_eeg ./ mean(MaxDeltaPowerEEG);  % normalize with delta power
    im_unit = im_unit ./ mean(MaxDeltaPowerUnit);
    wave_cross = sqrt(im_eeg) .* sqrt(im_unit);       % crosswavelet
    im = wave_cross .^ 2;
