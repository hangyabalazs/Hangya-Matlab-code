function hicwrun2
%HICWRUN2   Runs crosswavelet on a sequence of files.
%   HICWRUN2 calculates and saves wavelet and crosswavelet images.
%
...
% Progress indicator
wb = waitbar(0,'Running HICWRUN2...','Position',[360 250 275 50]);
global WB
WB(end+1) = wb;
...
for o = 1:sf
    ...
    mx = max(im_eeg(1:pwind2_spindle,:));
    x_lim = get(gca,'XLim');
    y_lim = get(gca,'YLim');
    xcoord = 3 * ((x_lim(2)-x_lim(1)) + x_lim(1)) / 4;
    ycoord = 1 * ((y_lim(2)-y_lim(1)) + y_lim(1)) / 4;
    ...
    mx = max(im_unit(1:pwind2_spindle,:));
    x_lim = get(gca,'XLim');
    y_lim = get(gca,'YLim');
    xcoord = 3 * ((x_lim(2)-x_lim(1)) + x_lim(1)) / 4;
    ycoord = 1 * ((y_lim(2)-y_lim(1)) + y_lim(1)) / 4;
