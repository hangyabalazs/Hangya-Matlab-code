function cz2meanphase_deep
%CZ2MEANPHASE_DEEP   Mean phase.
%   CZ2MEANPHASE_DEEP calculates mean wavelet phase and mean phase of the
...
%
%   CZ2PHASE_DEEP uses pyramidal layer theta as reference for deep
%   interneurons if available and omits them otherwise.
%
%   See also CZ2PHASE.

% Directories
global DATAPATH
inpdir = [DATAPATH 'Czurko2\Phase1\wavelet2\angs_deep\'];
table = [DATAPATH 'Czurko2\orig_phases_deep2.xls'];
table2 = [DATAPATH 'Czurko2\Phase1\mean_phases2_deep2.xls'];
resdir = [DATAPATH 'Czurko2\Phase1\'];
...
% Progress indicator
wb = waitbar(0,'Running CZ2MEANPHASE_DEEP...','Position',[360 250 315 50]);    %Progress indicator
global WB
WB(end+1) = wb;
...
xlswrite('mean_phases2_deep2b',xlsout)   % write Excel output
close(wb)
cd(mm)
