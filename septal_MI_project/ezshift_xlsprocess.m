function ezshift_xlsprocess
%EZSHIFT_XLSPROCESS    Generates plots from ZSHIFT output excel file.
%   EZSHIFT_XLSPROCESS works on the output excel file of
%   EZSHIFT_XLSCONVERT. It generates various plots.
%
...
for i = 1:size(atx,1)
    if isequal(atx{i,8},'PVp') & ~isnan(atx{i,4})
        ...
        PVp(end+1) = atx{i,4};
        if isequal(atx{i,1},'583_n3')
            h1 = plot(atx{i,4},atx{i,5},'y.','MarkerSize',50);
        end
    elseif isequal(atx{i,8},'PVn') & ~isnan(atx{i,4})
        h2 = plot(atx{i,4},atx{i,5},'.','MarkerSize',50,'MarkerFaceColor',...
    ...
    if atx{i,4} <= 0 & ~isnan(atx{i,4})
        Z0 = [Z0 atx{i,5}];
    elseif atx{i,4} > 0 & ~isnan(atx{i,4})
        Z1 = [Z1 atx{i,5}];
    end
end
% saveas(H1,'zshift_maxlenPV.fig')
% 
% H2 = mibox(PVp,PVn,'PV+','PV-');
% saveas(H2,'zshift_maxlenPVbox.fig')
...
for i = 1:size(atx,1)
    if isequal(atx{i,9},'HCNp') & ~isnan(atx{i,4})
        ...
        HCNp(end+1) = atx{i,4};
        if isequal(atx{i,1},'624_n1')
            h1 = plot(atx{i,4},atx{i,5},'m.','MarkerSize',50);
        end
...
% saveas(H3,'zshift_maxlenHCN.fig')
% 
% H4 = mibox(HCNp,HCNn,'HCN+','HCN-');
% saveas(H4,'zshift_maxlenHCNbox.fig')
cd(mm)
