function b_rentropy
%RENTROPY   Entropy calculation.
%   RENTROPY calculates entropies for 5 sec. data segments (80% overlaping windows). The concidered
%   distributions are interspike interval distribution and eeg amplitude distribution. Both signals
%   are downsampled on 1000 Hz. It calls ENTROPY for the raw entropy calculation. Outputs:
...
wh_to = [DATAPATH 'Entropy3\raw2\real\'];
files = dir(wh_from);
files = files(3:end);
...
% Progress indicator
wb = waitbar(0,'Running RENTROPY...','Position',[360 250 275 50]);    %Progress indicator
global WB
WB(end+1) = wb;
