function b_entryrun3c_cont2
%ENTRYRUN3C_CONT2   Runs entropy and PDC on a sequence of recordings.
%   ENTRYRUN3C_CONT2 calculates wavelet power and wavelet phase entropy for 
...
%
%   ENTRYRUN3C_CONT2 attaches theta and non-theta segment together and 
...
resdir = [DATAPATH 'Entry3c_cont2\Theta_hc\'];
...
% Progress indicator
wb = waitbar(0,'Running ENTRYRUN3C CONT2...','Position',[360 250 275 50]);
global WB
WB(end+1) = wb;
...
% Create random unit (shuffle)
    ...
    for t = 1:ic1
        ...
        ind2 = ind1 + increm - 1;
        vd{end+1} = vdisc_theta(find(vdisc_theta>=ind1&vdisc_theta<=ind2)) - ind1 + 1;
    end
    lvd = length(vd);
    rp = randperm(lvd);
    while any(rp==[1:lvd])
    ...
    for t = 1:ic1
        ...
        ind2 = ind1 + increm - 1;
        vd{end+1} = vdisc_noth(find(vdisc_noth>=ind1&vdisc_noth<=ind2)) - ind1 + 1;
    end
    lvd = length(vd);
    rp = randperm(lvd);
    while any(rp==[1:lvd])
