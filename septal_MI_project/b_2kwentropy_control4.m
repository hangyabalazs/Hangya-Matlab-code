function b_2kwentropy_control4
%2KWENTROPY_CONTROL4   Wavelet entropy calculation.
%   2KWENTROPY_CONTROL4 calculates wavelet magnitude entropies for MEGAWAVERUN_ENTRCTRL2 output
%   files. It calls ENTR2 for the raw entropy calculation. 2KWENTROPY_CONTROL4 computes entropy
...
wh_to = [DATAPATH 'Entropy2\control\random_vs_eeg\'];
files = dir(wh_from);
files = files(3:end);
...
% Progress indicator
wb = waitbar(0,'Running 2KWENTROPY CONTROL4...','Position',[360 250 275 50]);    %Progress indicator
global WB
WB(end+1) = wb;
