function b_rentropy_bh
%RENTROPY_BH   Entropy calculation.
%   RENTROPY_BH calculates entropies for 5 sec. data segments (80% overlaping windows). 
%   The concidered distributions are Blackman-Harris convolved unit amplitude distribution
%   and eeg amplitude distribution. Both signals are downsampled on 1000 Hz. It calls 
%   ENTR2 for the raw entropy calculation. Outputs:
...
wh_to = [DATAPATH 'Rentropy\raw_bh\real\'];
files = dir(wh_from);
files = files(3:end);
...
% Progress indicator
wb = waitbar(0,'Running RENTROPY BH...','Position',[360 250 275 50]);    %Progress indicator
global WB
WB(end+1) = wb;
...
for i = 1:sf    % CELL CYCLE
    ...
    abs1 = eeg;
    abs1 = abs1(1:10:end);
    abs2 = bhconv(vdisc,length(eeg));     % convolution
    abs2 = abs2(1:10:end);
    [k1 k2] = size(abs1);
