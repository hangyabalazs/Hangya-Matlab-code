function [Stats_eu, Stats_ue, STeu, STue] = b_entrymorestat6
%ENTRYMORESTAT6   Calculates statistics for entropy data.
%   ENTRYMORESTAT6 runs on the output of ENTRYRUN_THETA or ENTRYRUN3C. Edit
...
fn = [DATAPATH 'Entry3c\Stat\power\2all_entropy.xls'];
fnm = [DATAPATH 'Entry3c\Stat\power\stat_2all_entropy.mat'];
mm = pwd;
...
for o = 1:sf
    ...
    load(ftheta)
    ENT_eu_real_theta = aHx;
    ENT_ue_real_theta = aHy;
    mEeur_theta = mean(aHx);
    mEuer_theta = mean(aHy);

    cd(inpdir_noth)
    inx = find(strcmp(files_short{o},files_short_noth));
    fnoth = files_noth{inx};
    load(fnoth)
    ENT_eu_real_noth = aHx;
    ENT_ue_real_noth = aHy;
    mEeur_noth = mean(aHx);
    mEuer_noth = mean(aHy);
