function b_ica(d)
%ICA  Iterative cluster analysis.
...
%   ICA(D) creates different number of clusters from 2 to D and plots intraburst interval,
...
%   ICA, by itself, uses D = 20 default value.
%
%   ICA applies the following burst definition: intraburst intervals are the interspike
%   intervals containted by the largest interspike interval cluster.
%
%   See also ICA_BETA, BURST_CLS, BURST_CLS_AUTO and CLUSTER.
...
npt = input('Discard existing content or append data while writing ica.txt? /discard:  ENTER, append: a/','s');
if isempty(npt),
    fid = fopen('ica.txt','w');
elseif npt == 'a',
    fid = fopen('ica.txt','a');
...
%                     extraburstiv = [ivs(find(ivs<min(ivss))) ivs(find(ivs>max(ivss)))];
