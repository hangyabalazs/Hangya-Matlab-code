function ezshiftrun_hc
%EZSHIFTRUN_HC    Runs ZSHIFT on a set of files, calculates phase angles.
%   EZSHIFTRUN_HC uses an input and a result directory: user can set them
...
%
%   EZSHIFTRUN_HC uses its own ZSHIFT instead of ZSHIFT2.
%
%   EZSHIFTRUN_HC uses Rayleigh's Z-test (p = 0.005).
%
...
sf = length(files);
mmm = pwd;
cd([DATAPATH,'Ezshift_hc\']);  %Here are the results
create_subdir;         %create subdirectories

% Progress indicator
wb = waitbar(0,'Running EZSHIFTRUN_HC...','Position',[360 250 315 50]);    %Progress indicator
global WB
WB(end+1) = wb;
