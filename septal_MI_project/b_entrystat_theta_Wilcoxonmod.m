function [ThetaNoburst,ThetaBurst,TN,TB] = b_entrystat_theta_Wilcoxonmod
%ENTRYSTAT_THETA_WILCOXONMOD   Calculates statistics for entropy data (theta segments).
%   ENTRYSTAT_THETA_WILCOXONMOD runs on the output of ENTRYRUN_THETA or
...
%
%   See also ENTRYSTAT_NOTH_WILCOXONMOD.
...
for o = 1:sf
    ...
    [Wp_eu,Wh_eu] = ranksum(PDC_eu_real,PDC_eu_ctrl);
    [Wp_ue,Wh_ue] = ranksum(PDC_ue_real,PDC_ue_ctrl);
    if mPeuc > mPeur
        Wh_eu = 0;
        Wp_eu = -1;
    end
...
% Save
fn = [DATAPATH 'Entry\Stat\theta_Wilcoxonmod.xls'];
