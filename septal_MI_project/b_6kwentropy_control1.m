function b_6kwentropy_control1
%6KWENTROPY_CONTROL1   Wavelet entropy calculation.
%   6KWENTROPY_CONTROL1 calculates wavelet magnitude entropies for 2MEGAWAVERUN_ENTRCTRL output
%   files. It calls ENTR2 for the raw entropy calculation. 6KWENTROPY_CONTROL1 computes entropy
%   for eeg wavelet and delayed unit wavelet (1 second delay). Outputs:
...
%
%   See also ENTR2, WAVETHRES and MEGAWAVERUN_ENTRCTRL2.
...
wh_to = [DATAPATH 'Entropy6\control\delayed_unit_1sec\'];
files = dir(wh_from);
files = files(3:end);
...
% Progress indicator
wb = waitbar(0,'Running 6KWENTROPY CONTROL1...','Position',[360 250 275 50]);    %Progress indicator
global WB
WB(end+1) = wb;
...
for i = 1:11    % CELL CYCLE
    ...
    wavea = b_wavethres(wavea);     % thresholding
    wavee = b_wavethres(wavee);
    abs1 = wavea(pwind1:pwind2,:);  % eeg
    abs2 = wavee(pwind1:pwind2,:);  % delayed unit (1 sec.)
