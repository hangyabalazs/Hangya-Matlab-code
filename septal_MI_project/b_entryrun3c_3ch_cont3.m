function b_entryrun3_3ch2_cont3
%ENTRYRUN_3CH2_CONT3   Runs entropy and PDC on a sequence of theta segments.
%   ENTRYRUN_3CH2_CONT3 calculates wavelet power and wavelet phase entropy for
...
%
%   ENTRYRUN_3CH2_CONT3 attaches theta and non-theta segment together and 
...
resdir = [DATAPATH 'Entry3c_cont3_unitunit\Theta\'];
...
% Progress indicator
wb = waitbar(0,'Running ENTRYRUN3C 3CH CONT3...','Position',[360 250 275 50]);
global WB
WB(end+1) = wb;
...
% Create random unit (shuffle ISIs)
    ...
    isi_theta = diff(vdisc2_theta);
    isi_noth = diff(vdisc2_noth);
    ...
    psvd_theta = [vdisc2_theta(1) vdisc2_theta(1)+cumsum(psi1)];
    ...
    psvd_noth = [vdisc2_noth(1) vdisc2_noth(1)+cumsum(psi2)];
