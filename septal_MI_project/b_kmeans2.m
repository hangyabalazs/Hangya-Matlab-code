function [idx, C, sumD, D, sumdist] = b_kmeans2(X, k, varargin)
%KMEANS2 K-means clustering.
%   IDX = KMEANS2(X, K) partitions the points in the N-by-P data matrix
...
%   variables.  KMEANS2 returns an N-by-1 vector IDX containing the
%   cluster indices of each point.  By default, KMEANS2 uses squared
%   Euclidean distances.
%
%   KMEANS2 treats NaNs as missing data, and removes any rows of X that
%   contain NaNs. 
%
%   [IDX, C] = KMEANS2(X, K) returns the K cluster centroid locations in
%   the K-by-P matrix C.
%
%   [IDX, C, SUMD] = KMEANS2(X, K) returns the within-cluster sums of
%   point-to-centroid distances in the 1-by-K vector sumD.
%
%   [IDX, C, SUMD, D] = KMEANS2(X, K) returns distances from each point
%   to every centroid in the N-by-K matrix D.
%
%   [IDX, C, SUMD, D, SUMDIST] = KMEANS2(X, K) returns total sum of 
%   distances for all replications.
%
%   [ ... ] = KMEANS2(..., 'PARAM1',val1, 'PARAM2',val2, ...) allows you to
%   specify optional parameter name/value pairs to control the iterative
%   algorithm used by KMEANS2.  Parameters are:
%
%   'Distance' - Distance measure, in P-dimensional space, that KMEANS2
...
%                              KMEANS2 infers K from the first dimension of
...
%   KMEANS2 uses a two-phase iterative algorithm to minimize the sum of
...
%   KMEANS2 can converge to a local optimum, which in this case is a
