function b_zshiftrun_rao
%ZSHIFTRUN_RAO    Runs ZSHIFT on a set of files, calculates phase angles.
%   ZSHIFTRUN_RAO uses an input and a result directory: user can set them
...
%
%   ZSHIFTRUN_RAO uses its own ZSHIFT instead of ZSHIFT2.
%
%   ZSHIFTRUN_RAO uses Rao's spacing test instead of Rayleigh's Z-test -
%   see RAO for details.
%
...
sf = length(files);
mmm = pwd;
cd([DATAPATH,'Zshift_rao/']);  %Here are the results
create_subdir;         %create subdirectories

% Progress indicator
wb = waitbar(0,'Running ZSHIFTRUN RAO...','Position',[360 250 315 50]);    %Progress indicator
global WB
WB(end+1) = wb;
...
siglev = p05;  % level of significance: p=0.05
hold on
...
% -------------------------------------------------------------------------
function [U,p,p05] = rao(x)
%RAO    Rao's Spacing Test.
%   [U,P,P05] = RAO(X) calculates Rao's U-Statistic (U) with the corresponding
%   p-value (P). P05 gives U at p=0.05 significance level. Input argument X
