function b_entryrun3b_3ch
%ENTRYRUN3B_3CH_INV   Runs entropy and PDC on a sequence of theta segments.
%   ENTRYRUN3B_3CH_INV calculates wavelet power and wavelet phase entropy for
...
%
%   ENTRYRUN3B_3CH_INV attaches non-theta and theta segment together and
%   completes the calculation on the attached segments.  (Note that the 
%   difference between ENTRYRUN3_3B and ENTRYRUN3B_3B_INV is the order of
...
%   segment separately, in contrast to ENTRYRUN3_3CH.
%
...
resdir = [DATAPATH 'Entry2_unitunit_inv\Theta\'];
...
% Create new data (attach theta and non-theta segment)
    ...
    vdisc1_new = [vdisc1_noth-i_first_noth vdisc1_theta-i_first_theta+i_second_noth-i_first_noth];
    ...
    vdisc2_new = [vdisc2_noth-i_first_noth vdisc2_theta-i_first_theta+i_second_noth-i_first_noth];
...
% Create random MS unit (frequency-adjusted Poisson process)
    ...
    vdisc2_theta = vdisc2(find(vdisc2>=i_first_theta&vdisc2<=i_second_theta));
    vdisc2_noth = vdisc2(find(vdisc2>=i_first_noth&vdisc2<=i_second_noth));
    frq_theta = length(vdisc2_theta) / (i_second_theta - i_first_theta);
    frq_noth = length(vdisc2_noth) / (i_second_noth - i_first_noth);
