function b_entryrun_noth2_cont2
%ENTRYRUN_NOTH2_CONT2   Runs entropy and PDC on a sequence of non-theta segments.
%   ENTRYRUN_NOTH2_CONT2 calculates wavelet power and wavelet phase entropy
%   for non-theta segments using 1000-point non-overlapping and 50% overlapping
...
%   details).
%
%   See also DTF, 2DTF, ENTRY, ENTRYRUN3_CONT2 and ENTRYRUN_NOTH2.
...
resdir = [DATAPATH 'Entry0_cont2\Noth\'];
datadir = DATADIR2;
...
% Progress indicator
wb = waitbar(0,'Running ENTRYRUN NOTH2 CONT2...','Position',[360 250 275 50]);
global WB
WB(end+1) = wb;
...
% Create random unit (shuffle)
    ic1 = fix(seglen/10000);
    increm = round(seglen/ic1);     % 1-1.2 sec. long segments
    vd = {};
    for t = 1:ic1
        ind1 = i_first + (t - 1) * increm + 1;
        ind2 = ind1 + increm - 1;
        vd{end+1} = vdisc(find(vdisc>=ind1&vdisc<=ind2)) - ind1 + 1;
    end
    lvd = length(vd);
    rp = randperm(lvd);
    while any(rp==[1:lvd])
