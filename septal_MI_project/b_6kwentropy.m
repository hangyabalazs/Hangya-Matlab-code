function b_6kwentropy
%6WENTROPY   Wavelet entropy calculation.
%   6KWENTROPY calculates wavelet magnitude entropies for MEGAWAVERUN_ENTRCTRL2 output files. 
%   It calls ENTR2 for the raw entropy calculation. It computes entropy for the theta band
%   only and normalizes for the whole theta band. It uses 5 sec. long, 80 % overlaping windows.
%   Being a sixth generation entropy function, it thresholds wavelet power matricis before
...
% Progress indicator
wb = waitbar(0,'Running 6KWENTROPY...','Position',[360 250 275 50]);    %Progress indicator
global WB
WB(end+1) = wb;
...
for i = 1:11    % CELL CYCLE
    ...
    wavea = b_wavethres(wavea);
    waveb = b_wavethres(waveb);
