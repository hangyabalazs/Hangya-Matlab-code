function varargout = b_burst_gui_theta(varargin)
%B_BURST_GUI_THETA   GUI for cluster analysis.
%   BURST_GUI_THETA is a tool for cluster analysis on multiple data files.
%   For every datafile it is possible the choose an optimal cluster number
...
%
%   BURST_GUI_THETA runs on theta segments (theta analysis is required)!
%
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @b_burst_gui_theta_OpeningFcn, ...
                   'gui_OutputFcn',  @b_burst_gui_theta_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% -------------------------------------------------------------------------
% Executes just before b_burst_gui_theta is made visible.
% -------------------------------------------------------------------------
function b_burst_gui_theta_OpeningFcn(hObject, eventdata, handles, varargin)

% Choose default command line output for b_burst_gui_theta
...
% Initialize
global DATAPATH
handles.inpdir = [DATAPATH 'Burst\Burst\theta_hc\'];   % input directory
handles.resdir = [DATAPATH 'Burst\Cluster\Theta_hc\'];   % result directory
...
% -------------------------------------------------------------------------
function varargout = b_burst_gui_theta_OutputFcn(hObject, eventdata, handles) 
...
% Save
fn = [handles.resdir handles.th_name '_CLUSTER'];
...
for i = 1:length(act_list)
    if ~isempty(findstr('theta',act_list{i}))
        th_list(end+1).name = act_list{i};
        cmps = strread(th_list(end).name,'%s','delimiter','_MH'); % 3ch data needs M and H delimiter
        th_list(end).length = str2num(cmps{5}) - str2num(cmps{4});
        th_list(end).i_first = str2num(cmps{4});
        th_list(end).i_second = str2num(cmps{5});
    end
end
len_list = [th_list.length];
mxlc = find(len_list==max(len_list));
th = th_list(mxlc).name;
cmps2 = strread(th,'%s','delimiter','_');
handles.th_name = cmps2{1};
for nn = 2:length(cmps2)-1
    handles.th_name = [handles.th_name '_' cmps2{nn}];
end

% Load
handles.loaded = [handles.inpdir th];
load(handles.loaded)
guidata(hObject,handles)

% Create 'time' vector
dt = 0.0001;
time = [1:th_list(mxlc).i_second] * dt;
...
for dec = 2:5
    ...
    z = zeros(1,th_list(mxlc).i_second);
    z(Vdisc) = 1;
    z(Vdisc+1) = -1;
    hold off
    plot_handle = plot(time(th_list(mxlc).i_first:th_list(mxlc).i_second),...
        z(th_list(mxlc).i_first:th_list(mxlc).i_second),'b');
    axis([time(th_list(mxlc).i_first) time(th_list(mxlc).i_second) -1.5 1.5])
    hold on;
    ...
    set([gca; ach],'ButtonDownFcn','b_burst_gui_theta(''ccax'',gcbo,[],guidata(gcbo))');
end

% Set keypress function
set(gcf,'KeypressFcn','b_burst_gui_theta(''KPFcn'',gcbo,[],guidata(gcbo))');
guidata(hObject,handles)
