function b_2kwentropy_control6
%2KWENTROPY_CONTROL6   Wavelet entropy calculation.
%   2KWENTROPY_CONTROL6 calculates wavelet magnitude entropies for 2MEGAWAVERUN_ENTRCTRL output
%   files. It calls ENTR2 for the raw entropy calculation. 2KWENTROPY_CONTROL6 computes entropy
%   for random data wavelet and random data wavelet. Outputs:
...
wh_to = [DATAPATH 'Entropy2\control\random_vs_random\'];
files = dir(wh_from);
files = files(3:end);
...
% Progress indicator
wb = waitbar(0,'Running 2KWENTROPY CONTROL6...','Position',[360 250 275 50]);    %Progress indicator
global WB
WB(end+1) = wb;
...
for i = 1:sf-1    % CELL CYCLE
    ...
    ffnm = [wh_from '\' fnm];
    load(ffnm);
    abs1 = wavec(pwind1:pwind2,:);  % random data
    
    fnm = files2(i+1).name;
    ffnm = [wh_from '\' fnm];
    load(ffnm);
    abs2 = wavec(pwind1:pwind2,:);  % random data
