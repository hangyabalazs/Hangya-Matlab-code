function b_zshiftrun_onset
%ZSHIFTRUN_ONSET    Runs ZSHIFT on a set of files, calculates phase angles.
%   ZSHIFTRUN_ONSET uses an input and a result directory: user can set them
...
%
%   ZSHIFTRUN_ONSET uses its own ZSHIFT instead of ZSHIFT2.
%
%   ZSHIFTRUN_ONSET uses Rayleigh's Z-test.
%
%   ZSHIFTRUN_ONSET considers only spikes within one sec. from theta onset.
%
%   See also ZSHIFT2, ZSHIFTRUN3 and ZSHIFTRUN_RAO.
...
sf = length(files);
mmm = pwd;
cd([DATAPATH,'Zshift_onset/']);  %Here are the results
create_subdir;         %create subdirectories

% Progress indicator
wb = waitbar(0,'Running ZSHIFTRUN ONSET...','Position',[360 250 315 50]);    %Progress indicator
global WB
WB(end+1) = wb;
...
% Run 'zshift' on theta intervals
    ...
    for t = 1:th_index      % "THETA SEGMENT CYCLE"
        ...
        seglast = min(segments(2,t),segfirst+10000);
