function b_entryrun3b_inv_cont2
%ENTRYRUN3B_INV_CONT2   Runs entropy and PDC on a sequence of recordings.
%   ENTRYRUN3B_INV_CONT2 calculates wavelet power and wavelet phase entropy
...
%
%   ENTRYRUN3B_INV_CONT2 attaches non-theta and theta segment together and
%   completes the calculation on the attached segments. (Note that the
%   difference between ENTRYRUN_3B and ENTRYRUN_3B_INV is the order of
%   theta and non-theta segment.)
%
...
%
%   The program uses shuffled unit controls (see ENTRYRUN3_CONT2 for
%   details).
%
%   The program is implemented for 3-channel  data files as well.
%
%   See also DTF, 2DTF, ENTRY, ENTRYRUN3_CONT2, ENTRYRUN_THETA and 
%   ENTRYRUN_NOTH.
...
resdir = [DATAPATH 'Entry3b_inv_cont2\Theta\'];
datadir = DATADIR2;
...
% Progress indicator
wb = waitbar(0,'Running ENTRYRUN3B INV CONT2...','Position',[360 250 275 50]);
global WB
WB(end+1) = wb;
...
% Create random unit (shuffle)
    ...
    t = 1;
    vd = {};
    increm = 10000;     % 1 sec. long segments
    while t < (leneeg - increm)
        vd{end+1} = vdisc_new(find(vdisc_new>=t&vdisc_new<=min(t+increm-1,leneeg))) - t + 1;
        t = t + increm;
    end
    lvd = length(vd);
    rp = randperm(lvd);
    ...
    psvd_new = [];
    for j = 1:lvd
        psvd_new = [psvd_new vd{rp(j)}+(j-1)*increm];
    end
