...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @b_burst_gui_OpeningFcn, ...
                   'gui_OutputFcn',  @b_burst_gui_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% -------------------------------------------------------------------------
% Executes just before b_burst_gui is made visible.
% -------------------------------------------------------------------------
function b_burst_gui_OpeningFcn(hObject, eventdata, handles, varargin)

% Choose default command line output for b_burst_gui
...
% Initialize
global DATAPATH
handles.inpdir = [DATAPATH 'Burst\Burst\'];   % input directory
handles.resdir = [DATAPATH 'Burst\Cluster\Noth\'];   % result directory
...
% -------------------------------------------------------------------------
function varargout = b_burst_gui_OutputFcn(hObject, eventdata, handles) 
...
for i = 1:length(act_list)
    if ~isempty(findstr('noth',act_list{i}))
        ...
        cmps = strread(noth_list(end).name,'%s','delimiter','_');
...
handles.noth_name = [cmps2{1} '_' cmps2{2} '_' cmps2{3} '_' cmps2{4} '_' cmps2{5}];
...
for dec = 2:5
    ...
    set([gca; ach],'ButtonDownFcn','b_burst_gui(''ccax'',gcbo,[],guidata(gcbo))');
end

% Set keypress function
set(gcf,'KeypressFcn','b_burst_gui(''KPFcn'',gcbo,[],guidata(gcbo))');
guidata(hObject,handles)
