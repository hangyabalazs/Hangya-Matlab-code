function b_2kwentropy_control7
%2KWENTROPY_CONTROL7   Wavelet entropy calculation.
%   2KWENTROPY_CONTROL7 calculates wavelet magnitude entropies for 2MEGAWAVERUN_ENTRCTRL output
%   files. It calls ENTR2 for the raw entropy calculation. 2KWENTROPY_CONTROL7 computes entropy
%   for eeg wavelet and unit wavelet from different registrations. Outputs:
...
wh_to = [DATAPATH 'Entropy2\control\eeg_vs_unit\'];
files = dir(wh_from);
files = files(3:end);
...
% Progress indicator
wb = waitbar(0,'Running 2KWENTROPY CONTROL7...','Position',[360 250 275 50]);    %Progress indicator
global WB
WB(end+1) = wb;
