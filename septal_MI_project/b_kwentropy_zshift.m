function b_kwentropy_zshit
%KWENTROPY_ZSHIFT   Wavelet entropy calculation.
%   KWENTROPY_ZSHIFT calculates wavelet magnitude entropies for MEGAWAVERUN_ZSHIFT output files
%   (entropy for eeg and zshift-delayed unit). It calls ENTR2 for the raw entropy calculation. 
%   Being a second generation entropy function, it computes entropy for the theta band only and
%   normalizes for the whole theta band. 
%   Outputs:
...
%
%   See also ENTR2 and MEGAWAVERUN_ZSHIFT.
...
global DATADIR
global DATAPATH
wh_from = [DATAPATH 'Entropy7\megawave\'];
wh_to = [DATAPATH 'Entropy7\real\'];
files = dir(wh_from);
files = files(3:end);
...
% Progress indicator
wb = waitbar(0,'Running KWENTROPY ZSHIFT...','Position',[360 250 275 50]);    %Progress indicator
global WB
WB(end+1) = wb;
...
for i = 1:11    % CELL CYCLE
    ...
    abs2 = wavez(pwind1:pwind2,:);  % zshift-delayed unit
    clear wavea wavez
    [k1 k2] = size(abs1);
