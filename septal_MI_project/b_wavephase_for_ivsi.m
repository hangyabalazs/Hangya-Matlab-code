function [ds4,t3] = b_wavephase_for_ivsi
%WAVEPHASE_FOR_IVSI Verson of wavephase used by IVSI.
%   WAVEPHASE_FOR_IVSI has two output arguments: one contains the localization of the first
%   and last points of the theta intervals of the eeg, the other contains the localization 
%   of the eeg minimums.
%
%   WAVEPHASE_FOR_IVSI uses the following theta definition: more than 30 % of the eeg wavelet
%   power fall in the 3 - 6 Hz band.
%
%   Imported from Viktor.
%
%   See also WAVEPHASE and IVSI.
...
decc=input('If you accept this segment 1 else 0 ');
if decc==1 disp('Good boy!');
else disp('Looser');
    nbg=input('Specify the beginning (nth minimum)... ');
    nfin=input('...and we will show the END: ');
    if nfin==0
        nfin=length(t3);
    end;
    t3=t3(nbg:nfin);    %HB
    mbg=datinx1+bg+time(t3(nbg));
    mfin=datinx1+bg+time(t3(nfin));
    disp([mbg mfin])
    figure
plot(theta,'m')
set(gca,'XLim',[0 length(theta)])
hold on;
mi=min(theta); ma=max(theta);
for j=1:length(t3)  %HB
    rajz=[time(t3(j)) time(t3(j));mi ma];   %HB
    line(rajz(1,:),rajz(2,:));
end;
hold off;
title('Filtered EEG, minimums marked');
...
%   List of files calling WAVEPHASE_FOR_IVSI:
%   b_ivsi_main_for_ivsi3