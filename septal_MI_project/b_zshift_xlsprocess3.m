function zshift_xlsprocess3
%ZSHIFT_XLSPROCESS3    Generates plots from ZSHIFT output excel file.
%   ZSHIFT_XLSPROCESS3 works on the output excel file of ZSHIFTRUN (modified
...
%
%   See also ZSHIFTRUN2, ZSHIFTRUN3, ZSHIFT_XLSPROCESS and
%   ZSHIFT_XLSPROCESS2.
...
% Read excel file
global DATAPATH
fn = [DATAPATH 'Zshift4\text\zshift2_theta.xls'];
...
hold on
for i = 1:size(mtx,1)
    plot(mtx(i,3),mtx(i,4),'ko','MarkerSize',6,'MarkerFaceColor','black');
end
...
hold on
for i = 1:size(mtx,1)
    if mtx(i,3) > -4000 & mtx(i,3) < 4000 & mtx(i,4) < 600
        if mtx(i,10) == 1 & mtx(i,11) == 1
            h1 = plot(mtx(i,3),mtx(i,4),'mo','MarkerFaceColor','magenta','MarkerSize',12);
        elseif mtx(i,10) == 1 & mtx(i,11) ~= 1
            h2 = plot(mtx(i,3),mtx(i,4),'ro','MarkerFaceColor','red','MarkerSize',12);
        elseif mtx(i,10) ~= 1 & mtx(i,11) == 1
            h3 = plot(mtx(i,3),mtx(i,4),'bo','MarkerFaceColor','blue','MarkerSize',12);
        elseif mtx(i,10) ~= 1 & mtx(i,11) ~= 1
            h4 = plot(mtx(i,3),mtx(i,4),'k.','MarkerSize',20);
        end
    end
end
set(gca,'FontSize',16)
...
for i = 1:size(mtx,1)
    if mtx(i,3) > -4000 & mtx(i,3) < 4000 & mtx(i,4) < 600
        if mtx(i,12) == 1 | mtx(i,12) == 2 | mtx(i,12) == 7
            h1 = plot(mtx(i,3),mtx(i,4),'ro','MarkerFaceColor','red','MarkerSize',12);
        elseif mtx(i,12) == 4 | mtx(i,12) == 5
            h2 = plot(mtx(i,3),mtx(i,4),'bo','MarkerFaceColor','blue','MarkerSize',12);
        else
            h8 = plot(mtx(i,3),mtx(i,4),'k.','MarkerSize',20);
        end
    end
end
set(gca,'FontSize',16)
