function b_zshiftrun4
%ZSHIFTRUN4    Runs ZSHIFT on a set of files, calculates phase angles.
%   ZSHIFTRUN4 uses an input and a result directory: user can set them
...
%
%   ZSHIFTRUN4 uses its own ZSHIFT instead of ZSHIFT2.
%
%   ZSHIFTRUN4 uses Rayleigh's Z-test.
%
%   See also ZSHIFT2, ZSHIFTRUN3 and ZSHIFTRUN_ONSET.
...
sf = length(files);
mmm = pwd;
cd([DATAPATH,'Zshift_temp/']);  %Here are the results
create_subdir;         %create subdirectories

% Progress indicator
wb = waitbar(0,'Running ZSHIFTRUN4...','Position',[360 250 315 50]);    %Progress indicator
global WB
WB(end+1) = wb;
