function b_5kwentropy_control1
%5KWENTROPY_CONTROL1   Wavelet entropy calculation.
%   5KWENTROPY_CONTROL1 calculates wavelet magnitude entropies for 2MEGAWAVERUN_ENTRCTRL output
%   files. It calls ENTR2 for the raw entropy calculation. 5KWENTROPY_CONTROL1 computes entropy
...
wh_to = [DATAPATH 'Entropy5\control\delayed_unit_1sec\'];
files = dir(wh_from);
files = files(3:end);
...
% Progress indicator
wb = waitbar(0,'Running 5KWENTROPY CONTROL1...','Position',[360 250 275 50]);    %Progress indicator
global WB
WB(end+1) = wb;
