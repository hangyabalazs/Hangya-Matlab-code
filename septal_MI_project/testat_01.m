...
DF_theta = struct('PVp',[],'PVp_control',[],'PVn',[],'PVn_control',[],...
    ...
    'na',[],'na_control',[]);
DF_noth = struct('PVp',[],'PVp_control',[],'PVn',[],'PVn_control',[],...
    'HCNp',[],'HCNp_control',[],'HCNn',[],'HCNn_control',[],...
    'na',[],'na_control',[]);
NTE_thetaeu = struct('PVp',[],'PVp_control',[],'PVn',[],'PVn_control',[],...
    'HCNp',[],'HCNp_control',[],'HCNn',[],'HCNn_control',[],...
    'na',[],'na_control',[]);
NTE_notheu = struct('PVp',[],'PVp_control',[],'PVn',[],'PVn_control',[],...
    'HCNp',[],'HCNp_control',[],'HCNn',[],'HCNn_control',[],...
    'na',[],'na_control',[]);
NTE_thetaue = struct('PVp',[],'PVp_control',[],'PVn',[],'PVn_control',[],...
    'HCNp',[],'HCNp_control',[],'HCNn',[],'HCNn_control',[],...
    'na',[],'na_control',[]);
NTE_nothue = struct('PVp',[],'PVp_control',[],'PVn',[],'PVn_control',[],...
    'HCNp',[],'HCNp_control',[],'HCNn',[],'HCNn_control',[],...
    'na',[],'na_control',[]);
cellcounter = 0;
for o = 1:sf
    ...
    if ~isempty(id1)        % group MI values to anatomical categories
        ...
        if Wh_noth
            eval(['DF_noth.' id1 '(end+1:end+dsc) = fmh(DF_noth_eu,dsc);']);
            eval(['NTE_notheu.' id1 '(end+1:end+dsc) = fmh(NTE_noth_eu,dsc);']);
            eval(['NTE_nothue.' id1 '(end+1:end+dsc) = fmh(NTE_noth_ue,dsc);']);
        end
    end
    if ~isempty(id2)
        if Wh_theta
        ...
        if Wh_noth
            eval(['DF_noth.' id2 '(end+1:end+dsc) = fmh(DF_noth_eu,dsc);']);
            eval(['NTE_notheu.' id2 '(end+1:end+dsc) = fmh(NTE_noth_eu,dsc);']);
            eval(['NTE_nothue.' id2 '(end+1:end+dsc) = fmh(NTE_noth_ue,dsc);']);
        end
    end
    if isempty([id1 id2])
        if Wh_theta
        ...
        if Wh_noth
            eval(['DF_noth.na(end+1:end+dsc) = fmh(DF_noth_eu,dsc);']);
            eval(['NTE_notheu.na(end+1:end+dsc) = fmh(NTE_noth_eu,dsc);']);
            eval(['NTE_nothue.na(end+1:end+dsc) = fmh(NTE_noth_ue,dsc);']);
        end
    end
...
% Boxplot
H1 = mibox(DF_theta,NTE_thetaeu,NTE_thetaue);
% H2 = mibox(MI_noth);
...
% save('TEstat_01','MI_theta','MI_noth')
cd(mm)
