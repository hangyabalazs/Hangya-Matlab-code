function b_entryrun_theta2
%ENTRYRUN_THETA2   Runs entropy and PDC on a sequence of theta segments.
%   ENTRYRUN_THETA2 calculates wavelet power and wavelet phase entropy for
...
% pow = abs(wave).^2;
% phase = angle(wave);
[pow,period,scale,coi] = b_wavelet_pow3(dat,dt,pad,dj,s0,j1,mother,param,mis);
phase=1;
...
% pow = abs(wave).^2;
% phase = angle(wave);
[pow,period,scale,coi] = b_wavelet_pow3(zint,dt,pad,dj,s0,j1,mother,param,mis);
phase=1;
...
for i = 1:maxi*ovlp-ovlp+1        % ABS LOOP
    ...
    if isequal(porp,'power')
        [hx,hy,jh,Hx,Hy,Hxy,Hxcy,Hycx,Ixy,Uxy,Uyx] = b_entry_sub_temp(data1,data2,y1,bno,y2,bno); % MAIN
    elseif isequal(porp,'phase')
        [hx,hy,jh,Hx,Hy,Hxy,Hxcy,Hycx,Ixy,Uxy,Uyx] = b_entry_sub3_phase(data1,data2,y1,bno,y2,bno); % MAIN
    else
