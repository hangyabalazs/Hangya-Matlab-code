function b_rentropy_entropy
%RENTROPY_ENTROPY   Entropy calculation.
%   RENTROPY_ENTROPY calculates entropies for 5 sec. data segments (80% overlaping windows). The
%   concidered distributions are interspike interval distribution and eeg amplitude distribution.
%   Both signals are downsampled on 1000 Hz. It calls ENTROPY for the raw entropy calculation.
%   Outputs:
...
wh_to = [DATAPATH 'Rentropy\raw_entropy\real\'];
files = dir(wh_from);
files = files(3:end);
...
% Progress indicator
wb = waitbar(0,'Running RENTROPY ENTROPY...','Position',[360 250 275 50]);    %Progress indicator
global WB
WB(end+1) = wb;
...
% ABS LOOP
    for entloop = 1:entloopend*10-10        % WINDOW LOOP
        ...
        [hx,hy,jh,Hx,Hy,Hxy,Hxcy,Hycx,Ixy,Uxy,Uyx] = b_entropy(y1,y2); % MAIN
