function ezshiftrun_Rao
%EZSHIFTRUN_RAO    Runs ZSHIFT on a set of files, calculates phase angles.
%   EZSHIFTRUN_RAO uses an input and a result directory: user can set them
...
%
%   EZSHIFTRUN_RAO uses its own ZSHIFT instead of ZSHIFT2.
%
%   EZSHIFTRUN_RAO uses Rao's spacing test (p = 0.001).
%
%   See also ZSHIFT2, ZSHIFTRUN5 and EZSHIFTRUN.
...
sf = length(files);
mmm = pwd;
cd([DATAPATH,'Ezshift_Rao\']);  %Here are the results
...
% Progress indicator
wb = waitbar(0,'Running EZSHIFTRUN RAO...','Position',[360 250 315 50]);    %Progress indicator
global WB
WB(end+1) = wb;
...
siglev = p001;  % level of significance: p=0.001
hold on
