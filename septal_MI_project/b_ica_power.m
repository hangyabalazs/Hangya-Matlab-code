...
% Computing the input variables
    ...
    for seg = 1:segno   %"REGISTRATION SEGMENT CYCLE"
        ...
        clear time unit power      %free memory
...
%             fn_noext fln data eeg meret seglen segno seg datinx1 datinx2 th_int...
...
% Saving whole wavelet
        H = figure;
        save_whole_wavelet(H,power,filenam,datinx1,datinx2,f,newstep)
...
% Saving segment wavelet
            save_segmented_wavelet(H,power,intstart,intend,filenam,ind1,ind2,f,newstep)
...
% Single point spike triggered wavelet
            ...
            clear spstw_power aspstw_power sspstw_power   % free memory
...
% EEG analysis part
                [ScaleCvPower StwPowerMean StwPowerStd] =...
                    wave_segmentation(Burst,Vdisc,power,lenw,f,eeg,newstep,fdi,filenam,st,method,p,pend,...
                    ind1,ind2,fid_w_ward,fid_h_ward,fid_w_average,fid_h_average,fid_w_single,fid_h_single,H,seg);
                            
% Saving
                str = ['THETA_ICA_EEG_POWER_',method,'_',filenam,'_',num2str(ind1),'_',num2str(ind2),'.mat'];
                eval(['save ' str ' ScaleCvPower']);
                str = ['THETA_ICA_STW_POWER_',method,'_',filenam,'_',num2str(ind1),'_',num2str(ind2),'.mat'];
                eval(['save ' str ' StwPowerMean StwPowerStd']);
...
list = list';
save power_list list
cd(mmm);

% -----------------------------------------------------------------------------------------------------------------
function [ScaleCvPower, StwPowerMean, StwPowerStd] =...
    wave_segmentation(Burst,Vdisc,power,lenw,f,eeg,newstep,fdi,filenam,st,method,p,pend,...
    ind1,ind2,fid_w_ward,fid_h_ward,fid_w_average,fid_h_average,fid_w_single,fid_h_single,H,seg)
...
% ----------------------------------------------------------------------------------
function save_whole_wavelet(H,power,filenam,datinx1,datinx2,f,newstep)

% Plot power
cd wavelet
imagesc(power);

% Find & plot maximums in power
maxes = max(power);
sw2 = size(power,2);
maxloc = zeros(1,sw2);
for i = 1:sw2
...
% ----------------------------------------------------------------------------------
function save_segmented_wavelet(H,power,intstart,intend,filenam,ind1,ind2,f,newstep)
