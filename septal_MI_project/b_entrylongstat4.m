function b_entrylongstat4
%ENTRYLONGSTAT4   Calculates statistics for whole registration uncert. coeff.
%   ENTRYLONGSTAT4 calculates and saves t-test, Wilcoxon signrank-test and
...
%
%   Unlike ENTRYLONGSTAT, ENTRYLONGSTAT4 analyses segments with at least
%   100 spikes.
%
%   See also ENTRYLONGSTAT and ENTRYRUN4.

%   real_ue-real_eu>?ctrl_ue-ctrl_eu

% Directories
global DATAPATH
global DATADIR2
datadir = DATADIR2;
inpdir = [DATAPATH 'Entry_whole2\entropy\power\line\real\'];
inpdir2 = [DATAPATH 'Entry_whole2\entropy\power\line\control\'];
thetadir = [DATAPATH 'Wavelet2\theta_segments\'];
nothdir = [DATAPATH 'Wavelet2\nontheta_segments\'];
sharpdir = [DATAPATH 'Wavelet2\sharpwave_segments\'];
resdir = [DATAPATH 'Entry_whole2\Stat\'];
fnx = [resdir '\2longstat_ue.xls'];
fnm = [resdir '\2longstat_ue.mat'];
mm0 = pwd;
...
% Progress indicator
wb = waitbar(0,'Running ENTRYLONGSTAT4...','Position',[360 250 315 50]);    %Progress indicator
global WB
WB(end+1) = wb;
...
% Statistics for theta segments
    thetaseg = short_killer(min(ThetaSegments,length(aUxy)*sr),vdisc);
...
% Statistics for non-theta segments
    nothseg = short_killer(min(NonThetaSegments,length(aUxy)*sr),vdisc);
