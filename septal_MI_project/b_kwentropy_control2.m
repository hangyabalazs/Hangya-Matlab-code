function b_kwentropy_control2
%KWENTROPY_CONTROL2   Wavelet entropy calculation.
%   KWENTROPY_CONTROL2 calculates wavelet magnitude entropies for MEGAWAVERUN_ENTRCTRL3 output
%   files. It calls ENTR2 for the raw entropy calculation. KWENTROPY_CONTROL2 computes entropy
...
%
%   See also ENTR2 and MEGAWAVERUN_ENTRCTRL3.
...
wh_from2 = [DATAPATH 'Entropy\control\megawave2\'];
wh_to = [DATAPATH 'Entropy\control\delayed_unit_10sec\'];
files = dir(wh_from);
files = files(3:end);
...
% Progress indicator
wb = waitbar(0,'Running KWENTROPY CONTROL2...','Position',[360 250 275 50]);    %Progress indicator
global WB
WB(end+1) = wb;
...
for i = 1:10    % CELL CYCLE
    ...
    fname = fnm(end-9:end-4);
    load(fullfile(wh_from2,['MEGAWAVE_RESULT_' fname]));
    
    abs1 = wavea;  % eeg
    clear wavea;
    abs2 = wavef;  % delayed unit
    clear wavef;
    [k1 k2] = size(abs1);
