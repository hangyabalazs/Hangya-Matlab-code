function b_2kwentropy_control1
%2KWENTROPY_CONTROL1   Wavelet entropy calculation.
%   2KWENTROPY_CONTROL1 calculates wavelet magnitude entropies for 2MEGAWAVERUN_ENTRCTRL output
%   files. It calls ENTR2 for the raw entropy calculation. 2KWENTROPY_CONTROL1 computes entropy
...
wh_to = [DATAPATH 'Entropy2\control\delayed_unit_1sec\'];
files = dir(wh_from);
files = files(3:end);
...
% Progress indicator
wb = waitbar(0,'Running 2KWENTROPY CONTROL1...','Position',[360 250 275 50]);    %Progress indicator
global WB
WB(end+1) = wb;
