function b_entryrun3c_inv_convdtf
%ENTRYRUN3C_INV_CONVDTF   Runs entropy and PDC on a sequence of recordings.
%   ENTRYRUN3C_INV_CONVDTF calculates wavelet power and wavelet phase 
%   entropy for theta segments using 1000-point non-overlapping and 50% 
%   overlapping time windows. It saves line and time-frequency image 
%   visualization and entropy matrices as well.
%
%   It also calculates Partial Directed Coherence and saves PDC images, 
%   lines and PDC matrices.
%
%   ENTRYRUN3C_INV_CONVDTF attaches non-theta and theta segment together and
%   completes the calculation on the attached segments. (Note that the
%   difference between ENTRYRUN3C and ENTRYRUN3C_INV_CONVDTF is the order 
%   of theta and non-theta segment.)
%
...
%
%   It uses sinc-convolved unit for DTF calculation instead of instantenous
%   frequency function.
%
...
resdir = [DATAPATH 'Entry3c_inv\Theta\'];
datadir = DATADIR2;
...
% % Sinc convolution & wavelet transformation of unit
    [waveb_abs,waveb_phase,f] = unit_wavelet(vdisc_new,seglen_new,1);   % unit
%     [wavec_abs,wavec_phase,f] = unit_wavelet(psvd_new,seglen_new,0);    % random unit
%     
...
% -------------------------------------------------------------------------
function [pow,phase,f] = unit_wavelet(vdisc,lenu,zz)
...
for i = 1:maxi
    ...
    ind2 = ind1 + seglen - 1;
    eeg2 = eeg(ind1:ind2);
    zint2 = zint(round(ind1/10)+1:round(ind2/10));
    
    if ~isempty(zint2)
        [PDC(:,:,:,i),f] = dircoh(eeg2,zint2,sr,dsr);  % partial directed coherence
    else
        PDC(:,:,:,i) = 0;
        LIN1(end+1) = NaN;
        LIN2(end+1) = NaN;
...
% -------------------------------------------------------------------------
function [PDC,f] = dircoh(eeg,zint,sr,dsr)
...
zint = zint(1:2:end)';
un = zint;
noise = randn(size(un))/20*std(un);     % adding noise to the convolced unit;
un = un + noise;
