function b_2kwentropy_control8
%2KWENTROPY_CONTROL8   Wavelet entropy calculation.
%   2KWENTROPY_CONTROL8 calculates wavelet magnitude entropies for 2MEGAWAVERUN_ENTRCTRL output
%   files. It calls ENTR2 for the raw entropy calculation. 2KWENTROPY_CONTROL8 computes entropy
%   for eeg wavelet and eeg wavelet from different registrations. Outputs:
...
wh_to = [DATAPATH 'Entropy2\control\eeg_vs_eeg\'];
files = dir(wh_from);
files = files(3:end);
...
% Progress indicator
wb = waitbar(0,'Running 2KWENTROPY CONTROL8...','Position',[360 250 275 50]);    %Progress indicator
global WB
WB(end+1) = wb;
