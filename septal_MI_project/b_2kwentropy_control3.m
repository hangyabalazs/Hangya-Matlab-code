function b_2kwentropy_control3
%2KWENTROPY_CONTROL3   Wavelet entropy calculation.
%   2KWENTROPY_CONTROL3 calculates wavelet magnitude entropies for 2MEGAWAVERUN_ENTRCTRL output
%   files. It calls ENTR2 for the raw entropy calculation. 2KWENTROPY_CONTROL3 computes entropy
...
wh_to = [DATAPATH 'Entropy2\control\random_unit\'];
files = dir(wh_from);
files = files(3:end);
...
% Progress indicator
wb = waitbar(0,'Running 2KWENTROPY CONTROL3...','Position',[360 250 275 50]);    %Progress indicator
global WB
WB(end+1) = wb;
