function b_datatrasnsform3
%DATATRANSFORM3   Transforms sampling frequency to 10 kHz and converts data to usual form.
%   DATATRANSFORM3 resamples data registered on 16.667 kHz on 10 kHz and saves two channel
%   data (1.: eeg, 2.: unit).
%
%   See also DATATRANSFORM1, DATATRANSFORM2, DATATRANSFORM4 and RESAMPLE.
...
where1 = ['f:\raw_data\hcn\sampled_on_16_67_for_both\'];
files = dir(where1);
files = files(3:end);
sf = length(files);
mm = pwd;
...
% Transform EEG
    eeg = resample(eeg,3,5);
    new_sampfreq = round(16667*3/5);
