function b_kwentropy_control1
%KWENTROPY_CONTROL1   Wavelet entropy calculation.
%   KWENTROPY_CONTROL1 calculates wavelet magnitude entropies for MEGAWAVERUN_ENTRCTRL2 output
%   files. It calls ENTR2 for the raw entropy calculation. KWENTROPY_CONTROL1 computes entropy
...
%
%   See also ENTR and MEGAWAVERUN_ENTRCTRL2.
...
wh_to = [DATAPATH 'Entropy\control\delayed_unit\'];
files = dir(wh_from);
files = files(3:end);
...
% Progress indicator
wb = waitbar(0,'Running KWENTROPY CONTROL1...','Position',[360 250 275 50]);    %Progress indicator
global WB
WB(end+1) = wb;
