function b_wavephase
%WAVEPHASE  Computes Wavelet Spectrum.
%   WAVEPHASE uses the following theta definition: more than 30 % of the eeg wavelet
...
if decc==1 disp('Good boy!');
    ...
    mi=min(theta); ma=max(theta);
    for j=1:length(t3m)
        rajz=[time(t3m(j)) time(t3m(j));mi ma];
        line(rajz(1,:),rajz(2,:));
    end;
    hold off;
    title('Filtered EEG, minimums marked');
...
%   List of files calling WAVEPHASE: