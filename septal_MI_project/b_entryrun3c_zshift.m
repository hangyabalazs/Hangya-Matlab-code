function b_entryrun3c_zshift
%ENTRYRUN3C_ZSHIFT   Runs entropy and PDC on a sequence of recordings.
%   ENTRYRUN3C_ZSHIFT calculates wavelet power and wavelet phase entropy
...
%
%   ENTRYRUN3C_ZSHIFT shifts unit with the loaded zshift values - see
%   ZSHIFT2 and ZSHIFTRUN3 for details.
%
%   ENTRYRUN3C_ZSHIFT attaches theta and non-theta segment together and
%   completes the calculation on the attached segments.
%
%   It computes frequency adjusted random Poisson controls for entropy and
%   PDC calculations. Frequency is adjusted for theta and non-theta segment
%   separately, in contrast to ENTRYRUN3.
%
...
resdir = [DATAPATH 'Entry3c\Theta_newcells_zshift\'];
% datadir = [DATADIR 'mi_zshift_data\discriminated_hc\'];
datadir = DATADIR2;
fn = [DATAPATH 'Ezshift\zshift_longest_theta.xls'];
...
% Progress indicator
wb = waitbar(0,'Running ENTRYRUN3C ZSHIFT...','Position',[360 250 275 50]);
global WB
WB(end+1) = wb;

% Load
headerrows = 1;
[mtx ntx atx] = xlsread(fn,'maxlen');   % load zshift
ntx(1:headerrows,:) = [];
atx(1:headerrows,:) = [];
ln1 = atx(:,1);
...
% Create new data (attach theta and non-theta segment)
    ...
    vdisc_theta = vdisc(find(vdisc+dly>=i_first_theta&vdisc+dly<=i_second_theta)) + dly;    % shift unit
