function b_entryrun3
%ENTRYRUN3   Runs entropy and PDC on a sequence of recordings.
%   ENTRYRUN3 calculates wavelet power and wavelet phase entropy for theta
...
%
%   ENTRYRUN3 attaches theta and non-theta segment together and completes
%   the calculation on the attached segments.
%
%   The program is implemented for 3-channel  data files as well.
%
%   See also DTF, 2DTF, ENTRY, ENTRYRUN_THETA and ENTRYRUN_NOTH.
...
% Create random unit (frequency-adjusted Poisson process)
    ...
    vdisc_theta = vdisc(find(vdisc>=i_first_theta&vdisc<=i_second_theta));
    vdisc_noth = vdisc(find(vdisc>=i_first_noth&vdisc<=i_second_noth));
    frq = (length(vdisc_theta) + length(vdisc_noth)) / ...
