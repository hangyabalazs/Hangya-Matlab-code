function b_6kwentropy_control2
%6KWENTROPY_CONTROL2   Wavelet entropy calculation.
%   6KWENTROPY_CONTROL2 calculates wavelet magnitude entropies for 2MEGAWAVERUN_ENTRCTRL output
%   files. It calls ENTR2 for the raw entropy calculation. 6KWENTROPY_CONTROL2 computes entropy
...
%
%   See also ENTR2, WAVETHRES and 2MEGAWAVERUN_ENTRCTRL.
...
wh_to = [DATAPATH 'Entropy6\control\delayed_unit_10sec\'];
files = dir(wh_from);
files = files(3:end);
...
% Progress indicator
wb = waitbar(0,'Running 6KWENTROPY CONTROL2...','Position',[360 250 275 50]);    %Progress indicator
global WB
WB(end+1) = wb;
