function ezshift_xlsprocess2
%EZSHIFT_XLSPROCESS2    Generates plots from ZSHIFT output excel file.
%   EZSHIFT_XLSPROCESS2 works on the output excel file of
%   EZSHIFT_XLSCONVERT2. It generates various plots.
%
%   See also EZSHIFTRUN and EZSHIFT_XLSPROCESS.
...
global DATAPATH
dr = [DATAPATH 'Ezshift\'];
fn = [dr 'zshift_maxz_theta.xls'];
mm = pwd;
cd(dr)
headerrows = 1;
[mtx ntx atx] = xlsread(fn,'maxz');
ntx(1:headerrows,:) = [];
atx(1:headerrows,:) = [];
...
saveas(H1,'zshift_maxzPV.fig')

H2 = mibox(PVp,PVn,'PV+','PV-');
saveas(H2,'zshift_maxzPVbox.fig')
...
saveas(H3,'zshift_maxzHCN.fig')

H4 = mibox(HCNp,HCNn,'HCN+','HCN-');
saveas(H4,'zshift_maxzHCNbox.fig')
cd(mm)
