function b_entryrun4_3ch
%ENTRYRUN4_3CH   Runs entropy and PDC on a sequence of recordings.
%   ENTRYRUN4_3CH calculates wavelet power and wavelet phase entropy for 
...
%
%   ENTRYRUN4_3CH runs on all theta and non-theta segments.
%
...
resdir = [DATAPATH 'Entry_whole_3ch\'];
...
% Progress indicator
wb = waitbar(0,'Running ENTRYRUN4 3CH...','Position',[360 250 275 50]);
global WB
WB(end+1) = wb;
...
for o = 1:sf
    fname = files1(o).name;
    load([datadir fname])
    vdisc1 = vdisc(find(vdisc<1700000));      % cut end to prevent 'out of memory'
    
    fname = files2(o).name;
    load([datadir fname])
    vdisc2 = vdisc(find(vdisc<1700000));
...
% Create random unit (frequency-adjusted Poisson process)
    ...
    [psvd, ThetaSegments, NonThetaSegments, SharpWaveSegments] = ...
        genpoisson(thetadir,nothdir,sharpdir,fname,vdisc2);
        
% Sinc convolution & wavelet transformation of unit
    [wavea_abs,wavea_phase,f] = unit_wavelet(vdisc1,leneeg);   % HC unit
    [waveb_abs,waveb_phase,f] = unit_wavelet(vdisc2,leneeg);   % MS unit
