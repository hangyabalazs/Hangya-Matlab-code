function b_2kwentropy_control5
%2KWENTROPY_CONTROL5   Wavelet entropy calculation.
%   2KWENTROPY_CONTROL5 calculates wavelet magnitude entropies for 2MEGAWAVERUN_ENTRCTRL output
%   files. It calls ENTR2 for the raw entropy calculation. 2KWENTROPY_CONTROL5 computes entropy
%   for unit wavelet and random data wavelet. Outputs:
...
wh_to = [DATAPATH 'Entropy2\control\random_vs_unit\'];
files = dir(wh_from);
files = files(3:end);
...
% Progress indicator
wb = waitbar(0,'Running 2KWENTROPY CONTROL5...','Position',[360 250 275 50]);    %Progress indicator
global WB
WB(end+1) = wb;
