function b_ica_beta2b(d)
%ICA_BETA2B  Beta2b version of Iterative Cluster Analysis.
...
%   ICA_BETA2B(D) creates different number of clusters from 2 to D and computes intraburst 
...
%   ICA_BETA2B, by itself, uses D = 20 default value.
%
%   ICA_BETA2B, unlike ICA_BETA creates only matricies, not figures. You can display the 
%   figures using ICA_GUI2B.
%
%   ICA_BETA2B applies the following burst definition: intraburst intervals are the interspike
...
%   thresholding!
%
%   See also ICA, ICA_BETA, ICA_BETA2, ICA_WAVE, ICA_GUI2, ICA_GUI2B, BURST_CLS, BURST_CLS_AUTO
%   and CLUSTER.
...
where1 = [DATAPATH,'DATA\analysenow2d\'];    %Here are the data files
files = dir(where1);
files = files(3:end);
sf = length(files);
mmm = pwd;
...
% Return plot
        ...
        eval(['save(''THETA_ICA_',filenam,'.mat'',''ReturnPlotXData'',''ReturnPlotYData'')']);
