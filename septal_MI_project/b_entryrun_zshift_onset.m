function b_entryrun_zshift_onset
%ENTRYRUN_ZSHIFT_ONSET   Runs entropy on a sequence of theta segments.
%   ENTRYRUN_ZSHIFT_ONSET calculates wavelet power and wavelet phase entropy for
...
%
%   ENTRYRUN_ZSHIFT_ONSET calculates entropy for the first sec. of theta
%   segment, without downsampling (resolution: 100 ms).
%
%   ENTRYRUN_ZSHIFT_ONEST shifts unit with the loaded zshift values - see 
%   ZSHIFT2 and ZSHIFTRUN3 for details.
%
%   See also ENTRY, ENTRYRUN3, ENTRYRUN_ZSHIFT, ENTRYRUN_ONSET and
%   ENTRYRUN_THETA2.
...
resdir = [DATAPATH 'Entry_zshift_onset\Theta\'];
datadir = DATADIR2;
fn = [DATAPATH 'Zshift4\text\zshift3_theta.xls'];
...
% Progress indicator
wb = waitbar(0,'Running ENTRYRUN ZSHIFT ONSET...','Position',[360 250 275 50]);
global WB
WB(end+1) = wb;

% Load
mtx = xlsread(fn);      % load zshift
[temp ln1] = xlsread(fn,'A1:A185');
...
% Zshift unit
    ...
    dly = mtx(inx,3);
    lenu = length(eeg);
    vdisc = vdisc(find(vdisc+dly>0&vdisc+dly<lenu)) + dly;  % shift unit
    if length(vdisc) < 100
        continue
    end
