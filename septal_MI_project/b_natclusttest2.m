function list2 = b_natclusttest2
%NATCLUSTTEST2   Search for reasonable cutting ICC value.
%   NATCLUST runs on a sequence of files and determines cutting ICC (inconsistency 
%   coefficient) value, with whom the number of clusters by natural cluster analysing 
%   strategy is equal to 2. You can modify the input directory through editing the
%   program code.
%   The result is given in a structure array with fields 'name' - filename, 'limits'
%   - segment bounderies, 'method' - clustering method ('average', 'single' or 'ward')
%   and 'value' - the cutting ICC value. The result is saved in the output directory which
%   you can also  modify in the program code.
%
%   See also NATCLUST and NATCLUSTTEST.
...
% Import
list2 = [];      %initialize list of analysed files
...
% Cluster analysis
        for p = 1:pend      %"THETA SEGMENT CYCLE"
            ...
            for lnk = 1:3       %"METHOD CYCLE"
                ...
                ICCinf = 1.15470053837925;
                ICCsup = 1.15470053837926;
                incrementum = ICCsup - ICCinf;
                clusno = inf;
                ICC = ICCsup;
                while clusno ~= 2
                    % CLUSTER
                ...
                if islist    %generate list of analysed files
                    lenlist = length(list2);
                    list2(lenlist+1).name = filenam;
                    list2(lenlist+1).limits = [ind1 ind2];
                    list2(lenlist+1).method = method;
                    list2(lenlist+1).value = ICC;
                end
