function b_entryrun3c_inv_freq
%ENTRYRUN3C_INV_FREQ   Runs entropy and PDC on a sequence of recordings.
%   ENTRYRUN3C_INV_FERQ calculates entropy measures in the same way as
%   ENTRYRUN3C_INV, for real EEG and arteficial Poisson-units adjusted to
%   different ferquencies, in order to test whether changes of unti
%   ferquency alone influences uncertainty coefficients. For all the
%   details, see ENTRYRUN3C_INV.
%
%   See also ENTRYRUN3C_INV.
...
resdir = [DATAPATH 'Entry3c_inv_freq\Theta\'];
datadir = DATADIR2;
...
% Progress indicator
wb = waitbar(0,'Running ENTRYRUN3C INV FREQ...','Position',[360 250 275 50]);
global WB
WB(end+1) = wb;
...
% Create random unit (frequency-adjusted Poisson process)
    ...
    mlp = floor((o-0.01)/10) + 1;
    mlptr = [2 5 10];
    mlt = mlptr(mlp)
    frq_theta = mlt * frq_noth;
...
% Sinc convolution & wavelet transformation of unit
    ...
    if size(wavea_abs,2) > size(wavec_abs,2)    % adjust size
        wavea_abs = wavea_abs(:,1:end-1);
        wavea_phase = wavea_phase(:,1:end-1);
    elseif size(wavec_abs,2) > size(wavea_abs,2)
...
% Entropy
    ...
    clear wavea_abs wavea_phase wavec_abs wavec_phase r s
...
lm = floor(lm);
aHx_noth = aHx(1:lm);
aHx_theta = aHx(lm+2:end);
aHy_noth = aHy(1:lm);
aHy_theta = aHy(lm+2:end);
aHxy_noth = aHxy(1:lm);
aHxy_theta = aHxy(lm+2:end);
aIxy_noth = aIxy(1:lm);
aIxy_theta = aIxy(lm+2:end);
aIxynorm_noth = aIxynorm(1:lm);
aIxynorm_theta = aIxynorm(lm+2:end);
aRelHy_noth = aRelHy(1:lm);
aRelHy_theta = aRelHy(lm+2:end);
aRelHx_noth = aRelHx(1:lm);
aRelHx_theta = aRelHx(lm+2:end);
aHxcy_noth = aHxcy(1:lm);
aHxcy_theta = aHxcy(lm+2:end);
aHycx_noth = aHycx(1:lm);
aHycx_theta = aHycx(lm+2:end);
aUxy_noth = aUxy(1:lm);
aUxy_theta = aUxy(lm+2:end);
aUyx_noth = aUyx(1:lm);
aUyx_theta = aUyx(lm+2:end);
aBias_HR_noth = aBias_HR(1:lm);
aBias_HR_theta = aBias_HR(lm+2:end);
aBias_I_noth = aBias_I(1:lm);
aBias_I_theta = aBias_I(lm+2:end);
