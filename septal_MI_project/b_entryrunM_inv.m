function b_entryrunM_inv
%ENTRYRUNM_INV   Runs entropy and PDC on a sequence of recordings.
%   ENTRYRUNM_INV calculates wavelet power and wavelet phase entropy for
...
%
%   ENTRYRUNM_INV attaches non-theta and theta segment together and
%   completes the calculation on the attached segments. (Note that the
%   difference between ENTRYRUNM and ENTRYRUNM_INV is the order of theta
%   and non-theta segment.)
%
%   It computes controls for entropy and PDC calculations by shuffling 
%   interspike intervals in theta and non-theta segments.
%
...
resdir = [DATAPATH 'Entry3b_M_inv\Theta\'];
datadir = DATADIR2;
...
files_short = files_short(50)

sf = length(files_short);
[datalist, dlist_short] = b_filelist(datadir);

% Progress indicator
wb = waitbar(0,'Running ENTRYRUNM INV...','Position',[360 250 275 50]);
global WB
WB(end+1) = wb;
...
% Create random unit (shuffle ISIs)
    ...
    psvd_new = [psvd_theta-i_first_theta psvd_noth-i_first_noth+i_second_theta-i_first_theta];
    psvd_new = [psvd_noth-i_first_noth psvd_theta-i_first_theta+i_second_noth-i_first_noth];
...
for i = 1:maxi*ovlp-ovlp+1        % ABS LOOP
    ...
    if isequal(porp,'power')
        [hx,hy,jh,Hx,Hy,Hxy,Hxcy,Hycx,Ixy,Uxy,Uyx] = entry_sub1(data1,data2,y1,bno,y2,bno); % MAIN
...
% -------------------------------------------------------------------------
function [hx,hy,jh,Hx,Hy,Hxy,Hxcy,Hycx,Ixy,Uxy,Uyx] = entry_sub1(tfv1,tfv2,y1,h1,y2,h2)
