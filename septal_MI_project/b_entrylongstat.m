function b_entrylongstat
%ENTRYLONGSTAT   Calculates statistics for whole registration uncert. coeff.
%   ENTRYLONGSTAT calculates and saves t-test, Wilcoxon signrank-test and
...
% Directories
global DATAPATH
inpdir = [DATAPATH 'Entry_whole\entropy\power\line\real\'];
...
fn = [resdir '\longstat.xls'];
fnm = [resdir '\longstat.mat'];
mm0 = pwd;
...
% Statistics for theta segments
    thetaseg = short_killer(min(ThetaSegments,length(aUxy)*sr));
...
% Statistics for non-theta segments
    nothseg = short_killer(min(NonThetaSegments,length(aUxy)*sr));
...
xlswrite(fn,Tind1','Theta','B2');
xlswrite(fn,Tind2','Theta','C2');
xlswrite(fn,T,'Theta','D2');

xlswrite(fn,str,'Noth','B1');
xlswrite(fn,Nname','Noth','A2');
xlswrite(fn,Nind1','Noth','B2');
xlswrite(fn,Nind2','Noth','C2');
xlswrite(fn,N,'Noth','D2');
