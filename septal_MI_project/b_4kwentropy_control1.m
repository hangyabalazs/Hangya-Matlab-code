function b_4kwentropy_control1
%4KWENTROPY_CONTROL1   Wavelet entropy calculation.
%   4KWENTROPY_CONTROL1 calculates wavelet magnitude entropies for 2MEGAWAVERUN_ENTRCTRL output
%   files. It calls ENTR3 for the raw entropy calculation. 4KWENTROPY_CONTROL1 computes entropy
...
wh_to = [DATAPATH 'Entropy4\control\delayed_unit_1sec\'];
files = dir(wh_from);
files = files(3:end);
...
% Progress indicator
wb = waitbar(0,'Running 4KWENTROPY CONTROL1...','Position',[360 250 275 50]);    %Progress indicator
global WB
WB(end+1) = wb;
