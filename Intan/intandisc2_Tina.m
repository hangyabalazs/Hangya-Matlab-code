function [AllTimeStamps AllWaveForms] = intandisc2_Tina(data,thr,varargin)
%INTANDISC   Unit discrimination.
%   [T W] = INTANDISC(DATA,THR) performs threshold discrimination of
...
%   Time window for waveform data is set to -240 to 960 us.
%
...
chns{1} = [1 2 7 8];  %channel to tetrode assignment
chns{2} = [3 4 5 6];
chns{3} = [9 10 15 16];
chns{4} = [11 12 13 14];
...
for iT = 1:NumTetrodes
    ...
    for iC = 1:4
        chnum = iC;  % current channel
        disp(['tetrode: ' num2str(iT) '   Channel: ' num2str(chnum)])
        cdata = data(:,chns{iT}(chnum))';   % data from one channel
