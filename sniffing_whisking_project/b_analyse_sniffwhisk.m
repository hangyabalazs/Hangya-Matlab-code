function varargout = b_analyse_sniffwhisk(varargin)
% B_ANALYSE_SNIFFWHISK Application M-file for b_analyse_sniffwhisk.fig
%    FIG = B_ANALYSE_SNIFFWHISK launch b_analyse_sniffwhisk GUI.
%    B_ANALYSE_SNIFFWHISK('callback_name', ...) invoke the named callback.

% Last Modified by GUIDE v2.5 21-Feb-2011 19:46:02
...
set(fig,'Color',get(0,'DefaultUicontrolBackgroundColor'));
    ...
    set(handles.figure1,'CurrentAxes',handles.axes_eeg);
    handles.plot1_handle = plot(1,'Color',[1 1 1]);
    set(handles.figure1,'CurrentAxes',handles.axes_unit);
    ...
    handles.burstnum = -1;
    global DATAPATH
    burstdir = [DATAPATH 'Andi\Hajni_layer_5\Cluster\mat2\'];
    burstdir0 = [DATAPATH 'Andi\Hajni_layer_5\Cluster\mat\'];
    ...
    Cstart = imread([matlabroot '\work\Balazs\doubleleft_arrow.png']);
    Cprev = imread([matlabroot '\work\Balazs\left_arrow.png']);
    Cnext = imread([matlabroot '\work\Balazs\right_arrow.png']);
    ...
    addpath([matlabroot '\work\Nlx_converter'])
...
pos_text{6} = get(handles.text6,'Position');
set(handles.text6,...
    'Position',[((pos(3)-gui_pos(3))/2)+pos_text{6}(1) pos(4)-gui_pos(4)+pos_text{6}(2)...
        pos_text{6}(3) pos_text{6}(4)]);
set(handles.text6,'Units',old_units_text);
...
if isequal(get(handles.save_annot_submenu,'Enable'),'on')
    na = length(handles.annot);
    annis = zeros(1,na);
    for k = 1:na
        annis(k) = handles.annot{k}.IsSaved;
    end
    if any(~annis)
        button = questdlg_nomove('Do you want to save annotation objects?','Save Data Annotation','Yes');
        switch button
            case 'Yes'
                save_annot_submenu_Callback(h, eventdata, handles)
            case 'No'
                disp('Data annotation was not saved.')
            case 'Cancel'
                disp('Data importtation was interrupted.')
                return
        end
    end
end
...
cd([DATADIR 'HSW\'])
[pfilename, pathname] = uigetfile({'*.ncs','Neuralynx Continuous Data Files (*.ncs)';...
    '*.mat','All MAT-Files (*.mat)'; '*.*','All Files (*.*)'},'Select Datafile');

% If "Cancel" is selected then return
% profile on -detail operator
if isequal([pfilename,pathname],[0,0])
    return
end

% Clear previous annotations
clear_annot_submenu_Callback(h, eventdata, handles)
...
fpf = findstr(pathname,filesep);
filename = pathname(fpf(end-1)+1:end-1);
global FILENAME
FILENAME = filename;
fn = pathname;
dirstr = [DATAPATH 'analyse_gui\additional_data_for_analyse_gui\'];
nm = [dirstr filename '_vs'];
dr = dir(dirstr);
cell_dr = cell(1,length(dr)-1);
for m = 3:length(dr),
    cell_dr{m} = dr(m).name(1:end-7);
end
ss = strcmp(cell_dr,filename);
isfs = isempty(find(ss));
if isfs     %first time: import
    [eeg,unit,time,events] = in_for_analyse(fn,handles.sr);
    ...
    mintafr = handles.sr;
    dt = 1 / mintafr; 
    time = (0:eeglength - 1) * dt + startpoint; 
    handles.eegs = eegs;
    handles.units = units;
    handles.events = events;
...
b_liner_sniffwhisk(handles.plot1_handle,handles.eegs,handles.gui_pos,handles.la,eeglength,handles.sr,time(1));

set(handles.figure1,'CurrentAxes',handles.axes_unit);
axis([time(1) time(end) min_unit max_unit]);
b_liner2_sniffwhisk(handles.plot2_handle,handles.units,handles.gui_pos,handles.la,eeglength,handles.sr,time(1));

% Plot events
numev = length(handles.events);   % number of events
set(handles.figure1,'CurrentAxes',handles.axes_eeg)
y_lim = get(handles.axes_eeg,'YLim');
for k = 1:numev
    plot(handles.events{k},y_lim,'eeg')
end
set(handles.figure1,'CurrentAxes',handles.axes_unit)
y_lim = get(handles.axes_unit,'YLim');
for k = 1:numev
    plot(handles.events{k},y_lim,'unit')
end
drawnow

% Load annotation objects
dirstr = [DATAPATH 'analyse_gui\annotations\' filename '\'];
lf = [dirstr filename '_annot.mat'];
if b_isfilename(lf)
    load(lf)
    handles.annot = annot_objects;
    guidata(handles.figure1,handles)
    clear annot_objects
    disp('Data annotation was loaded.')
    if handles.plotannot   % plot annotation objects
        for k = 1:length(handles.annot)
            tannot = handles.annot{k};
            setparrentgui(tannot,handles.figure1)
            set(handles.figure1,'CurrentAxes',handles.axes_eeg)
            plot(tannot,[min_eeg max_eeg],'eeg')   % plot annotation
            set(handles.figure1,'CurrentAxes',handles.axes_unit)
            plot(tannot,[min_unit max_unit],'unit')
        end
        
        % Set buttondown and keypress functions
        set_ButtonDownFcns(h, eventdata, handles)
    end
end
...
if isfs
    ...
    startpoint = time(1);
    eeglength = length(eeg);
    save(nm,'eegs','units','eeglength','startpoint','events');
else
    [eeg,unit] = in_for_analyse2(fn,handles.sr);
    handles.eegs{1} = eeg;
...
slidstep = windowwidth / eps;
set(handles.slider1,'Value',time(1),'Min',time(1),'Max',time(end)-windowwidth+0.0001,'SliderStep',...
...
handles = guidata(handles.figure1);
kpf = 'b_analyse_sniffwhisk(''figure1_KeyPressFcn'',gcbo,[],guidata(gcbo))';
...
ch = get(handles.analysis_menu,'Children');
set(ch,'Enable','on')
ch = get(handles.annotation_menu,'Children');
set(ch,'Enable','on')

% Clear the global variables of previous cell
clear global SCALE WAVETIME WAVPROP_CURRENT HANDLES ANNOTOUT
...
% Clear message
set(handles.figure1,'Name','Sniffing Whisking Analysis');
    
    

% --------------------------------------------------------------------
function [eeg,unit,time,events] = in_for_analyse(fn,dsr)

% Data import
[p1 p2] = whichCSC(fn);   % get selected channel names
[TimeStamp, ChanNum, SampleFrequency, NumValSamples, Samples, NlxHeader] = ...
    Nlx2MatCSC(p1,[1 1 1 1 1],1,1,1);
eeg = (-1) * Samples(:);
sr_eeg = 512 / mean(diff(TimeStamp)) * 10^6;    % original sampling rate (changed later)
starttime1 = TimeStamp(1) / 10^6;
[TimeStamp, ChanNum, SampleFrequency, NumValSamples, Samples, NlxHeader] = ...
    Nlx2MatCSC(p2,[1 1 1 1 1],1,1,1);
unit = Samples(:);
sr_unit = 512 / mean(diff(TimeStamp)) * 10^6;    % original sampling rate (can be different from that for EEG)
...
[p q] = rat(dsr/sr_unit);
unit = resample(unit,p,q)';
dt = 1 / dsr;
time = (0:length(eeg)-1) * dt + TimeStamp(1) / 1000000;     % EEG time stamps

if timediff > 0.0001       % deal with misaligned data
    if ~isequal(dsr,1000)
        error('Data realignment only works for samplin rate = 1000 Hz.')
    end
    leadtime = timediff * dsr;    % lead of eeg in ms
    eeg(1:round(leadtime)) = [];  % drop lead: now they co-start
elseif timediff < -0.0001
    if ~isequal(dsr,1000)
        error('Data realignment only works for samplin rate = 1000 Hz.')
    end
    lagtime = (-1) * timediff * dsr;    % lag of eeg in ms
    unit(1:round(lagtime)) = [];  % drop lead of unit: now they co-start
end
ln = min(length(eeg),length(unit));
eeg = eeg(1:ln);
unit = unit(1:ln);   % now they co-terminate

% Filtering unit
nqf = dsr / 2;    % Nyquist frequency
...
% --------------------------------------------------------------------
function [eeg,unit] = in_for_analyse2(fn,dsr)
...
% -------------------------------------------------------------------------
function [p1 p2] = whichCSC(fn)

% Data table
tbl = {'R1' '2008-06-25_16-12-31' 'CSC4.ncs' 'CSC7.ncs';...
    'R3' '2008-07-09_14-46-47_R3' 'CSC4.ncs' 'CSC5.ncs';...
    'R3' '2008-07-21_14-16-20_R3' 'CSC4.ncs' 'CSC5.ncs';...
    'R7' '2008-08-27_15-26-23' 'CSC4.ncs' 'CSC6.ncs';...
    'R7' '2008-09-03_11-25-17' 'CSC4.ncs' 'CSC6.ncs';...
    'R7' '2008-09-03_15-59-08' 'CSC4.ncs' 'CSC6.ncs';...
    'R7' '2008-09-03_16-42-14' 'CSC4.ncs' 'CSC6.ncs';...
    'R7' '2008-09-03_16-51-55' 'CSC4.ncs' 'CSC7.ncs';...
    'R7' '2008-09-21_21-08-00' 'CSC4.ncs' 'CSC6.ncs';...
    'R7' '2008-09-21_21-39-42' 'CSC4.ncs' 'CSC6.ncs';...
    'R7' '2008-09-23_21-27-32' 'CSC4.ncs' 'CSC7.ncs';...
    'R7' '2008-09-24_15-29-53' 'CSC4.ncs' 'CSC7.ncs';...
    'P5' 'P5 2005-11-7' 'CSC4.ncs' 'CSC6.ncs';...
    'P5' 'P5 2005-11-8' 'CSC4.ncs' 'CSC7.ncs'};

% Parse pathname
fn2 = strrep(fn,'\',';');
cmps = strread(fn2,'%s','delimiter',';');
ses = cmps{end};
anim = cmps{end-1};

% Search the table
inx1 = strcmp(tbl(:,1),anim);
inx2 = strcmp(tbl(:,2),ses);
inx = find(inx1&inx2);  % matching index

% Output paths
p1 = [fn tbl{inx,3}];   % sniff
p2 = [fn tbl{inx,4}];   % whisk
...
try
    ...
    disp('Unable to save data annotation.')
    disp(ME.message)
end

% Clear global variables and delete GUI
clear global SCALE WAVETIME WHICHPERIODOGRAM WAVPROP_CURRENT HANDLES ANNOTOUT
clear global WAVE_EEG WAVE_EEG_COORD WAVE_EEG_PROP WAVE_UNIT WAVE_UNIT_COORD WAVE_UNIT_PROP
clear global AVERAGE_EEG AVERAGE_EEG_COORD AVERAGE_UNIT AVERAGE_UNIT_COORD
...
if x1 < 0 | x1 > handles.len
    ...
    b_liner_sniffwhisk(handles.plot1_handle,handles.eegs,handles.gui_pos,handles.la,handles.len,handles.sr,handles.time(1))
    
    set(handles.figure1,'CurrentAxes',handles.axes_unit);
    axis([x1 x_lim(2) y_unit(1) y_unit(2)]);
    b_liner2_sniffwhisk(handles.plot2_handle,handles.units,handles.gui_pos,handles.la,handles.len,handles.sr,handles.time(1))
end
...
b_liner_sniffwhisk(handles.plot1_handle,handles.eegs,handles.gui_pos,handles.la,handles.len,handles.sr,time(1));
b_liner2_sniffwhisk(handles.plot2_handle,handles.units,handles.gui_pos,handles.la,handles.len,handles.sr,time(1));
...
% -------------------------------------------------------------------------
% Button motion function - display event strings
% --------------------------------------------------------------------
function figure1_WindowButtonMotionFcn(h, eventdata, handles)

% Data cursor
cp = get(h,'CurrentPoint');
posaxeeg = get(handles.axes_eeg,'Position');
posaxunit = get(handles.axes_unit,'Position');
axeegxlim = get(handles.axes_eeg,'XLim');
axeegx = (cp(1) - posaxeeg(1)) / posaxeeg(3) * (axeegxlim(2) - (axeegxlim(1))) + axeegxlim(1);
% set(handles.text1,'String',num2str(axeegx))
tol = (axeegxlim(2) - axeegxlim(1)) / 100;

% Annotations
waset = 0;
if isfield(handles,'annot')
    evxs = cellfun(@(x)x.StartPoint,handles.annot);
    evxe = cellfun(@(x)x.EndPoint,handles.annot);
    evxsisdel = cellfun(@(x)x.IsDeleted,handles.annot);
    evxs = evxs(~evxsisdel);
    evxe = evxe(~evxsisdel);
    [sevxs inx] = sort(evxs,'ascend');
    lowneighbor = find(sevxs<axeegx,1,'last');
    highneighbor = find(sevxs>axeegx,1,'first');
    lowstarts = evxs < axeegx;
    highends = evxe > axeegx;
    nestings = find(lowstarts&highends);
    if axeegx - sevxs(lowneighbor) < tol
        set(handles.text6,'String',['START: ' handles.annot{inx(lowneighbor)}.AnnotString],'ForegroundColor',[0.25 0.75 0.25])
        waset = 1;
    elseif sevxs(highneighbor) - axeegx < tol
        set(handles.text6,'String',['START: ' handles.annot{inx(highneighbor)}.AnnotString],'ForegroundColor',[0.25 0.75 0.25])
        waset = 1;
    else
        if ~isempty(nestings)
            lne = length(nestings);
            str = cell(lne,1);
            for k = 1:length(nestings)
                str{k} = handles.annot{nestings(k)}.AnnotString;
            end
            set(handles.text6,'String',str,'ForegroundColor',[0.25 0.75 0.25])
            waset = 1;
        end
%         if ~isempty(lowneighbor) && ...
%                 ~isempty(handles.annot{inx(lowneighbor)}.EndPoint) && ...
%                 handles.annot{inx(lowneighbor)}.EndPoint > axeegx
%             set(handles.text6,'String',handles.annot{inx(lowneighbor)}.AnnotString,'ForegroundColor',[0.25 0.75 0.25])
%             waset = 1;
%         end
    end
    evxs = cellfun(@(x)x.EndPoint,handles.annot);
    evxsisdel = cellfun(@(x)x.IsDeleted,handles.annot);
    evxs = evxs(~evxsisdel);
    [sevxs inx] = sort(evxs,'ascend');
    lowneighbor = find(sevxs<axeegx,1,'last');
    highneighbor = find(sevxs>axeegx,1,'first');
    if axeegx - sevxs(lowneighbor) < tol
        set(handles.text6,'String',['END: ' handles.annot{inx(lowneighbor)}.AnnotString],'ForegroundColor',[0.25 0.75 0.25])
        waset = 1;
    elseif sevxs(highneighbor) - axeegx < tol
        set(handles.text6,'String',['END: ' handles.annot{inx(highneighbor)}.AnnotString],'ForegroundColor',[0.25 0.75 0.25])
        waset = 1;
    end
end
...
if isfield(handles,'events')
    evxs = cellfun(@(x)x.TimeStamp,handles.events);
    [sevxs inx] = sort(evxs,'ascend');
    lowneighbor = find(sevxs<axeegx,1,'last');
    highneighbor = find(sevxs>axeegx,1,'first');
    if axeegx - sevxs(lowneighbor) < tol
        set(handles.text6,'String',handles.events{inx(lowneighbor)}.EventString,'ForegroundColor','red')
        waset = 1;
    elseif sevxs(highneighbor) - axeegx < tol
        set(handles.text6,'String',handles.events{inx(highneighbor)}.EventString,'ForegroundColor','red')
        waset = 1;
    end
end
if ~waset
...
if isempty(wave)
    ...
    eval(str);
    sst_time = handles.time(datinx1:datinx2);
    newstep = handles.sr / wavprop.samprate;
    lensst = length(sst);
    
    if strcmp(st,'unit')        % unit sinc convolution
        sst = b_sincconv3(sst_time,sst,newstep,handles.sr);
    else
...
set(gcf,'KeypressFcn','b_analyse_sniffwhisk(''b_callfftswitch'',''eeg'')');
...
% Set keypress function
set(gcf,'KeypressFcn','b_analyse_sniffwhisk(''b_callfftswitch'',''unit'')');
...
bdf1 = 'b_analyse_sniffwhisk(''axes_eeg_ButtonDownFcn'',gcbo,[],guidata(gcbo))';
bdf2 = 'b_analyse_sniffwhisk(''axes_unit_ButtonDownFcn'',gcbo,[],guidata(gcbo))';
...
% -------------------------------------------------------------------------
% KeyPress Function for GUI window
% -------------------------------------------------------------------------
...
switch inp
case 'a'
    tannot = agannot([],[],'',handles);
    if ~isempty(tannot)
        handles.annot{end+1} = tannot;     % create annotation object
        if handles.plotannot
            set(handles.figure1,'CurrentAxes',handles.axes_eeg)
            plot(tannot,[handles.min_max{3} handles.min_max{4}],'eeg')   % plot annotation
            set(handles.figure1,'CurrentAxes',handles.axes_unit)
            plot(tannot,[handles.min_max{1} handles.min_max{2}],'unit')
            
            % Set buttondown and keypress functions
            set_ButtonDownFcns(hObject, eventdata, handles)
        end
    end
case 's'
    uiresume    % set annotation start point
case 'e'
    uiresume    % set annotation end point
end
guidata(handles.figure1,handles)
...
% --------------------------------------------------------------------
function new_annot_submenu_Callback(hObject, eventdata, handles)

% Create new annotation_menu by calling the keypress function
set(handles.figure1,'CurrentCharacter','a')
figure1_KeyPressFcn(hObject, eventdata, handles)



% --------------------------------------------------------------------
function clear_annot_submenu_Callback(hObject, eventdata, handles)

% Delete annotation_menu objects
na = length(handles.annot);
for k = 1:na
    delete(handles.annot{k})
end
handles.annot = {};
guidata(handles.figure1,handles)
...
% --------------------------------------------------------------------
function save_annot_submenu_Callback(hObject, eventdata, handles)

% Save annotation objects
global DATAPATH
dirstr = [DATAPATH 'analyse_gui\annotations\' handles.filename '\'];
nm = [dirstr handles.filename '_annot'];
annot_objects = handles.annot;
aisdel = cellfun(@(x)x.IsDeleted,annot_objects);
annot_objects = annot_objects(~aisdel);
save(nm,'annot_objects')
clear annot_objects
disp('Data annotation was saved.')