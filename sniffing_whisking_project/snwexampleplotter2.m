function snwexampleplotter2(cseg)
%SNWEXAMPLEPLOTTER2   Plot example for sniffing-whisking project.
%   SNWEXAMPLEPLOTTER2(CSEG) plots sniffing (raw, filtered, phase) and
...
figure;
A1 = axes('Position',[0.1300 0.2679 0.7750 0.6571]);
plot(standardize(lfsniff),'Color',[255 204 0]/255,'LineWidth',4)
hold on
plot(standardize(lsniff),'k','LineWidth',2)
A2 = axes('Position',[0.1300 0.1100 0.7750 0.0983]);
lrp = lresp_phase';
lrp(lrp>0) = -lrp(lrp>0);
imagesc(lrp)
cmp = colormap('bone');
linkaxes([A1 A2],'x')
axes(A1)
mx = max(standardize(lsniff));
hgt = mx * 0.15;
line([lfn2; lfn2],[zeros(size(lfn2))+mx; zeros(size(lfn2))+mx+hgt],...
    'Color','k','LineWidth',4)
% set(gcf,'Position',[1924 630 1900 342])
setappdata(gcf,'segment',cseg)

figure
plot(linspace(1,length(lfwhisk),length(lrms_whisk)),lrms_whisk/300,...
    'Color',[0 153 0]/255,'LineWidth',4)
hold on
plot(lfwhisk/300,'k')
mx = max(lfwhisk/300);
hgt = mx * 0.15;
line([lvdisc; lvdisc],[zeros(size(lvdisc))+mx; zeros(size(lvdisc))+mx+hgt],...
    'Color',[0 153 0]/255,'LineWidth',4)
