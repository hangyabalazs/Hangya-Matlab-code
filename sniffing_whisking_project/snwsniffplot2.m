function snwsniffplot2(rrat,rdate,csc1,csc2,msgn,thr,wl)
%SNWSNIFFPLOT2   Whisking phase relative to respiration.
%   SNWSNIFFPLOT2 is a modified version of SNWPHASE4. It generates a
...
%
%   SNWSNIFFPLOT2(RAT,DATE,CSC1,CSC2,MSGN,THR,WL) accepts input arguments
...
%
%   SNWSNIFFPLOT2 saves whisk events with high temporal precision.
%
...
global DATAPATH
global DATADIR
resdir = [DATAPATH 'SniffWhisk\phasehist_moreprecise2_midpoint\'];
...
% Detect whisking
[st nd pk1 pk2 pk3 fwhisk_ rms_whisk thr] = sspw3(whisk,sr,wl,10,inf,[],[],thr);  % whisking detection, see SSPW3
% pk = pk0;
% disp('RMS peak is used!!!!')
pk = mean([st; nd]);
disp('Midpoint is used!!!!')
% pk = pk3;
% disp('RMS-weighted time is used!!!!')
if plotting
    figure
...
vdisc = pk / wl;
sclr = 20;   % scaler for time_grp
time_grp = zeros(length(resp_phase2)*sclr,1);   % codes for types of whisking events, time-wise 
time_grp2 = zeros(length(resp_phase2)*sclr,1);   % indicator of cycles (time) before 1:2 whisking events
for k = 1:length(vdisc)
    ...
    switch lv
        case 2
            time_grp(floor(fn_pre*sclr):ceil(fn_post*sclr)) = 2;     % 2:1 coupling
        case 3              % 3:1 coupling
            time_grp(floor(fn_pre*sclr):ceil(fn_post*sclr)) = 3;
        case 1              % 1:1 coupling
            if lv_pre >= 1 || lv_post >= 1   % at least one event in 1 cycle pre or post
                time_grp(floor(fn_pre*sclr):ceil(fn_post*sclr)) = 1;
            else            % 1:2 coupling
                if lv_ppre >= 1 || lv_ppost >= 1    % no event in pre and post, but at least one event 2 cycles away
                    time_grp(floor(fn_ppre*sclr):ceil(fn_ppost*sclr)) = 0.5;
                    time_grp2(floor(fn_ppre*sclr):ceil((fn_pre-1)*sclr)) = 1;   % indicator for cyles before and after the ones with the whisking events in 1:2 coupling
                    time_grp2(floor(fn_post*sclr):ceil((fn_ppost-1)*sclr)) = 1;
                    if lv_ppre == 0     % prevent concatenation of 'out-of-phase' segments
                        time_grp(floor(fn_ppre*sclr):ceil(fn_ppre*sclr)) = 0;
                    elseif lv_ppost == 0
                        time_grp(floor(fn_ppost*sclr):ceil(fn_ppost*sclr)) = 0;
                    end
                else        % 1:3 coupling
                    if lv_pppre >= 1 || lv_pppost >= 1   % no event in 2 cycles around, but at least one event 3 cycles away
                        time_grp(floor(fn_pppre*sclr):ceil(fn_pppost*sclr)) = 1/3;
                        if lv_pppre == 0     % prevent concatenation of 'out-of-phase' segments
                            time_grp(floor(fn_pppre*sclr):ceil(fn_pppre*sclr)) = 0;
                        elseif lv_pppost == 0
                            time_grp(floor(fn_pppost*sclr):ceil(fn_pppost*sclr)) = 0;
                        end
                    end
                end
            end
...
grp = time_grp(round(vdisc*sclr))';    % group IDs for whisk events
...
for k = 1:length(dfs)-1     % loop through uniform segments
    ...
    tcseg = length(cseg) / sclr / sr * wl;   % length of the segment in seconds
    cgrp = grp(vdisc>=inx1s&vdisc<=inx2s&grp~=0);   % whisking events within the segment
    if tcseg < 0.5 || length(cgrp) < 3  % exculde segments < 0.5 s and segments with < 3 whisking events
        ...
        grp(vdisc>=inx1s&vdisc<=inx2s) = 0;
    else
        whs = find(vdisc>=inx1s&vdisc<=inx2s);
        vhs = vdisc(whs);
        whs2 = find(vdisc_rawwhisk>=inx1s&vdisc_rawwhisk<=inx2s);
        vhs2 = vdisc_rawwhisk(whs2);
        inst = fn(fn>=inx1s&fn<=inx2s+1);
        ...
        segments(seginx).start = inx1s;
        segments(seginx).end = inx2s;
...
for k = 2:length(grp)-1
    ...
    if grp(k) == 1 && (grp(k-1) ~= 1 || grp_split(k-1) == 1)
        ...
        time_grp_split(floor(fn_pre*sclr):ceil(fn_post*sclr)) = 0.9;
    end
end

% Convert time_grp back to data time
time_grp = time_grp(1:sclr:end);
time_grp2 = time_grp2(1:sclr:end);
time_grp_split = time_grp_split(1:sclr:end);
...
% -------------------------------------------------------------------------
function [st nd pk1 pk2 pk3 feeg rms thr] = sspw3(eeg,sr,wl,filtL,filtU,mlt1,mlt2,thr)
%SSPW3   Wave detection by RMS thresholding.
%   [ST ND PK1 PK2 PK3 FEEG RMS THR] = SSPW3(EEG,SR,WL,L,U,M1,M2)
%   detectects waves (filtered between L and U Hz) in EEG sampled at SR and
%   returns starting, end and peak points of epochs in ST, ND, PK1, PK2 and
%   PK3. PK1 localizes RMS peaks, PK2 finds the peaks of the original
%   signal and PK3 uses RMS-weighted time for peak localization. SSPW2 uses
%   the following criteria for wave detection: root-mean-square (window
%   size determined by WL) of filtered EEG should reach mean(RMS) + M2 *
%   std(RMS) and peak RMS should reach mean(RMS) + M1 * std(RMS). If M1 is
%   empty, the user is prompted to assess a threshold manually. If M2 is
%   empty, 0.1 of the discrimination thresold is used. Filtered EEG (FEEG),
%   root-mean-square (RMS) and discrimination threshold (THR) are also
%   returned.
%
...
for k = 1:length(pks)
    ...
    st(k) = sst;
    nd(k) = nnd;
    pk1(k) = sst + (find(rms(sst/wl:nnd/wl)==max(rms(sst/wl:nnd/wl)))-1) * wl;  % RMS peak
    pk2(k) = sst + find(feeg(sst:nnd)==max(feeg(sst:nnd)));   % peak of original data
    rmsl = rms(sst/wl:nnd/wl);
    rmsl(rmsl<0) = 0;
    pk3(k) = sum(rmsl.*(sst/wl:nnd/wl)) / sum(rmsl) * wl;   % RMS-weighted
    if pk1(k) < sst || pk2(k) < sst || pk3(k) < sst || ...
            pk1(k) > nnd+1 || pk2(k) > nnd+1 || pk3(k) > nnd+1
        error('Technical error 455.')
    end
end
st = st(st>0);
nd = nd(nd>0);
