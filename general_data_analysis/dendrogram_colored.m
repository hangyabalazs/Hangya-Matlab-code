function [hout,T,perm] = dendrogram_colored(Z,gr1,gr2,varargin)
%DENDROGRAM_COLORED Generate dendrogram plot.
%   DENDROGRAM_COLORED(Z) generates a dendrogram plot of the hierarchical
%   binary cluster tree represented by Z.  Z is an (M-1)-by-3 matrix,
%   generated by the LINKAGE function, where M is the number of objects in
...
%
%   DENDROGRAM_COLORED(Z,G1,G2) colors the leaves and corresponding
%   branches of groups G1 (red) and G2 (blue) of the dendrogram.
% 
%   DENDROGRAM_COLORED(Z,G1,G2,P) generates a dendrogram with no more than
%   P leaf nodes, by collapsing lower branches of the tree.  To display the
%   complete tree, set P = 0.
%
%   H = DENDROGRAM_COLORED(...) returns a vector of line handles.
%
%   [H,T] = DENDROGRAM_COLORED(...) generates a dendrogram and returns T, a vector
...
%
%   [H,T,PERM] = DENDROGRAM_COLORED(...) generates a dendrogram and returns the
...
%
%   [...] = DENDROGRAM_COLORED(...,'COLORTHRESHOLD',T) assigns a unique color to
...
%
%   [...] = DENDROGRAM_COLORED(...,'ORIENTATION',ORIENT) will orient the dendrogram 
...
%      'right'    --- right to left
%
%   [...] = DENDROGRAM_COLORED(...,'LABELS',S) accepts a character array or cell array
...
if groups > 1 && groups < (m-1)
    theGroups = ones(m-1,1) * 65;
    ccl1 = 2;
    ccl2 = 44;
    ccl3 = 23;
    for count = groups:-1:1
        if (theGroups(count) == 65)
            P = zeros(m-1,1);
            P(count) = 1;
...
%             numColors = numColors + 1;
            if any(ismember(gr1',lf1)) || any(ismember(gr1',lf2))
                theGroups(count) = ccl1;
            end
            if any(ismember(gr2',lf1)) || any(ismember(gr2',lf2))
                theGroups(count) = ccl2;
            end
            if (any(ismember(gr1',lf1)) || any(ismember(gr1',lf2))) && ...
                (any(ismember(gr2',lf1)) || any(ismember(gr2',lf2)))
                theGroups(count) = ccl3;
            end
        end
    end
    cmap = hsv;
