function varargout = b_thres3ch_gui(varargin)
%THRES3CH_GUI    Graphical User Interface for thresholding and discrimination 3-channel data files.
%   THRES3CH_GUI loads data preprocessed by THRESRUN2_3CH (online version 
%   has not been implemented yet). THRESRUN2 is an automatic thresholder
%   using THRES5. The unit with the multiple threshold is displayed in the
%   axes. User can zoom in an out with the mouse in the usual way. Pressing
%   down the shift key plus mouse click sets the threshold to new value.
%   Same result can be gained by editing the 'Threshold' box. Displayed
%   segment boundaries can be set int the 'Interval set' panel. Input
%   directory (THRESRUN2_3CH output directory, saved threshold values) and
%   output directory (discriminated data files) can be set as well.
...
%
%   THRES3CH_GUI is a version of THRES_GUI implemented for 3-channel data
%   files.
%
%   See also THRESRUN2_3CH.
...
if nargin == 0  % LAUNCH GUI
    ...
    try
        ...
        handles.inpdir = fullfile(DATAPATH,'Thres_3ch\');
        handles.resdir = [DATADIR '3ch_discriminated\'];
...
fs = b_findstr2(fln,'_','M','H');
if length(fs) > 1
    fname = fln(1:fs(2)-1);
else
    fname = fln;    % resampled files have short names
end
fn = ['THRES_' fln '.mat'];
...
otherwise   % zoom out
    ...
    axes(handles.axes1);
    b_liner(handles.plot1_handle, handles.units,handles.gui_pos,handles.la,handles.len);
end
drawnow;
