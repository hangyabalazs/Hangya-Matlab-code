function imisignio_stand(pat,patno,eg,nm_rows,nm_cols,esg)
%IMISIGNIO_STAND   Input-output function.
%   IMISIGNIOSTAND calculates input-output function for significant
%   propagation events. Standardized data is used. Results are plotted and
%   saved for each channels. Optional input arguments:
...
try
    resdir = [DATAPATH 'Ulbert\OITI_' patno '_EEG_' eg '\IOsign_stand\'];
    cd(resdir)
catch
    mkdir(resdir)
    cd(resdir)
