function imisignio_lin(pat,patno,eg,nm_rows,nm_cols,esg)
%IMISIGNIO_LIN   Input-output function.
%   IMISIGNIO_LIN calculates input-output function for significant propagation
%   events. Data is plotted and saved for each channels. Background color
%   for multiplot plots is set according to the strength of significant
%   linear correlations. Optional input
%   arguments:
...
try
    resdir = [DATAPATH 'Ulbert\OITI_' patno '_EEG_' eg '\IOsign_lin\'];
    cd(resdir)
catch
    mkdir(resdir)
    cd(resdir)
