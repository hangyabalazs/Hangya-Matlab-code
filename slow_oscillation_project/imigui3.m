function varargout = imigui3(varargin)
% IMIGUI3 M-file for imigui3.fig
%      IMIGUI3, by itself, creates a new IMIGUI3 or raises the existing
%      singleton*.
%
%      H = IMIGUI3 returns the handle to a new IMIGUI3 or the handle to
%      the existing singleton*.
%
%      IMIGUI3('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in IMIGUI3.M with the given input arguments.
%
%      IMIGUI3('Property','Value',...) creates a new IMIGUI3 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before imigui3_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to imigui3_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help imigui3
...
global DATAPATH
ff = [DATAPATH 'Ulbert\EEG_12\MImap\MIrawmaps_filt40_overlap10.mat'];
load(ff)
handles.rI = rI;
handles.Imax = max(rI(:));
handles.Imin = min(rI(:));
...
% Load MI shift data
ff = [DATAPATH 'Ulbert\EEG_12\MImap\MIshiftfine.mat'];
ff = [DATAPATH 'Ulbert\EEG_12\MImap\MIshiftcontrolfine.mat'];
load(ff)
handles.rIMaxLoc = rIMaxLoc;
handles.rIMax = rIMax;
handles.rIMaxmax = max(rIMax(:));
handles.rIMaxmin = min(rIMax(rIMax~=0));
handles.normrIMax = (rIMax - handles.rIMaxmin) / (handles.rIMaxmax - handles.rIMaxmin);
handles.rIMaxrate = 10;     % 10 rIMax value / sec.

% Original data
fn = 'F:\raw_data\human_SO\oiti37_lukacs\grid\mat_EEG_12\EEG12_4x5grid_18i_rs.mat';
fn = 'F:\raw_data\human_SO\oiti37_lukacs\grid\mat_EEG_12\arteficial_control.mat';
load(fn)
handles.data = data;
...
for x = 1:handles.chno
    for y = x+1:handles.chno  % color: strength; width: speed
        if ~isnan(troIM(x,y)) && trIML(x,y) > 10 &&  troIM(x,y) > 2.3475   % sig. lev.: 0.0001
        ...
        if ~isnan(troIM(y,x)) && trIML(y,x) > 10 &&  troIM(y,x) > 2.3475
