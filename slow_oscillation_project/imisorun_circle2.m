function imisorun_circle2(pat,patno,eg,nm_rows,nm_cols)
%IMISORUN_CIRCLE2   Slow oscillation propagation.
%   IMISORUN_CIRCLE2 calculates the number of circles and reciprocal
%   connections and compares them to random. It calculates normalized
%   results and statistical tests based on surrogate data.
%
%   See also IMISORUN_CIRCLE and IMISORUN.
...
try
    resdir = [DATAPATH 'Ulbert\OITI_' patno '_EEG_' eg '\Circle'];
    cd(resdir)
catch
    mkdir(resdir)
    cd(resdir)
...
randcircdist_norm = (randcircdist - mean(randcircdist)) / std(randcircdist);    % standardization
circno = (size(trjc3,1) - mean(randcircdist)) / std(randcircdist);

H = figure;
[nm xout] = hist(randcircdist_norm,20);
plot(xout,nm)
hold on
line([circno circno],get(gca,'YLim'),'Color',[0.8471 0.1608 0],'LineWidth',4)
set(gcf,'Position',[440 372 560 138])
set(gca,'LineWidth',2)
set(gca,'YTick',[])
set(gca,'FontSize',20)
box off
saveas(H,['randcircdist_norm_' eg])
saveas(H,['randcircdist_norm_' eg '.tif'])
p = length(find(randcircdist_norm>circno)) / mtn;
save circ3test p
...
randrecdist_norm = (randrecdist - mean(randrecdist)) / std(randrecdist);    % standardization
recno = (size(trjc2,1) - mean(randrecdist)) / std(randrecdist);

H = figure;
[nm xout] = hist(randrecdist_norm,20);
plot(xout,nm)
hold on
line([recno recno],get(gca,'YLim'),'Color',[0.8471 0.1608 0],'LineWidth',4)
set(gcf,'Position',[440 372 560 138])
set(gca,'LineWidth',2)
set(gca,'YTick',[])
set(gca,'FontSize',20)
box off
saveas(H,['randrecdist_norm_' eg])
saveas(H,['randrecdist_norm_' eg '.tif'])
p = length(find(randrecdist_norm>recno)) / mtn;
save rectest p
