function [MaxLocIxy MaxIxy Hsd Hse fcn fsd fsem fy] = imishift_io(eeg1,eeg2,sr)
%IMISHIFT_IO   Time-shifted mutual information.
%   [ML M] = IMISHIFT_IO(EEG1,EEG2,SR) calculates mutual information (MI)
%   on 1000-point non-overlapping raw EEG traces sampled on SR. MI is
...
%   returned in ML. It also plots input-output function for the optimally
%   shifted data. 
%   Note, that input EEG should be downsampled on 1000 Hz!
%
%   [ML M HSD HSE FCN FSD FSEM FY] = IMISHIFT_IO(EEG1,EEG2,SR) returns two
%   figure handles for input-output characteristics corresponding to
%   optimal delay, one with SD error bars and one with SEM error bars. It
%   also returns corresponding variables: FCN, mean I/O function; FSD, 
%   standard dev.; FSEM standard error; FY, XData for plotting.
%
...
for i = 1:maxii-1
    ...
    cumdatax = [cumdatax data1];    % for I/O function
    cumdatay = [cumdatay eeg2(inx1+MaxLocIxy(i):inx2+MaxLocIxy(i))];
