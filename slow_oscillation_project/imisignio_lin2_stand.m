function imisignio_lin2_stand(pat,patno,eg,nm_rows,nm_cols,esg)
%IMISIGNIO_LIN2_STAND   Input-output function.
%   IMISIGNIO_LIN2_STAND calculates input-output function for significant
%   propagation events based on cross-correlations. Standardized data are
%   used. Results are plotted and saved for each channels. Background color
%   for multiplot plots is set according to the strength of significant
%   linear correlations. Optional input arguments:
...
try
    resdir = [DATAPATH 'Ulbert\OITI_' patno '_EEG_' eg '\IOsign_lin2_stand\'];
    cd(resdir)
catch
    mkdir(resdir)
    cd(resdir)
