function varargout = imigui5_4x4(varargin)
% IMIGUI5_4X4 M-file for imigui5_4x4.fig
%      IMIGUI5_4X4, by itself, creates a new IMIGUI5_4X4 or raises the existing
%      singleton*.
%
%      H = IMIGUI5_4X4 returns the handle to a new IMIGUI5_4X4 or the handle to
%      the existing singleton*.
%
%      IMIGUI5_4X4('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in IMIGUI5_4X4.M with the given input arguments.
%
%      IMIGUI5_4X4('Property','Value',...) creates a new IMIGUI5_4X4 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before imigui5_4x4_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to imigui5_4x4_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help imigui5_4x4
...
global DATAPATH
ff = [DATAPATH 'Ulbert\OITI_39_EEG_10\MImap\MIrawmaps_EEG10_filt01_40_overlap10.mat'];
load(ff);nW=rI;
handles.nW = nW;
handles.Wmax = max(nW(:));
handles.Wmin = min(nW(:));
...
% Significance levels
ff = [DATAPATH 'Ulbert\OITI_39_EEG_10\MImap\siglev_EEG10.mat'];
% ff = [DATAPATH 'Ulbert\MImap\MIshiftcontrolfine.mat'];
load(ff)
handles.siglev = sl;

% Load MI shift data
ff = [DATAPATH 'Ulbert\OITI_39_EEG_10\MImap\MIshiftfine.mat'];
% ff = [DATAPATH 'Ulbert\MImap\MIshiftcontrolfine.mat'];
load(ff)
...
% Original data
global DATADIR
fn = [DATADIR 'human_SO\oiti39_gaal\grid\mat_EEG_10\EEG_10_1606_1671_rs.mat'];
...
for k = 1:numel_grid
    ...
    if rem(k,grid_horz) == 1 && k < numel_grid - grid_horz          % vertical
        ci = rIclr(handles,trI(k,k+grid_horz),sC);
        C(1,next) = ci;
        ciw = nWclr(handles,tnW(k,k+grid_horz),sC);
        Cw(1,next) = ciw;
        next = next + 1;
    elseif rem(k,grid_horz) > 1 && rem(k,grid_horz) < grid_horz && k < numel_grid - grid_horz
        ci = rIclr(handles,trI(k,k+grid_horz),sC);
    ...
    if rem(k,grid_horz) ~= 0 && k <= numel_grid - grid_horz     % diagonal
        ci = rIclr(handles,trI(k,k+grid_horz+1),sC);
        C(1,next) = ci;
        ciw = nWclr(handles,tnW(k,k+grid_horz+1),sC);
        Cw(1,next) = ciw;
        next = next + 1;
    end
    if rem(k,grid_horz) ~= 1 && k > grid_horz
...
for x = 1:handles.chno
    for y = x+1:handles.chno  % color: strength; width: speed
        if ~isnan(troIM(x,y)) && trIML(x,y) && troIM(x,y) > handles.siglev(4,2)   % sig. lev.: 0.0001
        ...
        if ~isnan(troIM(y,x)) && trIML(y,x) && troIM(y,x) > handles.siglev(4,2)
