function chan = ichanmatch
%ICHANMATCH   Checks if channel order is correct.
%   ICHANMATCH loads OITI mat and trc files and checks whether the channel
%   order in the mat file matches that of the trc file.
%
%   See also ICONVERT.
...
% Compare
[trcdata chnames] = trc_import(fid);
sd = size(data,2);
chan = cell(1,sd);
for k = 1:sd
    datarow = data(:,k);
    trm = 0;
    next = 1;
    sdtrc = size(trcdata,2);
    mxdiff = zeros(1,sdtrc);
    while ~trm
        if next > sdtrc
            fn = find(mxdiff==min(mxdiff));
            chan{k} = chnames{fn};
            disp(min(mxdiff));
            break
        end
        trcrow = trcdata(:,next);
        mn = min(length(datarow),length(trcrow));
        datarow = (datarow(1:mn) - mean(datarow(1:mn))) / std(datarow(1:mn));
        trcrow = (trcrow(1:mn) - mean(trcrow(1:mn))) / std(trcrow(1:mn));
        mxdiff(next) = max(trcrow(1:mn)-datarow(1:mn));
        if mxdiff(next) < 0.05
            chan{k} = chnames{k};
            trm = 1;
        end
...
% -------------------------------------------------------------------------
function [data, chnames] = trc_import(f)
...
switch d
    ...
    case {2,3}
        ...
        maxsec = (maxbyte - minbyte) / (orig_chnum * databyte * srate);
        fseek(f,0,-1);
        header = fread(f,handles.minbyte,'int8');
        fseek(f,184,-1);
        ordpoi = fread(f,1,'int32');
        fseek(f,200,-1);
        elpoi = fread(f,1,'int32');
