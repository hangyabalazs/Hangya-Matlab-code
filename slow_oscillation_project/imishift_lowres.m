function [MaxLocIxy MaxIxy] = imishift_lowres(eeg1,eeg2,sr)
%IMISHIFT_LOWRES   Time-shifted mutual information.
%   [ML M] = IMISHIFT_LOWRES(EEG1,EEG2,SR) calculates mutual information
%   (MI) on 1000-point non-overlapping raw EEG traces sampled on SR. MI is
...
%
%   Temporal resolution for IMISHIFT_LOWRES is 10 ms.
%
...
for i = 1:maxii-1
    ...
    T = [0:sr/100:sr];  % 10 ms  resolution
    Ixy = zeros(size(T));
    for t = T
