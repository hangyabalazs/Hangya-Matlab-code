function varargout = imigui4_4x4(varargin)
% IMIGUI4_4X4 M-file for imigui4_4x4.fig
%      IMIGUI4_4X4, by itself, creates a new IMIGUI4_4X4 or raises the existing
%      singleton*.
%
%      H = IMIGUI4_4X4 returns the handle to a new IMIGUI4_4X4 or the handle to
%      the existing singleton*.
%
%      IMIGUI4_4X4('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in IMIGUI4_4X4.M with the given input arguments.
%
%      IMIGUI4_4X4('Property','Value',...) creates a new IMIGUI4_4X4 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before imigui4_4x4_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to imigui4_4x4_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help imigui4_4x4
...
global DATAPATH
ff = [DATAPATH 'Ulbert\OITI_N1_EEG_157\MImap\MIrawmaps_EEG157_filt01_40_overlap10.mat'];
load(ff)
handles.rI = rI;
handles.Imax = max(rI(:));
handles.Imin = min(rI(:));
...
% Significance levels
ff = [DATAPATH 'Ulbert\OITI_N1_EEG_157\MImap\siglev_EEG157.mat'];
% ff = [DATAPATH 'Ulbert\MImap\MIshiftcontrolfine.mat'];
load(ff)
handles.siglev = sl;

% Load MI shift data
ff = [DATAPATH 'Ulbert\OITI_N1_EEG_157\MImap\MIshiftfine.mat'];
% ff = [DATAPATH 'Ulbert\MImap\MIshiftcontrolfine.mat'];
load(ff)
...
% Original data
global DATADIR
fn = [DATADIR 'human_SO\oitin1_wittner\grid\mat_EEG_157\EEG_157_1900_1965_rs.mat'];
