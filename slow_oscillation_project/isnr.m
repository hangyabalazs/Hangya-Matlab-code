function [rHx,rHy,rHxy,rIxy,rUxy,rUyx,cHx,cHy,cHxy,cIxy,cUxy,cUyx] = isnr(eeg1,eeg2,sr)
%ISNR   Wavelet-based Signal-to-noise rasio calculation.
%   IENTRY calculates wavelet power entropy using 1000-point non-overlapping
...
%
%   See also ENTRYRUN3C_CONT2 and AENTRY.

% Input argument check
error(nargchk(3,3,nargin))

% Create random unit (ISI shuffle)
leneeg = length(eeg1) * sr / 1000;
% isi = diff(vdisc);
% 
% lit = length(isi);
% rp = randperm(lit);
...
wavea_abs = eeg_wavelet(eeg1);    % eeg1

% Sinc convolution & wavelet transformation of unit
waveb_abs = eeg_wavelet(eeg2);   % eeg2
[wavec_abs,wavec_phase,f] = eeg_wavelet(psed);    % random eeg
...
abs1 = wavea_abs(pwind1:end,:);  % eeg1
abs2 = waveb_abs(pwind1:end,:);  % eeg2
abs3 = wavec_abs(pwind1:end,:);  % random eeg
