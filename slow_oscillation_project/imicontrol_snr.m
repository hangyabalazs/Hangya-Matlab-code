function imicontrol_snr
%IMICONTROL_SNR   Arteficial control for shifted mutual information calculation.
%   IMICONTROL_SNR generates arteficial control data by shifting a gaussian
...
data = data';
gns = 2:2:40;      % adding Gaussian noise
for k = 1:20
    data(:,k) = data(:,k) + rand(size(data(:,k))) * gns(k);
end
1
