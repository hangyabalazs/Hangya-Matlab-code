...
inhibition_start = max(inhibition_start,0.5);
...
semilogy(activation_start(activated),maxvalue(activated)./baseline(activated),'o','MarkerEdgeColor',[1 0.55 0.33],'MarkerFaceColor',[1 0.55 0.33],'MarkerSize',8)
hold on
semilogy(inhibition_start(inhibited),finh,'o','MarkerEdgeColor',[0.48 0.06 0.89],'MarkerFaceColor',[0.48 0.06 0.89],'MarkerSize',8)
semilogy(activation_start(tagged),maxvalue(tagged)./baseline(tagged),'o','MarkerEdgeColor',[0 0.7 0],'MarkerFaceColor',[0 0.7 0],'MarkerSize',8)
...
dist_activated = histc_overlap(activation_start(activated),edges);
...
edges = [0 0.5 1 2 4 7 10 20 30 60 90 120 150 180 200];
centers_tagged = (edges(1:end-1) + edges(2:end)) / 2;
dist_tagged = histc(activation_start(tagged),edges);
dist_tagged = dist_tagged(1:end-1) ./ diff(edges');
dist_tagged = dist_tagged / sum(dist_tagged);

edges = [0 0.5 1 2 10 15 20 30 60 90 120 150 180 200];
centers_inhibited = (edges(1:end-1) + edges(2:end)) / 2;
dist_inhibited = histc(inhibition_start(inhibited),edges);
...
dist_inhibited2 = exp(-dist_inhibited * 10);
