...
choosecb('VIP_gonogo')
global DATAPATH
load([DATAPATH 'VIP\responsesorter2_Miss_newwin4_stimaligned\allPSTH.mat'])  % load Hit PSTHs
[~, ~ , ~, maxvalue1_Hit] = groups(allstats,allpsth,tags);   % grouping of cells based on response

load([DATAPATH 'VIP\responsesorter2_CR_newwin4_stimaligned\allPSTH.mat'])  % load FA PSTHs
[VIPinx1 NTinx1 inhibited1 maxvalue1_FA] = groups(allstats,allpsth,tags);   % grouping of cells based on response

% Second CellBase: foot-shock
choosecb('VIP_gonogo2')
load([DATAPATH 'VIP\responsesorter2_Miss_newwin4_stimaligned_shock\allPSTH.mat'])  % load PSTHs
[~, ~, ~, maxvalue2_Hit] = groups(allstats,allpsth,tags);   % grouping of cells based on response

load([DATAPATH 'VIP\responsesorter2_CR_newwin4_stimaligned_shock\allPSTH.mat'])  % load PSTHs
[VIPinx2 NTinx2 inhibited2 maxvalue2_FA] = groups(allstats,allpsth,tags);   % grouping of cells based on response

% Maximal firing rates
mx_NT_FA = [maxvalue1_FA(NTinx1) maxvalue2_FA(NTinx2)];  % max FR after FA, NT cells
mx_NT_Hit = [maxvalue1_Hit(NTinx1) maxvalue2_Hit(NTinx2)];  % max FR after Hit, NT cells
mx_VIP_FA = [maxvalue1_FA(VIPinx1) maxvalue2_FA(VIPinx2)];  % max FR after FA, VIP cells
mx_VIP_Hit = [maxvalue1_Hit(VIPinx1) maxvalue2_Hit(VIPinx2)];  % max FR after Hit, VIP cells
...
% Marginal CDFs
edges = -2:0.01:20;  % histogram bin edges
...
% -------------------------------------------------------------------------
function [VIPinx NTinx inhibited maxvalue] = groups(allstats,allpsth,tags)
...
zp = 1200;
maxvalue = max(allpsth(:,zp+1:zp+400),[],2)';