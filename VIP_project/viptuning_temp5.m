...
xd = get(L,'XData');   % obtain no-stim. tuning curve
yd = get(L,'YData');
% plot(xd,yd/1.149*0.497,'r')   % ratio of peak values
% plot(xd,yd*mean(stim_mn./nostim_mn),'r')   % mean ratio
% plot(xd,yd*0.4,'r')
plot(xd,yd*0.35,'r')   % overlay scaled control curve
plot(xd,yd+mean(stim_mn-nostim_mn),'r')    % overlay shifted control curve
