function viptuning_jackknife
%VIPTUNING_JACKKNIFE   Spectro-temporal tuning curve.
%   VIPTUNING_JACKKNIFE calculates average auditory spectro-temporal tuning
...
%
%   VIPTUNING_JACKKNIFE also calculates and saves a jackknife sample for
...
%   (tuning curve peak).
%
%   See also VIPTUNING_REALIGN and VIPPLOTRESPONSE2.

% Results directory
global DATAPATH
resdir = [DATAPATH 'VIP\tuning_realign_jackknife\'];
...
% Jackknife sample for model parameters
bst = length(inh);   % size of jackknife sample
[fun_nostim fun_stim] = deal(cell(1,bst));  % preallocate
wb = waitbar(0,'Please wait...','Name','Running jackknife for inh. cells...');  % progress indicator
global WB
WB(end+1) = wb;
for b = 1:bst   % generate jackknife sample
    inhb = inh;
    inhb(b) = [];   % delete-1 jackknife
...
% Jackknife sample for model parameters
bst = length(act);   % size of jackknife sample
[fun_nostim fun_stim] = deal(cell(1,bst));  % preallocate
wb = waitbar(0,'Please wait...','Name','Running jackknife for act. cells...');  % progress indicator
WB(end+1) = wb;
for b = 1:bst   % generate jackknife sample
    actb = act;
    actb(b) = [];   % delete-1 jackknife
