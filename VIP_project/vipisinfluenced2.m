function [Wpi inhibition_start inhibition_end inhibition_time ...
    Wpa activation_start activation_end activation_time H Hr] = ...
    vipisinfluenced2(cellid)
%VIPISINFLUENCED2   Test for event-locked inhibition and activation.
%   VIPISINFLUENCED2(CELLID) tests whether spike number after an event
...
%
%   VIPISINFLUENCED2 takes a different bin raster (typically aligned to
...
%
%   [PI SI EI TI PA SA EA TA H] = VIPISINFLUENCED2(CELLID) returns start
...
%
%   See also VIPISINFLUENCED, SOMISINHIBITED2, SOMISSTIM3 and SOMPSTH_CALL2.
...
% Inhibition time
baseline_prob = mean(probb(1:st)) * 1000;  % spikes/sec (was spikes/bin before)
