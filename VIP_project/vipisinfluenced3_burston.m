...
%VIPISINFLUENCED3_BURSTON   Test for event-locked inhibition and activation.
%   VIPISINFLUENCED3_BURSTON(CELLID) tests whether spike number after an event
%   (e.g. 'BurstOn') changes. It uses a before-event baseline for
...
%   by crossings of the half-distence between the extreme and the baseline
%   before and after the minimum/maximum. This temporal window of
...
%
%   VIPISINFLUENCED3_BURSTON takes bin raster aligned to 'BurstOn' events
%   for both baseline and test distributions
%
%   [B PI MI SI EI PTI TI PA MA SA EA PTA TA H] =
%   VIPISINFLUENCED3_BURSTON(CELLID) returns minimal (MI) and maximal (MA)
%   value within 100 ms from the event; significance of inhibition (PI) and
%   activation (PA); start (SI), end (EI) and peak (PTI) time point of
%   inhibition, total inhibition time (TI), start point (SA), end (EA) and
%   peak (PTA) time point of activation, total activation time (TA) and
%   figure handle for SDF plot (H).
%
%   See also VIPISINFLUENCED, VIPISINFLUENCED2, VIPISINFLUENCED3,
%   SOMISINHIBITED2, SOMISSTIM3 and SOMPSTH_CALL2.
