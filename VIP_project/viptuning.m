...
load('C:\Balazs\_data\VIP\freq_tuning_data_unaffected.mat')   % cells unaffected by VIP stimulation
inh = [inh inhact1];
...
% Average tuning curve for the unaffected group
meantuning(f_tone,unaffected)  % [3 5 12 14 15 18 20] look inhibited in tuning; [1 8 19 21 24 27 28] look activated in tuning
...
for k = 1:NumCells;  % loop throuh all cells 
    ...
    nostimrate = nostimrate(:);   % enforce column vector form
    stimrate = stimrate(:);
...
for k = 1:NumCells   % loop through all cells
    ...
    stimrate = inh{k}.rate_laser;   % with laser stimulation
    nostimrate = nostimrate(:);   % enforce column vector form
    stimrate = stimrate(:);nninx = find(~isnan(stimrate)&~isnan(nostimrate));  % interpolate missing frequencies for averaging
