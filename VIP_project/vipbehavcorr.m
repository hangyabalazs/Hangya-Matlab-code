function vipbehavcorr
%VIPBEHAVCORR   Behavioral response profiles of VIP neurons.
%   VIPBEHAVCORR sorts PSTHs calculated by VIPRESPONSESORTER into groups
%   based on significant inhibition/activation and VIP positivity. Average
%   PSTHs are plotted.
%
...
choosecb('VIP_gonogo')
global DATAPATH
load([DATAPATH 'VIP\responsesorter2_FA_newwin\allPSTH.mat'])  % load PSTHs
[VIPactinx1 NTactinx1 inhibited1] = groups(allstats,tags);   % grouping of cells based on response
allpsth1 = allpsth;   % PSTH
allspsth1 = smoothPSTH(allpsth1);   % smoothed PSTH

% Second CellBase: foot-shock
choosecb('VIP_gonogo2')
load([DATAPATH 'VIP\responsesorter2_FA_newwin_shock\allPSTH.mat'])  % load PSTHs
[VIPactinx2 NTactinx2 inhibited2] = groups(allstats,tags);   % grouping of cells based on response
...
line([-1200 2200],[0 0],'Color','k')
set(gca,'box','off',...
    'FontSize',16,'TickDir','out','XLim',[-1000 2000],'Ylim',[-3 4])
xlabel('Time')
ylabel('Normalized firing rate')
