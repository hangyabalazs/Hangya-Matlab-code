...
fnm = [resdir 'tone_and_fb_rates.mat'];
...
for iC = 1:NumCells
    ...
    % Firing rates
    [spno1 spno2 len] = deal(nan(1,NumTrials));
    for iT = 1:NumTrials
        ...
        spno2(iT) = length(lst2(lst2>0&lst2<len(iT)));  % number of spikes after event2 within 'len'
    end
    fr1(iC) = sum(spno1) / sum(len);   % firing rate after first event
    fr2(iC) = sum(spno2) / sum(len);   % firing rate after second event 
end