...
[~, ~ , ~, baseline1_Hit] = groups(allstats,tags);   % grouping of cells based on response

load([DATAPATH 'VIP\responsesorter2_FA_newwin4\allPSTH.mat'])  % load FA PSTHs
[VIPactinx1 NTactinx1 inhibited1 baseline1_FA] = groups(allstats,tags);   % grouping of cells based on response

% Second CellBase: foot-shock
choosecb('VIP_gonogo2')
load([DATAPATH 'VIP\responsesorter2_Hit_newwin4_shock\allPSTH.mat'])  % load PSTHs
[~, ~, ~, baseline2_Hit] = groups(allstats,tags);   % grouping of cells based on response

load([DATAPATH 'VIP\responsesorter2_FA_newwin4_shock\allPSTH.mat'])  % load PSTHs
[VIPactinx2 NTactinx2 inhibited2 baseline2_FA] = groups(allstats,tags);   % grouping of cells based on response
...
mx_NT_FA = [log(fr_FA_tone1(NTactinx1)./baseline1_FA(NTactinx1)) log(fr_FA_tone2(NTactinx2)./baseline2_FA(NTactinx2))];  % max FR after FA, NT cells
mx_NT_Hit = [log(fr_Hit_tone1(NTactinx1)./baseline1_Hit(NTactinx1)) log(fr_Hit_tone2(NTactinx2)./baseline2_Hit(NTactinx2))];  % max FR after Hit, NT cells
mx_VIP_FA = [log(fr_FA_tone1(VIPactinx1)./baseline1_FA(VIPactinx1)) log(fr_FA_tone2(VIPactinx2)./baseline2_FA(VIPactinx2))];  % max FR after FA, VIP cells
mx_VIP_Hit = [log(fr_Hit_tone1(VIPactinx1)./baseline1_Hit(VIPactinx1)) log(fr_Hit_tone2(VIPactinx2)./baseline2_Hit(VIPactinx2))];  % max FR after Hit, VIP cells
...
% Marginal CDFs
edges = -2:0.05:4;  % histogram bin edges
...
% -------------------------------------------------------------------------
function [VIPactinx NTactinx inhibited baseline] = groups(allstats,tags)
