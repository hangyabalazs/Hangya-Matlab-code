...
[VIPinx1 NTinx1 inhibited1 maxvalue1_FA] = groups(allstats,tags);   % grouping of cells based on response
...
[VIPinx2 NTinx2 inhibited2 maxvalue2_FA] = groups(allstats,tags);   % grouping of cells based on response

% Load firing rates between tone and feedback, and after feedback
load([DATAPATH 'VIP\FRs\tone_and_fb_rates2.mat'])

% Maximal firing rates
mx_NT_FA = [log(fr_FA_fb1(NTinx1)./fr_FA_tone1(NTinx1)) log(fr_FA_fb2(NTinx2)./fr_FA_tone2(NTinx2))];  % max FR after FA, NT cells
mx_NT_Hit = [log(fr_Hit_fb1(NTinx1)./fr_Hit_tone1(NTinx1)) log(fr_Hit_fb2(NTinx2)./fr_Hit_tone2(NTinx2))];  % max FR after Hit, NT cells
mx_VIP_FA = [log(fr_FA_fb1(VIPinx1)./fr_FA_tone1(VIPinx1)) log(fr_FA_fb2(VIPinx2)./fr_FA_tone2(VIPinx2))];  % max FR after FA, VIP cells
mx_VIP_Hit = [log(fr_Hit_fb1(VIPinx1)./fr_Hit_tone1(VIPinx1)) log(fr_Hit_fb2(VIPinx2)./fr_Hit_tone2(VIPinx2))];  % max FR after Hit, VIP cells
...
% Marginal CDFs
edges = -12:2:52;  % histogram bin edges
...
% -------------------------------------------------------------------------
function [VIPinx NTinx inhibited maxvalue] = groups(allstats,tags)
