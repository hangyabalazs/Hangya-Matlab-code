...
global DATAPATH
load([DATAPATH 'VIP\responsesorter_Hit\allPSTH.mat'],'allstats')
Wpa1 = [allstats.Wpa];
load([DATAPATH 'VIP\responsesorter_FA\allPSTH.mat'],'allstats')
Wpa2 = [allstats.Wpa];
Wpa = nan(size(Wpa1));
...
global DATAPATH
load([DATAPATH 'VIP\responsesorter_FA_newwin_stimaligned_shock\allPSTH.mat'])
...
%% average PSTH - sign. cells, smoothed PSTH

figure
hold on;
errorshade(time,mean(allspsth(NTactinx,:)),std(allspsth(NTactinx,:))/sqrt(size(allspsth(NTactinx,:),1)),...
    'LineColor',[0.7 0.7 0.7],'ShadeColor',[0.7 0.7 0.7])
errorshade(time,mean(allspsth(VIPactinx,:)),std(allspsth(VIPactinx,:))/sqrt(size(allspsth(VIPactinx,:),1)),...
    'LineColor',[51 204 51]/255,'ShadeColor',[51 204 51]/255)
errorshade(time,mean(allspsth(inhibited,:)),std(allspsth(inhibited,:))/sqrt(size(allspsth(inhibited,:),1)),...
    'LineColor',[1 0.6 0.78],'ShadeColor',[1 0.6 0.78])
