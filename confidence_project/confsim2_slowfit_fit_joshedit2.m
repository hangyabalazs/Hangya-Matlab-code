...
%     dTF = sign(rand(1)-0.5) * d_inxTF / bnod * (mxd - mnd) + mnd;   % mid-percept from discretized percept
    ...
    for k = 1:nBoot
        ...
        if choiceTF   % left side choice
            ...
            % to this conf, add N(sigma) - sigma is parameter
...
for x = 1:NumTrials
    if so(x) == 1
        pD = CorrectConfTF(d_inx(x),:);
        HumanConfidenceReports
        % human conf -> prob (0 to 1)
        % CorrectConfTF - hist norm. to sum 1; 5 bins
        % human conf is in one of the bins
        % get the bins prob.
    elseif so(x) == 0
        pD = ErrorConfTF(d_inx(x),:);
