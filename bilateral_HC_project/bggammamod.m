function bggammamod(data,trials)
%BGGAMMAMOD   Modulation of gamma power by theta phase.
%   BGGAMMAMOD(DATA,TRIALS) compares the modulation of gamma power by theta
%   (8-10 Hz) phase in the ipsilateral and in the contralateral
...
%
%   See also BGMI, BGGAMMAMOD2, BGGAMMAMOD3 and BGGAMMAMOD4.

% Results directory
global DATAPATH
resdir = [DATAPATH 'Wheel\Gammamod\'];
mm = pwd;
cd(resdir)
...
% Wavelet MI
    thinx1 = find(f>=8,1,'last');    % theta frequency band bounderies: 8-10 Hz
    thinx2 = find(f<=10,1,'first');
    thetaR = nrm1(mean(wavea_phaseR(thinx2:thinx1,:))); % normalized
    thetaL = nrm1(mean(wavea_phaseL(thinx2:thinx1,:)));
    glinx1 = find(f>=30,1,'last');    % low gamma frequency band bounderies: 30-40 Hz
    glinx2 = find(f<=40,1,'first');
    gammalR = nrm1(mean(wavea_absR(glinx2:glinx1,:)));
    gammalL = nrm1(mean(wavea_absL(glinx2:glinx1,:)));
    ghinx1 = find(f>=80,1,'last');    % high gamma frequency band bounderies: 80-120 Hz
    ghinx2 = find(f<=120,1,'first');
    gammahR = nrm1(mean(wavea_absR(ghinx2:ghinx1,:)));
    gammahL = nrm1(mean(wavea_absL(ghinx2:ghinx1,:)));
