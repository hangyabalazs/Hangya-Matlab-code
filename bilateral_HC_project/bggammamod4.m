function bggammamod4(data,trials)
%BGGAMMAMOD4   Modulation of gamma power by theta phase.
%   BGGAMMAMOD4(DATA,TRIALS) compares the modulation of gamma power by theta
...
%   Hilbert transformation. Coupling is detected by the MI-shift method.
%
%   See also BGMI, BGGAMMAMOD, BGGAMMAMOD2 and BGGAMMAMOD3.

% Results directory
global DATAPATH
resdir = [DATAPATH 'Wheel\Gammamod3\'];
mm = pwd;
cd(resdir)
...
% MI
   [MItRglR MItRglL MItLglR MItLglL MItRghR MItRghL MItLghR MItLghL] = ...
        wavelet_mi(thetaR,thetaL,gammalR,gammalL,gammahR,gammahL,1000,10,sr);
...
function [MItRglR MItRglL MItLglR MItLglL MItRghR MItRghL MItLghR MItLghL] = ...
    wavelet_mi(thetaR,thetaL,gammalR,gammalL,gammahR,gammahL,WindowSize,Overlap,sr)
...
for i = 1:maxi*ovlp-ovlp+1        % ABS LOOP
    ...
    T = -sr/4:sr/200:sr/4;
    pMItRglR = zeros(size(T));
    pMItRglL = zeros(size(T));
    pMItLglR = zeros(size(T));
    pMItLglL = zeros(size(T));
    pMItRghR = zeros(size(T));
    pMItRghL = zeros(size(T));
    pMItLghR = zeros(size(T));
    pMItLghL = zeros(size(T));
    next = 1;
    for t = T
        yj1 = thetaR(inx1:inx2);   % eeg wavelet "cells"
        if inx1 + t < 1 || inx2 + t > length(thetaR)
            continue
        end
        yj2 = gammalR(inx1+t:inx2+t);
        numy = numel(yj1);
    ...
    MItRglR(i) = max(pMItRglR);
    MItRglL(i) = max(pMItRglL);
    MItLglR(i) = max(pMItLglR);
    MItLglL(i) = max(pMItLglL);
    MItRghR(i) = max(pMItRghR);
    MItRghL(i) = max(pMItRghL);
    MItLghR(i) = max(pMItLghR);
    MItLghL(i) = max(pMItLghL);
end     % end of abs loop
