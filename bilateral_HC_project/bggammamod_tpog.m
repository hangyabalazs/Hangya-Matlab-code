function bggammamod_tpog(data,trials)
%BGGAMMAMOD_TPOG   Modulation of gamma power by theta phase.
%   BGGAMMAMOD_TPOG(DATA,TRIALS) calculates phase histogram of high gamma
%   power values relative to theta (4-12 Hz) phase in the ipsilateral and
%   in the contralateral hippocampus. Low gamma (30-40 Hz) and high gamma
%   (80-120 Hz) are calculated separately. Phase and amplitude values are
%   generated by Hilbert transformation.
%
%   See also BGGAMMAMOD2 and BGGAMMAMOD_CCR.

% Results directory
global DATAPATH
resdir = [DATAPATH 'Wheel\Gammaphase\'];
mm = pwd;
cd(resdir)
...
% Theta phase of gamma peaks
    inxglR = find(gammalR>mean(gammalR)+1*std(gammalR));
    inxghR = find(gammahR>mean(gammahR)+1*std(gammahR));
    inxglL = find(gammalL>mean(gammalL)+1*std(gammalL));
    inxghL = find(gammahL>mean(gammahL)+1*std(gammahL));

% Plot
    HtRglR = figure;
    hist(thetaR(inxglR),20)
    HtRglL = figure;
    hist(thetaR(inxglL),20)
    HtRghR = figure;
    hist(thetaR(inxghR),20)
    HtRghL = figure;
    hist(thetaR(inxghL),20)
...
%             futureR_WE = [futureR_WE sum(aHy,2)];
    ...
    fn = ['g01_m13_t' num2str(k) '_TPoGtRglR.fig'];
    saveas(HtRglR,fn)
    fn = ['g01_m13_t' num2str(k) '_TPoGtRglL.fig'];
    saveas(HtRglL,fn)
    fn = ['g01_m13_t' num2str(k) '_TPoGtRhlR.fig'];
    saveas(HtRghR,fn)
    fn = ['g01_m13_t' num2str(k) '_TPoGtRhlL.fig'];
    saveas(HtRghL,fn)
