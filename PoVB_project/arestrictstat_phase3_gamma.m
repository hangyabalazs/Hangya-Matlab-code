function arestrictstat_phase3_gamma
%ARESTRICTSTAT_PHASE3_GAMMA   Mean phase histograms for bounded frequency band.
%   ARESTRICTSTAT_PHASE3_GAMMA calculates summary beta phase statistics for
%   Po, VPM, PoVPM, LD and nRT data. It visualizes circular mean and mean
%   resultant length for all cells on a polar plot. Edit code to modify
...
subplot(2,2,1)
ftm = [0 ftm_as.Po];
CMP = compass(real(0.5*exp(1).^(i*0.1)),imag(0.5*exp(1).^(i*0.1)));
set(CMP,'Color','white')
hold on
CMP = [];
for k = 2:length(ftm)
