function [rHx,rHy,rHxy,rIxy,rUxy,rUyx,cHx,cHy,cHxy,cIxy,cUxy,cUyx] = aentry2(eeg,vdisc,sr)
%AENTRY2   Runs entropy on a sequence of recordings.
%   AENTRY2 calculates wavelet power entropy using 1000-point non-overlapping
...
%
%   Selected frequency band: 30 - 100 Hz.
%
...
% Entropy
fnd = find(f>100);    % frequency band bounderies
pwind1 = fnd(end);
fnd = find(f<30);
pwind2 = fnd(1);
...
lag1 = 0.72;
param = -1;
mif = 25;          %minimal intresting frequency
mis = find(f>mif);
