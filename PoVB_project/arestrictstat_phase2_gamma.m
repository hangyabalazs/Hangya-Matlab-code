function arestrictstat_phase2_gamma
%ARESTRICTSTAT_PHASE2_GAMMA   Mean phase histograms for bounded frequency band.
%   ARESTRICTSTAT_PHASE2_GAMMA calculates summary beta phase statistics for
%   Po, VPM, PoVPM, LD and nRT data. Original phase histograms are
%   downsampled (to 100 elements) and pooled to provide balanced sets for
%   circular tests. Edit code to modify input and output directories!
%
...
inpdir2 = [DATAPATH 'Andi\Ketxyl\FreBandRestrict_phase_spindle_stand\'];   % phase analysis data
tabledir = ['Y:\_Projects\AUJ_ISTVAN\TABLES\'];
resdir = [DATAPATH 'Andi\Ketxyl\RestrictStat_spindle_stand_newtest\'];
mm = pwd;
dr = dir(inpdir);
...
for o = 3:length(dr)
    ...
    inx = [find(strcmp({atx{:,1}},fname)) find(strcmp({atx{:,2}},fname))];
    loc = atx{inx,3};   % localization
    
    n_fs = length(aang_fs);     % burst first spikes
    z = n_fs * (mvl_fs ^ 2);  % Rayleigh's Z statistic
    p_fs = exp(1) ^ (-1 * z) * (1 + (2 * z - z ^ 2) / ...
        (4 * n_fs) - (24 * z - 132 * z ^ 2 + 76 * z ^ 3 - 9 * z ^ 4) / (288 * n_fs ^ 2));
    n_sp = length(aang_sp);     % single spikes
    z = n_sp * (mvl_sp ^ 2);  % Rayleigh's Z statistic
    p_sp = exp(1) ^ (-1 * z) * (1 + (2 * z - z ^ 2) / ...
        (4 * n_sp) - (24 * z - 132 * z ^ 2 + 76 * z ^ 3 - 9 * z ^ 4) / (288 * n_sp ^ 2));
    n_as = length(aang_as);     % all spikes
    z = n_as * (mvl_as ^ 2);  % Rayleigh's Z statistic
    p_as = exp(1) ^ (-1 * z) * (1 + (2 * z - z ^ 2) / ...
        (4 * n_as) - (24 * z - 132 * z ^ 2 + 76 * z ^ 3 - 9 * z ^ 4) / (288 * n_as ^ 2));
...
if ~isempty(SP.nRT)
    ...
    alla_sp.nRT = zeros(1,length(cnts));
end
...
header = {'bas' 'bic' 'localization' 'angle' 'MRL' 'beta power' 'normalized beta power'};
xlswrite('restricted_betatable',header,'Sheet1','A1');     % write Excel file
xlswrite('restricted_betatable',atx2,'Sheet1','A2');     % write Excel file
cd(mm)
