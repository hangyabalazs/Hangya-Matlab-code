function aphaserun_gamma_stand
%APHASERUN_GAMMA_STAND    Phase analysis.
%   APHASERUN_GAMMA_STAND calculates action potential phases relative to
%   beta-band LFP oscillation. Phase histograms for all spikes, burst first
%   spikes, all first spikes and single spikes are plotted and saved. A
%   single von Mises probability density function, as well as the mixture
%   of two von Mises density functions are fitted on the phase
%   distribution. EEG waveletis also calculated, plotted and saved. Edit
%   program code to modify input and output directories!
%
%   APHASERUN_GAMMA_STAND performs EEG standarization before calculation
...
resdir1 = [DATAPATH 'Andi\Ketxyl\Beta\'];
mm = pwd;
...
%     H8 = figure;
    ...
    [aang_fs dinx_fs cyclen1 feeg ahee] = aphase_gamma(eeg,vb1,sr);    % PHASE - burst first spikes
    n_fs = length(aang_fs);
    ...
    [aang_sp dinx_sp] = aphase_gamma(eeg,sspo,sr);    % PHASE - single spikes
    n_sp = length(aang_sp);
    ...
    [aang_as dinx_as] = aphase_gamma(eeg,round(vdisc/20),sr);    % PHASE - all spikes
    n_as = length(aang_as);
    ...
    [aang_ibsang aang_sspoang aang_allang cycnb] = aphaseb_gamma(eeg,sspo,ibs,vb1,sr);
...
% -------------------------------------------------------------------------
function [ang_ibsang ang_sspoang ang_allang cycnb] = aphaseb_gamma(eeg,sspo,ibs,vb1,sr)
...
% Filtering EEG
nqf = sr / 2;
flt = fir1(4096,[20 40]/nqf,'band');      % bandpass filtering
...
for k = 1:length(fn)-1
    ...
    if ~(axs < 2 * sd)  | (fn(k+1) - fn(k) < 0.025 * sr)
        seeg = feeg(fn(k):fn(k+1));
        axs = max(seeg) - min(seeg);
