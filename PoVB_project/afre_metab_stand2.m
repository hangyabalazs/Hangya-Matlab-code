function afre_metab_stand2(inpdir1)
%AFRE_METAB_STAND2   Analysis of slow and fast cycles in restricted frequency bands.
%   AFRE_METAB_STAND2(DR) calculates and saves phase histograms and cycle
%   length vs. phase scatter plots for slow (1.1-1.6 Hz) and fast (2-2.5
%   Hz) cycles in two cases: (i) analysis is restricted to those segments
%   where EEG frequency is between 1.1 and 1.6 Hz on average, (ii) analysis
%   is restricted to those segments where EEG frequency is between 2 and
%   2.5 Hz on average. Percentage of missed cycles and cycle length
%   distributions are also saved. EEG is standardized before phase
%   calculations. Input directory should be given as an argument (DR).
%
...
resdir1 = [DATAPATH 'Andi\Ketxyl\FreBandRestrict_metab5\'];
mm = pwd;
dbstop if error
...
% Phase histograms
        ...
        if 1.1 < freq & freq < 1.6        % restrict frequency band
            ...
            lvb1inx = find((vb1>ind1(k)&vb1<ind2(k)));
            libspno = intraburstnum(lvb1inx);
            [paang_fs paang_fs_slow paang_fs_fast pibspno pibspno_slow pibspno_fast...
                cycinfo_fs] = laphase_stand_plus(lfeeg,lahee,lvb1,libspno,dsr);    % PHASE - burst first spikes
            ...
            ibspno1 = [ibspno1 pibspno];
            ibspno_slow1 = [ibspno_slow1 pibspno_slow];
            ibspno_fast1 = [ibspno_fast1 pibspno_fast];
            ...
            [paang_afsp paang_afsp_slow paang_afsp_fast cycinfo_afsp pcvdmean pcvdfirst] = laphase_stand(lfeeg,lahee,lafsp,dsr);    % PHASE - all first spikes
            ...
            cvdmean_afsp1 = [cvdmean_afsp1 pcvdmean];
            cvdfirst_afsp1 = [cvdfirst_afsp1 pcvdfirst];
        ...
        if 2 < freq & freq < 2.5        % restrict frequency band
            ...
            ibspno2 = [ibspno2 pibspno];
            ibspno_slow2 = [ibspno_slow2 pibspno_slow];
            ibspno_fast2 = [ibspno_fast2 pibspno_fast];
            ...
            cvdmean_afsp2 = [cvdmean_afsp2 pcvdmean];
            cvdfirst_afsp2 = [cvdfirst_afsp2 pcvdfirst];
...
%     text(60,y_lim(2)-(y_lim(2)-y_lim(1))/6-(y_lim(2)-y_lim(1))/6,str,'Color','red')
%     subplot(2,2,2)      % single spikes - slow cycles
...
%     text(60,y_lim(2)-(y_lim(2)-y_lim(1))/6-(y_lim(2)-y_lim(1))/6,str,'Color','red')
%     subplot(2,2,3)      % single spikes - fast cycles
...
%     H = figure;      % phase histograms
%     set(gcf,'Position',[1 31 1278 920])
%     subplot(2,2,1)      % burst first spikes
...
%     text(60,y_lim(2)-(y_lim(2)-y_lim(1))/6-(y_lim(2)-y_lim(1))/6,str,'Color','red')
%     subplot(2,2,2)      % burst first spikes - slow cycles
...
%     text(60,y_lim(2)-(y_lim(2)-y_lim(1))/6-(y_lim(2)-y_lim(1))/6,str,'Color','red')
%     subplot(2,2,3)      % burst first spikes - fast cycles
...
%     H = figure;      % phase histograms
%     set(gcf,'Position',[1 31 1278 920])
%     subplot(2,2,1)      % all first spikes
...
%     text(60,y_lim(2)-(y_lim(2)-y_lim(1))/6-(y_lim(2)-y_lim(1))/6,str,'Color','red')
%     subplot(2,2,2)      % all first spikes - slow cycles
...
%     text(60,y_lim(2)-(y_lim(2)-y_lim(1))/6-(y_lim(2)-y_lim(1))/6,str,'Color','red')
%     subplot(2,2,3)      % all first spikes - fast cycles
...
%     subplot(2,2,1)
%     phss = cvdmean_afsp1;
%     cycs = cycinfo_afsp1.all;
%     plot(cycs,phss,'k.')
%     hold on
%     plot(cycs(cycs<1/1.1&cycs>1/1.6),phss(cycs<1/1.1&cycs>1/1.6),'b.')
%     plot(cycs(cycs<1/2&cycs>1/2.5),phss(cycs<1/2&cycs>1/2.5),'r.')
%     xlabel('cycle length')
%     ylabel('mean phase')
% %     [gr icp err] = linefit(cycs(~isnan(phss)),cvdmean_afsp1(~isnan(phss)));  % fit line
%     coef = polyfit(cycs(~isnan(phss)),cvdmean_afsp1(~isnan(phss)),1);
%     gr = coef(1);
%     icp = coef(2);
%     x = [min(cycs):0.01:max(cycs)];
%     y = x .* gr + icp;
%     plot(x,y,'g')
%     [b,bint,r,rint,stats] = regress(cycs',[ones(length(phss),1) phss']);
%     R1 = sqrt(stats(1));         % correlation coefficient (R-value of the regression)
%     preR = corrcoef(cycs(~isnan(phss)),phss(~isnan(phss)));
%     R = preR(2);
...
%     subplot(2,2,2)
%     phss = cvdmean_afsp1;
%     cycs = 1 ./ cycinfo_afsp1.all;
%     plot(cycs,phss,'k.')
%     hold on
%     plot(cycs(cycs>1.1&cycs<1.6),phss(cycs>1.1&cycs<1.6),'b.')
%     plot(cycs(cycs>2&cycs<2.5),phss(cycs>2&cycs<2.5),'r.')
...
%     xlabel('cycle length')
%     ylabel('first phase')
% %     [gr icp err] = linefit(cycs(~isnan(phss)),cvdfirst_afsp1(~isnan(phss)));  % fit line
%     coef = polyfit(cycs(~isnan(phss)),cvdfirst_afsp1(~isnan(phss)),1);
%     gr = coef(1);
%     icp = coef(2);
%     x = [min(cycs):0.01:max(cycs)];
%     y = x .* gr + icp;
...
%     text((x_lim(2)-x_lim(1))/2+x_lim(1),(y_lim(2)-y_lim(1))*0.8+y_lim(1),['F: ' num2str(F)])
%     text((x_lim(2)-x_lim(1))/2+x_lim(1),(y_lim(2)-y_lim(1))*0.7+y_lim(1),['p (F-test): ' num2str(p)])
%     fns = [fname(1:end-4) '_CVSP1.fig'];
%     saveas(H,fns)
%     fns = [fname(1:end-4) '_CVSP1.jpg'];
%     saveas(H,fns)
end
...
%     xlabel('cycle length')
%     ylabel('mean phase')
% %     [gr icp err] = linefit(cycs(~isnan(phss)),cvdmean_afsp2(~isnan(phss)));  % fit line
%     coef = polyfit(cycs(~isnan(phss)),cvdmean_afsp2(~isnan(phss)),1);
%     gr = coef(1);
%     icp = coef(2);
%     x = [min(cycs):0.01:max(cycs)];
%     y = x .* gr + icp;
...
%     xlabel('cycle length')
%     ylabel('first phase')
% %     [gr icp err] = linefit(cycs(~isnan(phss)),cvdfirst_afsp2(~isnan(phss)));  % fit line
%     coef = polyfit(cycs(~isnan(phss)),cvdfirst_afsp2(~isnan(phss)),1);
%     gr = coef(1);
%     icp = coef(2);
%     x = [min(cycs):0.01:max(cycs)];
%     y = x .* gr + icp;
...
%     text((x_lim(2)-x_lim(1))/2+x_lim(1),(y_lim(2)-y_lim(1))*0.8+y_lim(1),['F: ' num2str(F)])
%     text((x_lim(2)-x_lim(1))/2+x_lim(1),(y_lim(2)-y_lim(1))*0.7+y_lim(1),['p (F-test): ' num2str(p)])
%     fns = [fname(1:end-4) '_CVSP2.fig'];
%     saveas(H,fns)
%     fns = [fname(1:end-4) '_CVSP2.jpg'];
%     saveas(H,fns)
end
% xlsname = [resdir1 'angles.xls'];   % write results to excel file
...
% xlswrite(xlsname,{fname},'sheet2',['A' num2str(pref)])
% xlswrite(xlsname,{fname},'sheet3',['A' num2str(pref)])
% if ~isempty(aang_afsp1)
%     xlswrite(xlsname,ang_afsp1,'sheet1',['B' num2str(pref)])
%     xlswrite(xlsname,ang_afsp_slow1,'sheet1',['C' num2str(pref)])
%     xlswrite(xlsname,ang_afsp_fast1,'sheet1',['D' num2str(pref)])
%     xlswrite(xlsname,mvl_afsp1,'sheet2',['B' num2str(pref)])
%     xlswrite(xlsname,mvl_afsp_slow1,'sheet2',['C' num2str(pref)])
%     xlswrite(xlsname,mvl_afsp_fast1,'sheet2',['D' num2str(pref)])
%     xlswrite(xlsname,n_afsp1,'sheet3',['B' num2str(pref)])
%     xlswrite(xlsname,n_afsp_slow1,'sheet3',['C' num2str(pref)])
%     xlswrite(xlsname,n_afsp_fast1,'sheet3',['D' num2str(pref)])
% end
% if ~isempty(aang_afsp2)
%     xlswrite(xlsname,ang_afsp2,'sheet1',['E' num2str(pref)])
%     xlswrite(xlsname,ang_afsp_slow2,'sheet1',['F' num2str(pref)])
%     xlswrite(xlsname,ang_afsp_fast2,'sheet1',['G' num2str(pref)])
%     xlswrite(xlsname,mvl_afsp2,'sheet2',['E' num2str(pref)])
%     xlswrite(xlsname,mvl_afsp_slow2,'sheet2',['F' num2str(pref)])
%     xlswrite(xlsname,mvl_afsp_fast2,'sheet2',['G' num2str(pref)])
%     xlswrite(xlsname,n_afsp2,'sheet3',['E' num2str(pref)])
%     xlswrite(xlsname,n_afsp_slow2,'sheet3',['F' num2str(pref)])
%     xlswrite(xlsname,n_afsp_fast2,'sheet3',['G' num2str(pref)])
% end
% xlsname = [resdir1 'angles_50.xls'];   % write results to excel file
% if b_isfilename(xlsname)
%     [ntz mtz atz] = xlsread(xlsname,'sheet1');
%     pref = size(atz,1) + 1;
% else
...
xlsname = [resdir1 'ibspno.xls'];   % write results to excel file
if b_isfilename(xlsname)
    [ntz mtz atz] = xlsread(xlsname,'sheet1');
    pref = size(atz,1) + 1;
else
...
xlswrite(xlsname,{fname},'sheet3',['A' num2str(pref)])
if ~isempty(ibspno1)
    xlswrite(xlsname,mean(ibspno1),'sheet1',['B' num2str(pref)])
    xlswrite(xlsname,mean(ibspno_slow1),'sheet1',['C' num2str(pref)])
    xlswrite(xlsname,mean(ibspno_fast1),'sheet1',['D' num2str(pref)])
    xlswrite(xlsname,length(ibspno1),'sheet2',['B' num2str(pref)])
    xlswrite(xlsname,length(ibspno_slow1),'sheet2',['C' num2str(pref)])
    xlswrite(xlsname,length(ibspno_fast1),'sheet2',['D' num2str(pref)])
end
if ~isempty(ibspno2)
    xlswrite(xlsname,mean(ibspno2),'sheet1',['E' num2str(pref)])
    xlswrite(xlsname,mean(ibspno_slow2),'sheet1',['F' num2str(pref)])
    xlswrite(xlsname,mean(ibspno_fast2),'sheet1',['G' num2str(pref)])
    xlswrite(xlsname,length(ibspno2),'sheet2',['E' num2str(pref)])
    xlswrite(xlsname,length(ibspno_slow2),'sheet2',['F' num2str(pref)])
    xlswrite(xlsname,length(ibspno_fast2),'sheet2',['G' num2str(pref)])
end
...
% -------------------------------------------------------------------------
function [ang1 ang2 ang3 cycinfo cycvdmean cycvdfirst] = laphase_stand(feeg,ahee,vdisc,sr)
...
%
%   [A1 A2 A3 CI AM AF] = LAPHASE_STAND(FEEG,AHEE,VDISC,SR) returns mean 
%   (AM) and first (AF) phase for within cycle events, calculated for all 
%   cycles.
%
%   See also HILBERT.
...
for k = 1:length(fn)-1
    ...
    if (axs < 2 * sd)  | (fn(k+1) - fn(k) < 0.25 * sr)
        ...
        if isempty(cycvd)
            ...
            cycvdmean = [cycvdmean NaN];
            cycvdfirst = [cycvdfirst NaN];
        else
            cvdmean = circular_mean(ahee(cycvd),'rad');
            cycvdmean = [cycvdmean cvdmean];
            cvdfirst = ahee(cycvd(1));
            cycvdfirst = [cycvdfirst cvdfirst];
        end
        if cf > 1.1 & cf < 1.6
            allcyc2 = [allcyc2 cl];
            if isempty(cycvd)
...
% -------------------------------------------------------------------------
function [ang1 ang2 ang3 ibspno1 ibspno2 ibspno3 cycinfo cycvdmean cycvdfirst] = ...
    laphase_stand_plus(feeg,ahee,vdisc,intraburstnum,sr)
%LAPHASE_STAND_PLUS    Phase angles for unit relative to EEG.
%   [A1 A2 A3] = LAPHASE_STAND_PLUS(FEEG,AHEE,VDISC,IBSPNO,SR) calculates
%   Hilbert phase angles (A1) for discriminated unit (VDISC) relative to
%   filtered EEG (FEEG), when sampling frequency is given in SR and
%   Hilbert-transform of the EEG in AHEE. It also returns angle sets
%   corresponding to slow (1.1-1.6) and fast (2-2.5 Hz) cycles (A2). Cycles
%   not fulfilling the following 2 criteria are discarded: (i) EEG amp.
%   higher then 2SD; (ii) min. 250 ms length. Indices of discarded spikes
%   of vdisc are returned in I.
%
%   [A1 A2 A3 I1 I2 I3] = LAPHASE_STAND_PLUS(FEEG,AHEE,VDISC,IBSPNO,SR)
%   returns intraburst spike number for all non-excluded (I1), slow (I2)
%   and fast (I3) cycles.
%
%   [A1 A2 A3 I1 I2 I3 CI] = LAPHASE_STAND_PLUS(FEEG,AHEE,VDISC,IBSPNO,SR)
%   returns cycle information in CI. The fields of the CI structure
%   contains the length of all cycles, slow/fast cycles and missed cycles.
%
%   [A1 A2 A3 I1 I2 I3 CI AM AF] =
%   LAPHASE_STAND_PLUS(FEEG,AHEE,VDISC,IBSPNO,SR) returns mean (AM) and
%   first (AF) phase for within cycle events, calculated for all cycles.
%
%   See also HILBERT.
...
for k = 1:length(fn)-1
    ...
    sahee = ahee(fn(k):fn(k+1));
    cycvd = vdisc(vdisc>fn(k)&vdisc<fn(k+1));
    cycvdinx = find(vdisc>fn(k)&vdisc<fn(k+1));
    libspno = intraburstnum(cycvdinx);
    if (axs < 2 * sd)  | (fn(k+1) - fn(k) < 0.25 * sr)
        inx1 = [inx1 find(vdisc>fn(k)&vdisc<fn(k+1))];
    else
        allcyc = [allcyc cl];
        ibspno1 = [ibspno1 libspno];
        if isempty(cycvd)
            missedcyc = [missedcyc cl];
            cycvdmean = [cycvdmean NaN];
            cycvdfirst = [cycvdfirst NaN];
