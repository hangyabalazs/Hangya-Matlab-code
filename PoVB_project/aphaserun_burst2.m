function aphaserun_burst2
%APHASERUN_BURST2    Phase analysis.
%   APHASERUN_BURST2 calculates action potential phases relative to slow
%   LFP oscillation. It calculates the dependence of the phase distribution
%   on the burst parameters (intraburst spike no., intraburst frequency,
%   burst length. Phase histograms for all spikes, burst first spikes, all
%   first spikes and single spikes are plotted and saved, along with the
%   corresponding normalized Spike Triggered Averages. A single von Mises
%   probability density function, as well as the mixture of two von Mises
%   density functions are fitted on the phase distribution. EEG wavelet and
%   autosorrelation are also calculated, plotted and saved. Edit program
%   code to modify input and output directories!
%
%   See also APHASE4, ASTANORM, WATSON2 and WATSONTWOFIT2.

% Directories
global DATAPATH
inpdir1 = 'Y:\_Projects\AUJ_ISTVAN\DATA\MAT\control\';    % mat files
inpdir2 = [DATAPATH 'Andi\Ketxyl\Cluster\mat2\control\'];   % burst analysis data
resdir1 = [DATAPATH 'Andi\Ketxyl\AllPhase_control\'];
mm = pwd;
...
for o = 1:sf
    ...
    [aang_fs dinx_fs] = aphase4(eeg,vb1,sr);    % PHASE - burst first spikes
    n_fs = length(aang_fs);
    ...
    edges = [-180:20:180];
    [nm_fs,xout_fs] = hist(aang_fs,edges);   % phase histogram
    ...
    [aang_sp dinx_sp] = aphase4(eeg,sspo,sr);    % PHASE - single spikes
    n_sp = length(aang_sp);
    ...
    [nm_sp,xout_sp] = hist(aang_sp,edges);   % phase histogram
    
    [aang_as dinx_as] = aphase4(eeg,round(vdisc/20),sr);    % PHASE - all spikes
    n_as = length(aang_as);
    ...
    [nm_as,xout_as] = hist(aang_as,edges);   % phase histogram
    ...
    [aang_afsp dinx_afsp] = aphase4(eeg,afsp,sr);    % PHASE - all first spikes
    ...
    [nm_afsp,xout_afsp] = hist(aang_afsp,edges);   % phase histogram
    ...
    [nm_ibsang,xout_ibsang] = hist(aang_ibsang,edges);   % phase histogram
    ...
    [nm_sspoang,xout_sspoang] = hist(aang_sspoang,edges);   % phase histogram
    ...
    [nm_allang,xout_allang] = hist(aang_allang,edges);   % phase histogram
...
%     colormap(flipud(jet))
    ...
    bar(xout_as,nm_as/length(aang_as))
    ...
    bar(xout_afsp,nm_afsp/length(aang_afsp))
    title(gca,[titlestr ' all first spikes'])
    y_lim = ylim;
    axis([-200 200 y_lim(1) y_lim(2)])
    str = ['\it{Mean angle: }' '\bf ' num2str(ang_afsp)];
    ...
    bar(xout_fs,nm_fs/length(aang_fs))
    ...
    bar(xout_sp,nm_sp/length(aang_sp))
    ...
    bar(xout_allang,nm_allang/length(aang_allang))
    ...
    bar(xout_ibsang,nm_ibsang/length(aang_ibsang))
    ...
    bar(xout_sspoang,nm_sspoang/length(aang_sspoang))
