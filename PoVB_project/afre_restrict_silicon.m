function afre_restrict_silicon
%AFRE_RESTRICT_SILICON   Phase analysis restricted to a frequency band.
%   AFRE_RESTRICT_SILICON calculates and saves phase analysis results
%   for a given EEG frequency band (1.1 - 1.6 Hz) of silicon probe data.
%   See APHASE_STAND for details on the analysis.
%
...
resdir = [DATAPATH 'Andi\Ketxyl\FreBandRestrict_phase_silicon\'];
mm = pwd;
...
for o = 1:sf
    ...
    len = length(eeg);
    clear eeg1 eeg2
    ff = [inpdir fname '.res.1'];       % load unit
    res = textread(ff);     % spike timestamps
    ff = [inpdir fname '.clu.1'];
