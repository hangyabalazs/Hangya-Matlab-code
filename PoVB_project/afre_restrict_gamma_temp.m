function afre_restrict(inpdir1)
%AFRE_RESTRICT   Phase and burst analysis restricted to a frequency band.
%   APHASERUN_CORR4B(DR) calculates and saves phase and burst analysis
%   results for a given EEG frequency band. See ACLUSTERCUT and
%   APHASE_BURST2_STAND for details on the analysis.
...
resdir1 = [DATAPATH 'Andi\Ketxyl\FreBandRestrict_phase_gamma\'];
% resdir2 = [DATAPATH 'Andi\Ketxyl\FreBandRestrict_burst_gamma\'];
mm = pwd;
...
% Progress indicator
[wb,awb1,awb2] = waitbar2([0 0],'Running AFRE RESTRICT...');
global WB
WB(end+1) = wb;
...
for o = 1:sf
    ...
    flt = fir1(4096,[10 20]/nqf,'band');      % bandpass filtering
...
save(fn,'aang_fs','aang_sp','aang_as','aang_afsp','aang_ibsang',...
    ...
    'mvl_ibsang','mvl_sspoang','mvl_allang','cycnb');
...
for k = 1:length(fn)-1
    ...
    if (axs < 2 * sd)  | (fn(k+1) - fn(k) < 0.05 * sr)
