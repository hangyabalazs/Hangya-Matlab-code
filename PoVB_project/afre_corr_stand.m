function afre_corr_stand(inpdir1)
%AFRE_CORR_STAND   Correlation between EEG frequency and firing pattern.
%   AFRE_CORRSTAND(DR) calculates and saves correlation between EEG
%   frequency and
%       (i) mean phase angle of all first spikes (circular-linear corr.)
%       (ii) mean resultant length (iii) STA index
%       (iv) burstiness
...
%   variables are saved. EEG is standardized for phase calculations.
%
...
resdir = [DATAPATH 'Andi\Ketxyl\AfreCorr_stand_resubmission\'];
mm = pwd;
...
% Progress indicator
[wb,awb1,awb2] = waitbar2([0 0],'Running AFRE CORR STAND...');
global WB
WB(end+1) = wb;
