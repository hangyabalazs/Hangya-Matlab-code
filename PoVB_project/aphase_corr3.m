function aphase_corr3
%APHASE_CORR3   Correlation between EEG frequency and firing pattern.
%   APHASE_CORR3 calculates and saves correlation between 
%       (i) mean phase angle of all first spikes and EEG frequency
%       (ii) burstiness and EEG frequency.
%
%   APHASE_CORR3 calculates phase angles for the burst first spikes only.
%
%   See also APHASE and APHASERUN_BURST2.

% Directories
global DATAPATH
inpdir1 = 'X:\In_Vivo\_analysis\acsady_csoport\auj_istvan\mat_ket_xyl_control\AUJ31_n1\';    % mat files
inpdir2 = [DATAPATH 'Andi\Ketxyl\Cluster\mat2\control\'];   % burst analysis data
resdir = [DATAPATH 'Andi\Ketxyl\PhaseCorr3\'];
mm = pwd;
...
% Progress indicator
[wb,awb1,awb2] = waitbar2([0 0],'Running APHASE CORR3...');
global WB
WB(end+1) = wb;
...
% Phase histograms
        ...
        lvb1 = round(lvb1/20);   % downsample unit on 1000 Hz
        
        [aang_fs dinx_fs cyclen_fs] = laphase(eeg2,lvb1,dsr);    % PHASE - burst first spikes
        n_fs = length(aang_fs);
        ...
        freq(k) = 1 / cyclen_fs * 1000;
        meanangle(k) = ang_fs;
        
        nm = histc(aang_fs,edges);   % phase histogram
        Phist(:,k) = nm(1:end-1)';
