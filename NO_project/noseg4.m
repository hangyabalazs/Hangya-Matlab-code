function noseg4
%NOSEG4   Analysis of theta and non-theta segments (NO project).
%   NOSEG4 calculates the amplitude (SD) of theta segments in the last 1800
%   s for pre-injection files or in the first 3600 s in post-injection
%   files. The results are plotted against time and also saved in an Excel
%   file.
%
%   See also NOSEG.
...
% Progress indicator
wb = waitbar(0,'Running NOSEG4...','Position',[360 250 275 50]);
global WB
WB(end+1) = wb;
...
for o = 1:sf
    ...
    ntf = ThetaSegments(1,:);       % theta segment SD
    ntl = ThetaSegments(2,:);
    ntlen = (ntl - ntf) / sr;
    if strcmp(id2,'pre')
    ...
    for k = 1:lenin    % segment loop
        ...
        tp(k) = std(eeg(ntfi(k):ntli(k)));
        plot(mean(thinx1,thinx2),tp(k),'.r','MarkerSize',16)
    end
    tps(o) = mean(tp);
    fntp = [resdir 'ThetaSD\' fname(1:end-4) '_TSD.fig'];
    saveas(H,fntp)
    close(H)
    waitbar(o/sf)
end
...
% Save
xlswrite([resdir 'thetasd.xls'],names','sheet1','A1')
xlswrite([resdir 'thetasd.xls'],tps','sheet1','B1')
