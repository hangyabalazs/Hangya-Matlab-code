function [c,lags,rccg] = xcorr_wrand(x,varargin)
%XCORR_WRAND Cross-correlation function estimates.
%   C = XCORR_WRAND(A,B), where A and B are length M vectors (M>1), returns
...
%   vector.
%
%   XCORR_WRAND produces an estimate of the correlation between two random
...
%   signals.
%
%   XCORR_WRAND(A), when A is a vector, is the auto-correlation sequence.   
%   XCORR_WRAND(A), when A is an M-by-N matrix, is a large matrix with
%   2*M-1 rows whose N^2 columns contain the cross-correlation sequences
%   for all combinations of the columns of A. The zeroth lag of the output
%   correlation is in the middle of the sequence, at element or row M.
%
%   XCORR_WRAND(...,MAXLAG) computes the (auto/cross) correlation over the
...
%
%   XCORR_WRAND(...,SCALEOPT), normalizes the correlation according to SCALEOPT:
...
%
%   [C,LAGS,RCCG] = XCORR_WRAND(X,Y,MAXLAG,SCALEOPT,RANDVEC) or 
%   XCORR_WRAND(X,Y,MAXLAG,RANDVEC) uses RANDVEC to calculate shifted
%   cross-correlations (RCCG), which can be used for confidence interval
%   calculations. RANDVEC should contain numbers greater than MAXLAG. The
%   rows of RCCG correspond to the elements of RANDVVEC.
%
...
rccg = nan(ncr,ll);
for k = 1:ncr
    rccg(k,:) = c(randVec(k)-maxlag+1:randVec(k)-maxlag+ll,:)';
end
