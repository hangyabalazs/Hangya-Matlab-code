function [gr icp err] = linefit3(x,y)
%LINEFIT3   Fits line on points using exact distances for minimalization.
%   [GR ICP ERR] = LINEFIT3(X,Y) fits line on the point-pairs in X and Y and
...
%
%   LINEFIT3 uses y-wise distance to choose between the two roots to avoid
%   scaling-dependence of this choice.
%
%   Reference:
...
a1 = sy - b1 * sx;
a2 = sy - b2 * sx;
Rsquare1 = sum((y-(a1+x.*b1)).^2);
Rsquare2 = sum((y-(a2+x.*b2)).^2);
