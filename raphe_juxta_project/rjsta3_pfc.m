function rjsta3_pfc
%RJSTA3_PFC   Spike Triggered Average and phase analysis (raphe juxta project).
%   RJSTA3_PFC calculates and saves the following outputs, relative to the
%   prefrontal cortex EEG:
%       theta modulation
%       firing rate
%       firing phase during theta segments greater than 3 s.
%       normalized STA amplitude
%       firing phase during non-theta segments relative to delta
%           oscillation
%
...
% Progress indicator
wb = waitbar(0,'Running RJSTA3 PFC...','Position',[360 250 275 50]);
global WB
WB(end+1) = wb;
...
for o = 16:sf
    ...
    flt = fir1(4096,4/nqf,'low');      % lowpass filtering on 5 Hz
    feeg = filtfilt(flt,1,eeg2);
    feeg = (feeg - mean(feeg)) / std(feeg);
    ahee = angle(hilbert(feeg));    % Hilbert-transformation
    [aang cycinfo] = laphase_stand(feeg,ahee,VdiscNonTheta2,dsr);    % PHASE - all first spikes
    [aang ang mvl ftm n nm] = phist(aang);
    H = figure;      % phase histogram
    bar(cnts,nm'/length(aang))
    cmps = strread(fname,'%s','delimiter','_');
    title(gca,[titlestr])
    y_lim = ylim;
    ...
    fnph = [fname(1:end-4) '_nontheta_PFC_phase'];
    saveas(H,fnph);
...
% -------------------------------------------------------------------------
function [ang cycinfo] = laphase_stand(feeg,ahee,vdisc,sr)
%LAPHASE_STAND    Phase angles for unit relative to EEG.
%   A1 = LAPHASE_STAND(FEEG,AHEE,VDISC,SR) calculates Hilbert phase 
...
%
%   [A1 CI] = LAPHASE_STAND(FEEG,AHEE,VDISC,SR) returns cycle
