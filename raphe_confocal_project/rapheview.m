...
%     ml = find(thlen==max(thlen));
    ...
    ThetaFiringRate = zeros(1,th_index);
    ...
    for t = 1:th_index      % theta segment cycle
        ...
        if ~isempty(vdisc_theta)        % firing rate and theta-modulation
            ...
            ThetaFiringRate(t) = (length(vdisc_theta) - 1) / efflen;
...
%     nml = find(nthlen==max(nthlen));
    NonThetaFiringRate = zeros(1,no_index);
    ...
    for t = 1:no_index      % non-theta segment cycle
        ...
        if ~isempty(vdisc_noth)        % firing rate and theta-modulation
            ...
            NonThetaFiringRate(t) = (length(vdisc_noth) - 1) / efflen;
            ...
            NonThetaFiringRate(t) = NaN;
            NonThetaThetamod(t) = NaN;
        end
    end
    fn = [fname(1:end-4) '_PhaseFrateThetamod.mat'];   % save
    save(fn,'ThetaAng','ThetaMvl','ThetaFiringRate','ThetaThetamod',...
        'NonThetaFiringRate','NonThetaThetamod');
    xlsout{o,1} = fname(1:end-4);   % Excel output
    xlsout{o,2} = circular_mean(ThetaAng(~isnan(ThetaAng)),'rad') * 180 / pi;
    xlsout{o,3} = b_mean_nonnan(ThetaMvl);
    xlsout{o,4} = b_mean_nonnan(ThetaFiringRate);
    xlsout{o,5} = b_mean_nonnan(ThetaThetamod);
    xlsout{o,6} = b_mean_nonnan(NonThetaFiringRate);
