function [zmaxloc_t,ang_t,mvl_t zmaxloc_c,ang_c,mvl_c] = b_wzshift2(tI,cI,vdtI,vdcI)
%WZSHIFT2   Z-shift method for Tisza data.
%   WZSHIFT2 implements z-shift method for Tisza runoff data. See ZSHIFT and
...
%
%   WZSHIFT2 considers only common events, i.e. events which has a
%   corresponding event in the other discriminated time series within 3
%   days.
%
%   See also ZSHIFT, ZSHIFTRUN2 and WZSHIFT.

% Find common peaks
vdtI2 = compeaks(vdtI,vdcI);
vdcI2 = compeaks(vdcI,vdtI);

% Z-shift
[zmaxloc_t,ang_t,mvl_t] = zshift(vdtI2,cI);
[zmaxloc_c,ang_c,mvl_c] = zshift(vdcI2,tI);
...
% -------------------------------------------------------------------------
function vdisc1b = compeaks(vdisc1,vdisc2)

vdisc1b = [];
for k = 1:length(vdisc1)
    pos = vdisc1(k);
    lg = any(vdisc2>=(pos-3)&vdisc2<=(pos+3));
    if lg
        vdisc1b(end+1) = pos;
    end
end