function sgphasestat_5chan
%SGPHASESTAT_5CHAN   Comparison of phase distributions on expectancy data.
%   SGPHASESTAT_5CHAN compares delta phases recorded on Fz, C3, Cz, C4 and
%   Pz electrodes under different conditions (10%, 37%, 64% and 91%) by
%   Watson-test. Concentration parameters are compared by an F-test due to
%   Fisher (see KAPPACOMPARE for details). Results are plotted and saved
%   along with circular means. Edit code to modify input and output
%   directories!
%
...
resdir = [DATAPATH 'Expectancy\kappacompare_5chan_cueonset\'];
mm = pwd;
cd(resdir)

% Import
ff = [inpdir 'exp1cue_deltaphase_singleEEGHILB.mat'];
load(ff)
data = singleEEGHILB;
