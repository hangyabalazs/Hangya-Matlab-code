function sgphaselatcorr2
%SGPHASELATCORR2   Correlation between delta phase and P300 latency.
%   SGPHASELATCORR2 calculates linear-circular correlation between 0 ms
%   delta phase and P300 latency. Latency is plotted against sorted
%   deltaphase and regression results are displayed on the plot. Delta
%   phase histograms with mean latency in each phase bin are also plotted
%   and saved.
%
%   SGPHASELATCORR2 excludes data where the amplitude maximum locaction
...
%
%   See also SGPHASELATCORR and SGPHASEAMPCORR2.
...
resdir = [DATAPATH 'Expectancy\FzCzPz_SE\phase_lat2\'];
mm = pwd;
cd(resdir)
...
for dim2 = 1:4
    for dim3 = 1:3
        ...
        saveas(H1,['sorted_deltaphase_lat ' titlestr])    % save
%         saveas(H2,['hist_deltaphase_lat ' titlestr])
    end
end
...
deltaphase = cell(1,13);    % extract delta phase and latency
lat = cell(1,13);
spvr_delta = zeros(13,100,2);
for k = 1:13
    deltaphase{k} = squeeze(data(k,dim2,dim3,1,:));
    lat{k} = squeeze(data(k,dim2,dim3,3,:));
    deltaphase_vs_lat = squeeze(data(k,dim2,dim3,[1 3],:))';
...
subplot(2,1,1)      % conditional mean latency: E(lat|phi1<phase<phi2)
