function mspoptimaltau2(ratno)
%MSPOPTIMALTAU2   Optimal time lag for TE calculation.
%   MSPOPTIMALTAU2 calculates transfers entropy for the all theta segments
%   either spontane or induced and longest non-theta.
%   It saves optimal tau values for
...
const = sr / dsr;
mm = ['s'; 'p'; 'n'];
parfor (o =  1:sf)    % no. of the measurement
            ...
            for segno = 1:size(segs,2) % analyze each segments
                ...
                for shankno = 1:4
                    ...
                    for nc = 1:ncells
                        ...
                        cDF_seg_eu = zeros(1,length(T));
                        cNTE_seg_eu = zeros(1,length(T));
                        cNTE_seg_ue = zeros(1,length(T));
                        next = 1;
                            ...
                            cTE.seg.eu = [cTE.seg.eu TE_seg_eu];
                            cTE.seg.ue = [cTE.seg.ue TE_seg_ue];
                            cNTE.seg.eu = [cNTE.seg.eu NTE_seg_eu];
                            cNTE.seg.ue = [cNTE.seg.ue NTE_seg_ue];
                            cBias.seg.eu = [cBias.seg.eu TE_seg_eu_bias];
                            cBias.seg.eu_shuffled = [cBias.seg.eu_shuffled TE_seg_eu_shuffled_bias];
                            cBias.seg.ue = [cBias.seg.ue TE_seg_ue_bias];
                            cBias.seg.ue_shuffled = [cBias.seg.ue_shuffled TE_seg_ue_shuffled_bias];
                            cDF_seg_eu(next) = DF_seg_eu;
                            
                            cNTE_seg_eu(next) = NTE_seg_eu;
                            cNTE_seg_ue(next) = NTE_seg_ue;
                            
                            next = next + 1;
                        end
                        mab = max(abs(cNTE_seg_eu),abs(cNTE_seg_ue));
                        seginx = find(mab==max(mab));
                        if length(seginx) > 1
                            seginx = seginx(1);
                        end
                        mdf_seg = cDF_seg_eu(seginx);
                        mDF_seg(o) = mdf_seg;
                        mtau_seg = T(seginx);
                        mTau_seg(o) = mtau_seg;
                        ...
                        H1 = figure;
                        plot(T,cDF_seg_eu,'r')
                        title('DF seg EEG->unit')
                        ffn = [tedir fname '_' num2str(ch) '_' num2str(shankno) '_' num2str(nc)...
                            '_' mm(tpp) num2str(segno) '_DFseg.fig' ];
                        saveas(H1,ffn)
                        
                        mfn = [tedir fname '_' num2str(ch) '_' num2str(shankno) '_' num2str(nc)...
                            '_' mm(tpp) num2str(segno) '_mTE.mat' ];
                        parlsave(mfn,'mdf_seg','mtau_seg')
