function trenrun3_modTP
%TRENRUN3_MODTP   Calculates transfer entropy on a sequence of recordings.
%   TRENRUN3_MODTP calculates wavelet power transfer entropy for the
%   longest theta and non-theta segments. (The segments are attached
%   together for wavelet calculation and extrema for discretisation are
%   calculated from the whole wavelet.) See TREN for detailed calculations
%   and references.
%
%   TRENRUN3_MODTP calculates optimal tau value by maximizing NTE. It
%   applies a corrected version of TP bias correction.
%
...
resdir = [DATAPATH 'TE\second_try_tauNTEmax_modTP\'];
...
H_X2FcX2_corr = max(H_X2FcX2_corr,0);
...
H_X2FcX2_X1_corr = max(H_X2FcX2_X1_corr,0);

TE_corr = H_X2FcX2_corr - H_X2FcX2_X1_corr;
TE_corr = max(TE_corr,0);
TE_bias = TE - TE_corr;