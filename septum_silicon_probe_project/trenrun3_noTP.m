function trenrun3_noTP
%TRENRUN3_NOTP   Calculates transfer entropy on a sequence of recordings.
%   TRENRUN3_NOTP calculates wavelet power transfer entropy for the longest
%   theta and non-theta segments. (The segments are attached together for
%   wavelet calculation and extrema for discretisation are calculated from
%   the whole wavelet.) See TREN for detailed calculations and references.
%
%   TRENRUN3_NOTP calculates optimal tau value by maximizing NTE. It does
%   not apply TP bias correction method.
%
%   See also TREN and TRENRUN3.
...
resdir = [DATAPATH 'TE\second_try_tauNTEmax_noTP\'];
...
mDF_noth = zeros(1,sf);
mTau_theta = zeros(1,sf);
mTau_noth = zeros(1,sf);
...
% Transfer entropy
    ...
    next = 1;
    for tau = T
        [TE H_X2FcX2] = ltren(abs1_theta,abs2_theta,...
            min_abs1,max_abs1,min_abs2,max_abs2,1000,tau);  % theta, EEG->unit
        TE_theta_eu = TE;
        TE = ltren(abs1_theta,abs3_theta,...
        ...
        [TE H_X2FcX2] = ltren(abs2_theta,abs1_theta,...
            min_abs2,max_abs2,min_abs1,max_abs1,1000,tau);  % theta, unit->EEG
        TE_theta_ue = TE;
        TE = ltren(abs3_theta,abs1_theta,...
...
% Plot and save
    ...
    saveas(H2,ffn)
    mfn = [filenam '_TE.mat'];
    save(mfn,'cDF_theta_eu','cDF_noth_eu','cTE','cNTE')
    mfn = [filenam '_mTE.mat'];
