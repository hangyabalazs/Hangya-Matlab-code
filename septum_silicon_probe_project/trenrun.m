function trenrun
%TRENRUN   Calculates transfer entropy on a sequence of recordings.
%   TRENRUN calculates wavelet power transfer entropy for the longest theta
...
%
%   TRENRUN calculates optimal tau value by maximizing DF.
%
%   See also TREN.
...
resdir = [DATAPATH 'TE\second_try2\'];
...
%     cNTE.noth.ue = [];
    ...
    thetainx = find(abs(cDF_theta_eu)==max(abs(cDF_theta_eu)));
    nothinx = find(abs(cDF_noth_eu)==max(abs(cDF_noth_eu)));
    if length(thetainx) > 1
        thetainx = thetainx(1);
    elseif isempty(thetainx)
        thetainx = 1;   % in this case, cDF_theta_eu contains NaNs
    end
    if length(nothinx) > 1
        nothinx = nothinx(1);
    elseif isempty(nothinx)
        nothinx = 1;   % in this case, cDF_noth_eu contains NaNs
    end
